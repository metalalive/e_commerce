...
int app_rpc_cfg_deinit(arpc_cfg_t *cfg) {
    ...
    if(cfg->binding_reply.entries) {
        // TODO, free space allocated to RPC callee
        assert(0);
    }
...
static int parse_cfg_rpc__broker_bindings(json_t *objs, arpc_cfg_t *cfg)
{
    ...
    json_array_foreach(objs, idx, obj) {
        ...
        *bcfg = (arpc_cfg_bind_t) {.q_name = strdup(queue), .exchange_name = strdup(exchange),
        ...
        bcfg->flags = (arpc_qcfg_flg_t) {.durable=durable, .passive=1, .exclusive=0, .auto_delete=0};
        // TODO, for temp reply queue, the flags should be :
        //"durable": false, "passive": false, "exclusive": false, "auto-delete": true,
