...
static void  _api_filefetch__deinit_primitives (h2o_req_t *req, h2o_handler_t *hdlr,
...
static void  _api_cache__proceed_datablock_done (asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result,
...
{ // look for cached file
    asa_op_localfs_cfg_t  *asa_cached_local = init_fn(req->conn->ctx->loop, spec, err_info,
            NUM_USRARGS_ASA_LOCAL,  ASA_LOCAL_RD_BUF_SZ,  _api_common_cachefile_init_done, 
            _api_common_cachefile_deinit_done);
    if(asa_cached_local) {
