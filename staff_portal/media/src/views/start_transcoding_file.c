...
static __attribute__((optimize("O0"))) void _api_start_transcode__send_async_job (db_query_t *target, db_query_result_t *rs)
...
} // end of _load_filechunk_info__row_fetch
...
#define  NUM_USR_ARGS  3
    ...
    db_query_cfg_t  cfg = {
        ...
        .callbacks = {.result_rdy = app_db_async_dummy_cb,
...
static void _mark_old_transcoded_version (RESTAPI_HANDLER_ARGS(self, req), app_middleware_node_t *node)
{
    ...
    char raw_sql[raw_sql_sz];
    {
        ...
        size_t nwrite_sql = snprintf(&raw_sql[0], raw_sql_sz, _sql_patt, &param_markers[0],
