...
static  int parse_cfg_transcoder_muxer(
        json_t *obj, aav_cfg_input_t *cfg, void *(*finder)(const char *))
{
    json_t *item = NULL;
    int idx = 0;
...
static int parse_cfg_transcoder_codec(
        json_t *obj, aav_cfg_codec_t *cfg, AVCodec *(*finder)(const char *))
{
...
static int parse_cfg_transcoder_resolution_video(json_t *obj, aav_cfg_resolution_v_t *cfg)
{
    ...
    json_array_foreach(pixels, idx, item) {
        ...
        if(width <= 0 || height <= 0) {
            fprintf(stderr, "[cfg-parser][transcoder] incorrect resolution format, width:%d, height:%d \n",
                    width, height);
            goto error;
        }
...
{
    int err = 0;
    err = parse_cfg_transcoder_muxer(json_object_get(obj, "demuxers"),
            cfg, app_av_find_input_format);
    if(err) { goto error; }
    err = parse_cfg_transcoder_codec(json_object_get(obj, "decoders"),
            &cfg->decoder, avcodec_find_decoder_by_name);
    if(err) { goto error; }
    return 0;
error:
    return -1;
...
{
    int err = 0;
    err = parse_cfg_transcoder_muxer(json_object_get(obj, "muxers"),
            (aav_cfg_input_t *)cfg, app_av_find_output_format);
    if(err) { goto error; }
    err = parse_cfg_transcoder_codec(json_object_get(obj, "encoders"),
            &cfg->encoder, avcodec_find_encoder_by_name);
    if(err) { goto error; }
    err = parse_cfg_transcoder_resolution_video(
            json_object_get(obj, "video"), &cfg->resolution.video);
    if(err) { goto error; }
...
static void transcoder_cfg_deinit_common(aav_cfg_input_t *cfg) {
    ...
    if(demuxers) { free(demuxers); }
    if(codec_v ) { free(codec_v ); }
    if(codec_a ) { free(codec_a ); }
