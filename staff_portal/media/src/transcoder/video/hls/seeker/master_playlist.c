...
static  __attribute__((optimize("O0")))  ASA_RES_CODE atfp_hls__open_src_mst_plist (atfp_hls_t *hlsproc)
{
    ...
    if(entry) {
        ...
        if(hlsproc->internal.num_plist_merged == 0) {
            ...
            json_object_set_new(err_info, "transcoder", json_string("[hls] source master playlist not found"));
        }
    }
    return result;
    ...
    if (result == ASTORAGE_RESULT_COMPLETE) {
        ...
        if(!err) {
            ...
            if(num_versions_found > 0) {
                // change to another function iterating different version folders for master playlist
