...
static  void _atfp_hls_stream__segfile_read_cb (asa_op_base_cfg_t *asa_src, ASA_RES_CODE result, size_t nread)
{
    ...
    if (result == ASTORAGE_RESULT_COMPLETE) {
        ...
        success = EVP_EncryptUpdate(cipher_ctx, wr_ptr, &num_encrypt, (const unsigned char *)asa_src->op.read.dst, nread);
        if(success) {
...
static EVP_CIPHER_CTX *_atfp_hls_stream__init_cipher (json_t *spec, json_t *err_info)
{
    ...
    if(!success) {
        fprintf(stderr, "[hls][segm] line:%d, IV length mismatch in key item, expect=%d, actual=%ld \r\n",
                __LINE__, HLS__NBYTES_IV, expect_nbytes_iv);
        goto done;
    }
    success = expect_nbytes_key == HLS__NBYTES_KEY;
    if(!success) {
        fprintf(stderr, "[hls][segm] line:%d, key length mismatch in key item, expect=%d, actual=%ld \r\n",
    ...
    if(!key_octet || !iv_octet || actual_nbytes_key!=expect_nbytes_key || 
        ...
        fprintf(stderr, "[hls][segm] line:%d, hex string cannot be converted to octet array,"
                " key=%ld, IV=%ld \r\n", __LINE__, actual_nbytes_key, actual_nbytes_iv);
        success = 0; goto done;
    }
    success = EVP_EncryptInit_ex (cipher_ctx, NULL, NULL, (const unsigned char *)&key_octet[0], 
            (const unsigned char *)&iv_octet[0]);
    if(success)
...
#define  PATH_PATTERN  "%s/%d/%08x/%s/%s"
    ...
    size_t nwrite = snprintf(&filepath[0], filepath_sz, PATH_PATTERN, asa_src->storage->base_path, _usr_id,
             _upld_req_id, ATFP__COMMITTED_FOLDER_NAME, _detail_path);
#undef  PATH_PATTERN 
    ...
    asa_src->deinit = _atfp_hls__stream_seeker_asasrc_deinit; // not apply the deinit function in seeker/common.c
    return result;
