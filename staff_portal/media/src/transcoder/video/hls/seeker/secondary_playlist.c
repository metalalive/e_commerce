...
static size_t  _atfp_hls__build_lvl2_plist__modify_map_tag (json_t *spec, const char *_version,
...
#undef   URL_PATTERN
    *rd_ptr = ahead_rd_p,  ahead_rd_p = strchr(*rd_ptr, '\n');
    ...
    CONDITIONAL_WRITE(
           ...
           (*wr_ptr)[cpy_sz - 1] = 0x0; // NULL terminating string
        )
    *rd_ptr = ahead_rd_p;
done:
    return nwrite;
...
static  void _atfp_hls__open_local_keyfile_cb (asa_op_base_cfg_t *_asa_local, ASA_RES_CODE result)
{
    ...
    if (result == ASTORAGE_RESULT_COMPLETE) {
        ...
        fprintf(stderr, "[hls][lvl2_plist] line:%d, failed to open crypto key file \r\n", __LINE__);
