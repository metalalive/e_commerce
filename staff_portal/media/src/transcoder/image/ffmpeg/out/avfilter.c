...
static  AVFilterContext * _atfp_img__setup_filt_ctx (AVFilterGraph *filt_grf, const char *v_buf_label,
        const char *name, const char *arg, AVFilterInOut *_inout)
{
    AVFilterContext *ctx_out = NULL;
    const AVFilter *buffer  = avfilter_get_by_name(v_buf_label);
    if (!buffer) {
        av_log(NULL, AV_LOG_ERROR, "[atfp][img][ff_out][filter] line:%d, avfilter"
                " not found\n", __LINE__);
        goto done;
    }
    int err = avfilter_graph_create_filter(&ctx_out, buffer, name, arg,
              NULL, filt_grf);
    if ((err < 0) || (!ctx_out)) {
        av_log(NULL, AV_LOG_ERROR, "[atfp][img][ff_out][filter] line:%d, failed to"
...
void  atfp__image_dst__avfilt_init (atfp_av_ctx_t *src, atfp_av_ctx_t *dst, json_t *filt_spec, json_t *err_info)
{
    int idx = 0, err = 0;
    ...
    if(!_img_dec_ctx || !_img_enc_ctx || !_img_enc_ctx->enc_ctx ) {
        av_log(NULL, AV_LOG_INFO, "[atfp][img][ff_out][filter] line:%d,"
                " no decoder/encoder provided \n", __LINE__);
        json_object_set_new(err_info, "transcoder", json_string("[img][ff_out][filter] missing decoder/encoder"));
        err = AVERROR(EINVAL);
    } else {
        ...
        if (!filt_out || !filt_in || !_img_enc_ctx->filter_graph) {
            err = AVERROR(ENOMEM);
            json_object_set_new(err_info, "transcoder", json_string("[img][ff_out][filter]"
...
#define  NBYTES_FILTER_SPEC_RAW   256
        ...
        if(err) {
            ...
            int nwrite = snprintf( &filter_src_arg[0], NBYTES_FILTER_SPEC_RAW,
                    "video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d",
                    _img_dec_ctx->width, _img_dec_ctx->height, _img_dec_ctx->pix_fmt,
                    _img_dec_ctx->time_base.num, _img_dec_ctx->time_base.den,
                    _img_dec_ctx->sample_aspect_ratio.num, _img_dec_ctx->sample_aspect_ratio.den );
            if (nwrite >= NBYTES_FILTER_SPEC_RAW) {
                av_log(NULL, AV_LOG_ERROR, "[atfp][img][ff-out][filter] line:%d, filter"
                        " source arg error \n", __LINE__);
                json_object_set_new(err_info, "transcoder", json_string("[img][ff-out][filter]"
                    " failed to generate filter spec"));
                err = AVERROR(ENOMEM);
                goto done;
            }
...
#undef   NBYTES_FILTER_SPEC_RAW
        ...
        _img_enc_ctx->filt_src_ctx  = _atfp_img__setup_filt_ctx (_img_enc_ctx->filter_graph,
                "buffer", "in", &filter_src_arg[0], filt_out);
        _img_enc_ctx->filt_sink_ctx = _atfp_img__setup_filt_ctx (_img_enc_ctx->filter_graph,
                "buffersink", "out",  NULL, filt_in);
