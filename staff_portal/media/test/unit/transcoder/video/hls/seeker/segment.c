...
#define   RD_BUF_MAX_SZ         (sizeof(UTEST__SEGM_ORIGIN_CONTENT) + 1)
    ...
    unsigned char *mock_key_octet = (unsigned char *) "\x5D\x4A\x36\x83\x31\x75\x1A\x38\x44\xA3\x83\x13\x17\x51\xA3\x84";
    unsigned char *mock_iv_octet  = (unsigned char *) "\xD4\xA3\x83\x31\x75\x1A\x38\x45\xD4\xA3\x83\x31\x75\x1A\x38\x4E";
    ...
    if(err_cnt == 0) {
        ...
        expect(EVP_EncryptInit_ex, will_return(1), when(ctx, is_equal_to(&mock_cipher_ctx)),
                when(key, is_equal_to(NULL)),  when(iv, is_equal_to(NULL)) );
        expect(OPENSSL_hexstr2buf, will_return(mock_key_octet), when(str, is_equal_to_string(UTEST__CRYPTO_CHOSEN_KEY_HEX)),
                will_set_contents_of_parameter(len, &expect_key_octet_sz, sizeof(long)) );
        expect(OPENSSL_hexstr2buf, will_return(mock_iv_octet), when(str, is_equal_to_string(UTEST__CRYPTO_CHOSEN_IV_HEX)),
        ...
        expect(_utest_hls_enc_segm__common_done_cb, when(asa_src, is_not_null), when(err_cnt, is_equal_to(0)),
        ...
        assert_that(mock_fp.internal.op.encrypt_segment , is_equal_to(atfp_hls_stream__encrypt_segment__continue));
    }
...
#define   RD_BUF_MAX_SZ         (sizeof(UTEST__SEGM_ORIGIN_CONTENT) + 1)
    ...
    unsigned char *mock_key_octet = (unsigned char *) "\x5D\x4A\x36\x83\x31\x75\x1A\x44\xA3\x83\x13\x17\x51\xA3\x84";
    unsigned char *mock_iv_octet  = (unsigned char *) "\xD4\xA3\x83\x75\x1A\x45\xD4\xA3\x83\x31\x75\x1A\x38\x4E";
    json_object_set_new(mock_doc_metadata, "key_id", json_string(UTEST__CRYPTOKEY_CHOSEN_ID)); \
    {
        const char *_wr_buf = UTEST__CRYPTOKEY_MIN_CONTENT;
        size_t   _wr_buf_sz = sizeof(UTEST__CRYPTOKEY_MIN_CONTENT) - 1;
    ...
    if(err_cnt == 0) {
        ...
        expect(CRYPTO_free, when(addr, is_equal_to(mock_iv_octet)));
        expect(EVP_CIPHER_CTX_free, when(ctx, is_equal_to(&mock_cipher_ctx)));
        expect(_utest_hls_enc_segm__common_done_cb, when(asa_src, is_not_null), when(err_cnt, is_greater_than(0)),
                when(is_final, is_equal_to(0)), when(eof_reached, is_equal_to(0)) );
        while(!mock_done_flag)
            uv_run(loop, UV_RUN_ONCE);
        asa_op_base_cfg_t *asa_src = mock_fp.super.data.storage.handle;
...
#define   RD_BUF_MAX_SZ         (sizeof(UTEST__SEGM_ORIGIN_CONTENT) + 1)
    ...
    if(err_cnt == 0) {
        ...
        expect(EVP_aes_128_cbc, will_return(&mock_cipher_aes));
        expect(EVP_CIPHER_CTX_new, will_return(&mock_cipher_ctx));
        expect(EVP_EncryptInit_ex, will_return(success), when(ctx, is_equal_to(&mock_cipher_ctx)),
                when(key, is_equal_to(NULL)),  when(iv, is_equal_to(NULL)) );
        expect(EVP_CIPHER_CTX_free, when(ctx, is_equal_to(&mock_cipher_ctx)));
        expect(_utest_hls_enc_segm__common_done_cb, when(asa_src, is_not_null), when(err_cnt, is_greater_than(0)),
                when(is_final, is_equal_to(0)), when(eof_reached, is_equal_to(0)) );
...
#define  HLS__ENCRYPT_SEGMENT_CONTINUE__SETUP \
    ...
    asa_op_base_cfg_t  mock_asa_src = {.storage=&mock_storage_cfg, .cb_args={ \
        .entries=mock_asasrc_cb_args, .size=2}, .op={.read={.dst_max_nbytes=RD_BUF_MAX_SZ, \
        .dst=&mock_read_buf[0] }}}; \
    atfp_hls_t  mock_fp = {.super = {.data = {.callback=_utest_hls_enc_segm__common_done_cb, \
        .spec=mock_spec, .error=mock_err_info, .storage={.handle=&mock_asa_src}}}, \
...
#define   RD_BUF_MAX_SZ          sizeof(SEGM_CONTENT_CHUNK1) + 3
    ...
    { // subcase 1
        ...
        expect(EVP_EncryptUpdate, will_return(1), when(ctx, is_equal_to(&mock_cipher_ctx)),
        ...
        expect(EVP_CIPHER_CTX_block_size, will_return(16));
        expect(utest_storage_fn_read,
                will_set_contents_of_parameter(rd_data_p, SEGM_CONTENT_CHUNK2, chunk_sz),
        ...
        expect(EVP_EncryptFinal_ex, will_return(1), when(ctx, is_equal_to(&mock_cipher_ctx)),
...
#define   RD_BUF_MAX_SZ       100 
    ...
    expect(_utest_hls_enc_segm__common_done_cb, when(err_cnt, is_greater_than(0)),
            when(is_final, is_equal_to(0)), when(eof_reached, is_equal_to(0)),  
...
#define   RD_BUF_MAX_SZ          sizeof(SEGM_CONTENT_CHUNK) + 3
    ...
    ASA_RES_CODE expect_evt_result = ASTORAGE_RESULT_COMPLETE;
    {
        ...
        int success= 0;
        expect(EVP_EncryptUpdate, will_return(success), when(ctx, is_equal_to(&mock_cipher_ctx)),
