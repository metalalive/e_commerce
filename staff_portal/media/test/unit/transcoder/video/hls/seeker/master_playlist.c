...
#define  HLS__BUILD_MST_PLIST_START__SETUP \
    ...
    asa_cfg_t  mock_src_storage_cfg = {.alias=MOCK_STORAGE_ALIAS, .base_path=UTEST_ASASRC_BASEPATH, \
        .ops={ .fn_scandir=app_storage_localfs_scandir, .fn_scandir_next=app_storage_localfs_scandir_next, \
    ...
    atfp_hls_t  mock_fp = {.super = {.data = {.callback=_utest_hls_build_mst_plist_start__done_cb, \
        .spec=mock_spec, .error=mock_err_info, .usr_id=MOCK_USER_ID, .upld_req_id=MOCK_UPLD_REQ_1_ID, \
        .storage={.handle=NULL}}}, .internal={.op={.build_master_playlist=atfp_hls_stream__build_mst_plist}} \
    }; \
...
#define  UTEST_UNRELATED_FOLDER_NAME   ATFP__COMMITTED_FOLDER_NAME "/" "will_be_excluded"
#define  UTEST_UNRELATED_FILE_NAME     ATFP__COMMITTED_FOLDER_NAME "/" "will_also.be_cut"
...
Ensure(atfp_hls_test__build_mst_plist__start_ok_1) {
    ...
    { // master playlist in each version folder should provide `bandwidth` attribute in `ext-x-stream-inf` tag
        ...
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
                UTEST_UNRELATED_FOLDER_NAME, UTEST_OPS_MKDIR );
    }
...
#define  _COMMON_CODE() \
    ...
    if(err_cnt == 0) { \
        expect(_utest_hls_build_mst_plist_start__done_cb,  when(asa_src, is_not_null), when(err_cnt, is_equal_to(0)), \
        ...
        assert_that(mock_fp.internal.op.build_master_playlist, is_equal_to(atfp_hls_stream__build_mst_plist__continue)); \
        assert_that(mock_fp.internal.op.build_master_playlist, is_not_equal_to(atfp_hls_stream__build_mst_plist)); \
    }
    _COMMON_CODE()
...
Ensure(atfp_hls_test__build_mst_plist__start_ok_2) {
    ...
    { // refresh and new version is found, update master playlist
        ...
        const char *_wr_buf = "abcde"; size_t _wr_buf_sz = 5;
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
            UTEST_UNRELATED_FILE_NAME, UTEST_OPS_WRITE2FILE );
    }
    json_t  *expected_folder_names = json_object();
    json_object_set_new(expected_folder_names, UTEST_RESOURCE_VERSION_1, json_true());
    json_object_set_new(expected_folder_names, UTEST_RESOURCE_VERSION_2, json_true());
    ...
    UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
             UTEST_UNRELATED_FOLDER_NAME, UTEST_OPS_RMDIR );
    UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
             UTEST_UNRELATED_FILE_NAME, UTEST_OPS_UNLINK );
    HLS__BUILD_MST_PLIST_START__TEARDOWN
...
Ensure(atfp_hls_test__build_mst_plist__start_nonexist) {
    ...
    assert_that(err_cnt, is_equal_to(0));
    if(err_cnt == 0) {
        expect(_utest_hls_build_mst_plist_start__done_cb,  when(asa_src, is_not_null), when(err_cnt, is_greater_than(0)), \
            when(num_versions_found, is_equal_to(0)) );
        uv_run(loop, UV_RUN_ONCE);
...
#define  UTEST_OPS_DUP_SCANDIR_PATH(_path, _path_sz)  { mock_scandir_path = strndup(_path, _path_sz); }
...
#define  HLS__BUILD_MST_PLIST_CONTINUE__SETUP(_versions, _num_versions) \
    ...
    UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID, \
            ATFP__COMMITTED_FOLDER_NAME, UTEST_OPS_DUP_SCANDIR_PATH ); \
    void  *mock_asa_src_cb_args [NUM_CB_ARGS_ASAOBJ]; \
    asa_op_localfs_cfg_t  mock_asa_src = {.loop=loop, .super={.storage=&mock_src_storage_cfg, .op={ \
        .scandir={.fileinfo={.size=_num_versions, .rd_idx=0, .data=_versions}, .path=mock_scandir_path}, \
        .read={.dst_max_nbytes=RD_BUF_MAX_SZ, .dst=&mock_rd_buf[0]}}, \
        .cb_args={.entries=mock_asa_src_cb_args,  .size=NUM_CB_ARGS_ASAOBJ }}}; \
    atfp_hls_t  mock_fp = {.super = {.data = {.callback=_utest_hls_build_mst_plist_continue__done_cb, \
        .spec=mock_spec, .error=mock_err_info, .usr_id=MOCK_USER_ID, .upld_req_id=MOCK_UPLD_REQ_1_ID, \
...
#define  UTEST_RESOURCE_PATH_VERSION_1   ATFP__COMMITTED_FOLDER_NAME "/" UTEST_RESOURCE_VERSION_1
#define  UTEST_RESOURCE_PATH_VERSION_2   ATFP__COMMITTED_FOLDER_NAME "/" UTEST_RESOURCE_VERSION_2
Ensure(atfp_hls_test__build_mst_plist__continue_ok_1)
{ // parse valid data from first playlist read
    asa_dirent_t  mock_versions[UTEST_NUM_VERSIONS] = {{.type=ASA_DIRENT_DIR, .name=UTEST_RESOURCE_VERSION_1},
...
#define  TEST_WRITE_DATA   HLS_MST_PLIST_CONTENT_HEADER"#EXT-X-STREAM-INF:BANDWIDTH=123456,RESOLUTION=160x120\n"HLS_PLAYLIST_FILENAME"\n\n"
...
#define  TEST_WRITE_DATA   HLS_MST_PLIST_CONTENT_HEADER"#EXT-X-STREAM-INF:BANDWIDTH=765432,RESOLUTION=189x320\n"HLS_PLAYLIST_FILENAME"\n\n"
        _wr_buf = TEST_WRITE_DATA,  _wr_buf_sz = sizeof(TEST_WRITE_DATA);
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
...
#define  _COMMON_CODE(_expect_doc_id_str, _expect_detail_str, _expect_final) \
    ...
    if(err_cnt == 0) { \
        ...
        const char  *expect_data_beginwith = (_num_plist_merged == 0)? HLS_MST_PLIST_CONTENT_HEADER: "\n#EXT-X-STREAM-INF:"; \
        expect(_utest_hls_build_mst_plist_continue__done_cb,  when(out_chunkbytes_sz, is_greater_than(0)), \
    ...
    }
    {
        ...
        const char *expect_detail = MOCK__QUERYPARAM_LABEL__DETAIL "=" UTEST_RESOURCE_VERSION_1 "/" HLS_PLAYLIST_FILENAME;
        _COMMON_CODE(expect_doc_id, expect_detail, 0)
    } {
        const char *expect_doc_id = MOCK__QUERYPARAM_LABEL__RES_ID "=" MOCK_ENCRYPTED_DOC_ID ;
        const char *expect_detail = MOCK__QUERYPARAM_LABEL__DETAIL "=" UTEST_RESOURCE_VERSION_2 "/" HLS_PLAYLIST_FILENAME;
...
#define  UTEST_RESOURCE_PATH_VERSION_2   ATFP__COMMITTED_FOLDER_NAME "/" UTEST_RESOURCE_VERSION_2
#define  UTEST_RESOURCE_PATH_VERSION_3   ATFP__COMMITTED_FOLDER_NAME "/" UTEST_RESOURCE_VERSION_3
Ensure(atfp_hls_test__build_mst_plist__continue_ok_2)
{ // parse valid data from second playlist read
    asa_dirent_t  mock_versions[UTEST_NUM_VERSIONS] = {{.type=ASA_DIRENT_DIR, .name=UTEST_RESOURCE_VERSION_1},
        {.type=ASA_DIRENT_DIR, .name=UTEST_RESOURCE_VERSION_2},  {.type=ASA_DIRENT_DIR, .name=UTEST_RESOURCE_VERSION_3}
    };
    HLS__BUILD_MST_PLIST_CONTINUE__SETUP(&mock_versions[0], UTEST_NUM_VERSIONS);
    {
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
        ...
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
                UTEST_RESOURCE_PATH_VERSION_3, UTEST_OPS_MKDIR );
        const char *_wr_buf = NULL; size_t _wr_buf_sz = 0;
#define  TEST_WRITE_DATA   HLS_MST_PLIST_CONTENT_HEADER"#EXT-X-STREAM-INF:BANDWIDTH=893807,RESOLUTION=760x390\n"HLS_PLAYLIST_FILENAME"\n\n"
        _wr_buf = TEST_WRITE_DATA,  _wr_buf_sz = sizeof(TEST_WRITE_DATA);
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
...
#undef   TEST_WRITE_DATA
        ...
        const char *expect_doc_id = MOCK__QUERYPARAM_LABEL__RES_ID "=" MOCK_ENCRYPTED_DOC_ID ;
        const char *expect_detail = MOCK__QUERYPARAM_LABEL__DETAIL "=" UTEST_RESOURCE_VERSION_3 "/" HLS_PLAYLIST_FILENAME;
        _COMMON_CODE(expect_doc_id, expect_detail, 1)
    } { // end of scandir reached, no playlist will be loaded
        expect(_utest_hls_build_mst_plist_continue__done_cb,  when(out_chunkbytes_sz, is_equal_to(0)),
...
Ensure(atfp_hls_test__build_mst_plist__missing_file) {
    ...
    { // missing playlist is possible if the video was transcoded in other format
        ...
        assert_that(err_cnt, is_equal_to(0));
        if(err_cnt == 0) {
            expect(_utest_hls_build_mst_plist_continue__done_cb, when(out_chunkbytes, is_null), 
...
Ensure(atfp_hls_test__build_mst_plist__invalid_content) {
    ...
    UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
            UTEST_RESOURCE_PATH_VERSION_1, UTEST_OPS_MKDIR );
    {
        const char *_wr_buf = "unrelated content"; size_t _wr_buf_sz =  sizeof("unrelated content");
        UTEST_RUN_OPERATION_WITH_PATH(UTEST_ASASRC_BASEPATH, MOCK_USER_ID, MOCK_UPLD_REQ_1_ID,
                UTEST_RESOURCE_PATH_VERSION_1 "/" HLS_MASTER_PLAYLIST_FILENAME, UTEST_OPS_WRITE2FILE);
        mock_fp.internal.op.build_master_playlist (&mock_fp);
        size_t err_cnt = json_object_size(mock_err_info);
