...
#define  UTEST_KEYMAP_RAWDATA  "{"  \
     "\""KEY_ID_1"\":{\"key\":{\"nbytes\":4,\"data\":\"3C0EA878\"},\"alg\":\""ALG_1"\",\"timestamp\":1666684717}," \
     "\""KEY_ID_2"\":{\"key\":{\"nbytes\":4,\"data\":\"534B56F6\"},\"alg\":\""ALG_2"\",\"timestamp\":1666685421}," \
     "\""KEY_ID_3"\":{\"key\":{\"nbytes\":4,\"data\":\"5E30A5CF\"},\"alg\":\""ALG_3"\",\"timestamp\":1666685591}" \
"}"
...
Ensure(atfp_hls_test__crypto__encrypt_doc_id_ok) {
    ...
    size_t  out_sz = 0;
    {
        ...
        expect(EVP_CIPHER_CTX_set_key_length, will_return(1), when(ctx, is_equal_to(&mock_ctx)),
                when(keylen, is_equal_to(UTEST_KEY_OCTET_SZ)));
        expect(OPENSSL_hexstr2buf, will_return(expect_key_octet),  when(str, is_equal_to_string(UTEST_KEY_HEX)),
                will_set_contents_of_parameter(len, &expect_key_len, sizeof(long))  );
        expect(OPENSSL_hexstr2buf, will_return(expect_iv_octet),   when(str, is_equal_to_string(UTEST_IV_HEX)),
                will_set_contents_of_parameter(len, &expect_iv_len, sizeof(long))  );
        expect(EVP_EncryptInit_ex,  will_return(1), when(ctx, is_equal_to(&mock_ctx)),
        ...
        size_t  expect_ct_sz[2] = {strlen(EXPECT_CIPHERTEXT_PART1) * sizeof(char),
            strlen(EXPECT_CIPHERTEXT_PART2) * sizeof(char)};
        expect(EVP_EncryptUpdate,  will_return(1), when(ctx, is_equal_to(&mock_ctx)),
        ...
        expect(EVP_EncryptFinal_ex,  will_return(1), when(ctx, is_equal_to(&mock_ctx)),
    ...
    int success = atfp_encrypt_document_id ((EVP_CIPHER_CTX *)&mock_ctx, &mock_fp_data, key_item, &out, &out_sz);
    ...
    if(out && out_sz > 0) {
        ...
        unsigned char *actual_doc_id_octet = base64_decode((const unsigned char *)out,
                out_sz, &actual_doc_id_octet_sz );
...
Ensure(atfp_hls_test__crypto__encrypt_doc_id_error) {
    ...
    size_t  out_sz = 0;
    {
        ...
        expect(EVP_EncryptInit_ex,  will_return(1), when(ctx, is_equal_to(&mock_ctx)),
                when(key, is_equal_to_string(expect_key_octet)),  when(iv, is_equal_to_string(expect_iv_octet))    );
        expect(EVP_CIPHER_CTX_block_size,  will_return(UTEST_ENCRYPT_BLOCK_SZ));
        expect(EVP_EncryptUpdate,  will_return(0), when(ctx, is_equal_to(&mock_ctx)),
                when(in, is_equal_to_string(EXPECT_PLAINTEXT))       ); // assume error happened
        expect(CRYPTO_free, when(addr, is_equal_to(expect_iv_octet)));
        expect(CRYPTO_free, when(addr, is_equal_to(expect_key_octet)));
    }
    int success = atfp_encrypt_document_id ((EVP_CIPHER_CTX *)&mock_ctx, &mock_fp_data, key_item, &out, &out_sz);
