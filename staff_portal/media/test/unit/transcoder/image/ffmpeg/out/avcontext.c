...
Ensure(atfp_img_ffo_test__avinit_ok)
{
    UTEST_FFM_INIT_SETUP
    {
        ...
        expect(avio_open, will_return(0), when(filename, is_equal_to_string(mock_outfile_path)),
...
#define  UTEST_FFM_ENCODE_PROCESS_SETUP \
    ...
    atfp_av_ctx_t  mock_avctx = {.stream_ctx={.encode=&mock_st_encode_ctx}, \
        .fmt_ctx=&mock_av_ofmt_ctx }; \

Ensure(atfp_img_ffo_test__av_encode_ok)
{
    UTEST_FFM_ENCODE_PROCESS_SETUP
    expect(avcodec_send_frame, will_return(0), when(codec_ctx, is_equal_to(&mock_encoder_ctx)),
           when(frame, is_equal_to(&mock_avctx.intermediate_data.encode.frame))  );
    expect(avcodec_receive_packet, will_return(0), when(codec_ctx, is_equal_to(&mock_encoder_ctx)),
...
Ensure(atfp_img_ffo_test__av_encode_no_more_pkt)
{
    ...
    expect(avcodec_receive_packet, will_return(AVERROR_EOF),
            when(codec_ctx, is_equal_to(&mock_encoder_ctx)) );
    int ret =  atfp__image_dst__encode_frame(&mock_avctx);
...
Ensure(atfp_img_ffo_test__av_write_pkt_ok)
{
    ...
    expect(av_write_frame, will_return(0),  when(fmt_ctx, is_equal_to(&mock_av_ofmt_ctx)),
...
{
    AVFormatContext  mock_av_ofmt_ctx = {0};
    atfp_av_ctx_t  mock_avctx = {.fmt_ctx=&mock_av_ofmt_ctx, .intermediate_data={
        .encode={._final={.file_trailer_wrote=0, .file_header_wrote=1}}}};
