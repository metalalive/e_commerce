...
static void utest_img__av_deinit (atfp_av_ctx_t *_avctx)
{ mock(_avctx); }
...
#define  UTEST_ATFP_IMG__INIT_SETUP \
    ...
    asa_cfg_t  mock_storage_local_cfg = {.ops={.fn_open=app_storage_localfs_open, \
        .fn_close=app_storage_localfs_close,  .fn_unlink=app_storage_localfs_unlink }}; \
    asa_cfg_t  mock_storage_remote_cfg = {.ops={.fn_close=utest_storage_remote_close,}}; \
    asa_op_base_cfg_t  mock_asa_src = {.cb_args={.size=NUM_CB_ARGS_ASAOBJ, .entries=asasrc_cb_args}, \
        .storage=&mock_storage_remote_cfg, .deinit=utest_img__asasrc_final_dealloc }; \
    asa_op_localfs_cfg_t  mock_asa_local = { .loop=loop, .file={.file=-1}, \
        .super={ .storage=&mock_storage_local_cfg, .deinit=utest_img__asalocal_final_dealloc, \
    ...
    mock_fp->super.data = (atfp_data_t){.error=mock_errinfo, .rpc_receipt=&mock_rpc_receipt, \
        .callback=utest_atfp_usr_cb, .storage={.handle=&mock_asa_src}}; \
...
Ensure(atfp_img_ffi_test__init_ok)
{
    ...
    { // init
        ...
        expect(utest_storage_remote_close, will_return(ASTORAGE_RESULT_UNKNOWN_ERROR),
