...
#define  ITEST_REQ_ITEM_GEN(_usr_id, _tr, _rn, _ed) "{\"usr_id\":" #_usr_id ",\"access_control\":{" \
    "\"transcode\":" #_tr ",\"renew\":" #_rn ",\"edit_acl\":" #_ed "}}"

#define  REQ_ITEM_1   ITEST_REQ_ITEM_GEN(13, true,  false, true)
#define  REQ_ITEM_2   ITEST_REQ_ITEM_GEN(24, false, false, false)
#define  REQ_ITEM_3   ITEST_REQ_ITEM_GEN(38, false, false, true)
#define  REQ_ITEM_4   ITEST_REQ_ITEM_GEN(59, true,  true,  false)
#define  REQ_ITEM_5   ITEST_REQ_ITEM_GEN(16, true,  true, true)
#define  REQ_ITEM_6   ITEST_REQ_ITEM_GEN(13, false, true, false)
#define  REQ_ITEM_7   ITEST_REQ_ITEM_GEN(38, true, false, false)
#define  REQ_ITEM_8   ITEST_REQ_ITEM_GEN(51, true, false, true)
#define  REQ_ITEM_9   ITEST_REQ_ITEM_GEN(66, false, false, false)
#define  REQ_ITEM_10  ITEST_REQ_ITEM_GEN(29, true, true, false)
#define  REQ_ITEM_11  ITEST_REQ_ITEM_GEN(71, false, true, true)
#define  REQ_ITEM_12  ITEST_REQ_ITEM_GEN(34, true, false, false)

#define  URL_PATTERN  "https://localhost:8010/file/acl?res_id=%s"
...
static void _available_resource_lookup(json_t **upld_req)
{
    ...
    *upld_req = NULL;
    json_array_foreach(_app_itest_active_upload_requests, idx, req) {
        existing_acl = json_object_get(req, "acl");
        if(existing_acl)
            continue;
...
static void test_verify__edit_acl_common(CURL *handle, test_setup_priv_t *privdata, void *_usr_arg)
{
    ...
    if(usr_args->expect_resp_code == 200) {
        if(usr_args->req_body_serialtxt) {
            ...
            if(new_acl)
                json_object_set_new(usr_args->upld_req, "acl", new_acl);
        } else {
...
} // end of  test_verify__edit_acl_common


#define  ITEST_ACL_COMMON_CODE_SETUP  \
    ...
    size_t url_sz = strlen(_res_id_urlencoded) + sizeof(URL_PATTERN); \
    char url[url_sz]; \
    size_t nwrite = sprintf(&url[0], URL_PATTERN, _res_id_urlencoded); \
...
static void _itest_api_edit_acl__common (json_t *upld_req, uint32_t auth_usr_id, const char *req_body_serialtxt,
        int  expect_resp_code, const char *expect_err_field)
{
    ITEST_ACL_COMMON_CODE_SETUP
    ...
    itest_usrarg_t  usr_args = {.upld_req=upld_req, .expect_resp_code=expect_resp_code,
           .expect_err_field=expect_err_field, .req_body_serialtxt=req_body_serialtxt };
    run_client_request(&setup_data, test_verify__edit_acl_common, &usr_args);
    ITEST_ACL_COMMON_CODE_TEARDOWN
} // end of  _itest_api_edit_acl__common


static void _itest_api_read_acl__common (json_t *upld_req, uint32_t auth_usr_id, int expect_resp_code)
{
    ITEST_ACL_COMMON_CODE_SETUP
    ...
    run_client_request(&setup_data, test_verify__edit_acl_common, &usr_args);
    ITEST_ACL_COMMON_CODE_TEARDOWN
} // end of  _itest_api_read_acl__common


Ensure(api_edit_acl__test_ok)
{
    json_t *upld_req = NULL;
    _available_resource_lookup(&upld_req);
    if(!upld_req)
        return;
    { // subcase 1
...
#undef  REQ_BODY_SERIALTXT
        ...
        _itest_api_edit_acl__common (upld_req, res_owner_id, req_body_serialtxt, 200, NULL);
        _itest_api_read_acl__common (upld_req, res_owner_id, 200);
...
#undef  REQ_BODY_SERIALTXT
        ...
        _itest_api_edit_acl__common (upld_req, 13, req_body_serialtxt, 200, NULL);
        _itest_api_read_acl__common (upld_req, 13, 200);
...
#undef  REQ_BODY_SERIALTXT
        itest_rpc_usermgt__setup_usr_ids(&other_usr_ids[0], 4, 0);
        _itest_api_edit_acl__common (upld_req, 51, req_body_serialtxt, 200, NULL);
        _itest_api_read_acl__common (upld_req, 51, 200);
...
} // end of  api_edit_acl__test_ok


Ensure(api_edit_acl__test_invalid_usr_in_reqbody)
{
    json_t *upld_req = NULL;
    _available_resource_lookup(&upld_req);
    if(!upld_req)
...
#undef  REQ_BODY_SERIALTXT
    itest_rpc_usermgt__setup_usr_ids(&other_usr_ids[0], 4, 0);
    _itest_api_edit_acl__common (upld_req, res_owner_id, req_body_serialtxt, 400, NULL);
}

Ensure(api_edit_acl__test_permission_denied)
...
#undef  REQ_BODY_SERIALTXT
    itest_rpc_usermgt__setup_usr_ids(&other_usr_ids[0], 3, 0);
    _itest_api_edit_acl__common (upld_req, 13, req_body_serialtxt, 403, NULL);
}

Ensure(api_edit_acl__test_rpc_no_response)
{
    json_t *upld_req = NULL;
    _available_resource_lookup(&upld_req);
    if(!upld_req)
...
#undef  REQ_BODY_SERIALTXT
    ...
    _itest_api_edit_acl__common (upld_req, res_owner_id, req_body_serialtxt, 503, NULL);
}

Ensure(api_edit_acl__test_ok2)
{
    json_t *upld_req = NULL;
    while(1) {
        _available_resource_lookup(&upld_req);
...
} // end of  api_edit_acl__test_ok2


TestSuite *api_file_acl_tests(void)
{
    TestSuite *suite = create_test_suite();
    add_test(suite, api_edit_acl__test_invalid_usr_in_reqbody);
    add_test(suite, api_edit_acl__test_permission_denied);
    add_test(suite, api_edit_acl__test_rpc_no_response);
    add_test(suite, api_edit_acl__test_ok);
    add_test(suite, api_edit_acl__test_ok2);
    return suite;
