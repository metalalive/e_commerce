...
class IngredientItems extends CommonPresentableProductItemForm {
    constructor(props) {
        let _valid_fields_name = ['category',];
    ...
    _single_item_category_field_render(val, idx) {
        return (
    ...
    _single_item_menu_render(val, idx) {
        ...
        let category_field = this._single_item_category_field_render(val, idx);
        let attributes_field = this._single_item_attributes_field_render(val, idx);
        return (<> {name_field} {category_field} {attributes_field} </>);
