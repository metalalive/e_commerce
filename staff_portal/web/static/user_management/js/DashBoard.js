...
const common_req_opt = {method:"GET", mode:"cors", credentials:"include", headers:{'accept':'application/json'},};
...
var uri_dict = {
    // web URLs
    web_login:  _web_base_url+'/login',
    web_authrole_add    : _web_base_url+'/usermgt/roles/add',
    web_authrole_update : _web_base_url+'/usermgt/roles/update',
    web_quotatype_add   : _web_base_url+'/usermgt/quota/add',
    web_quotatype_update: _web_base_url+'/usermgt/quota/update',
    web_usrgrp_add    : _web_base_url+'/usermgt/usergrps/add',   
    web_usrgrp_update : _web_base_url+'/usermgt/usergrps/update',
    web_usrprof_add   : _web_base_url+'/usermgt/usrprofs/add',   
    web_usrprof_update: _web_base_url+'/usermgt/usrprofs/update',
    // API endpoints
    api_logout: _api_base_url+'/logout',
    api_authrole:    [_api_base_url+'/usermgt/roles',    _api_base_url+'/usermgt/role'],
    api_usrprof: [_api_base_url+'/usermgt/usrprofs', _api_base_url+'/usermgt/usrprof'],
    api_usrgrp:  [_api_base_url+'/usermgt/usrgrps',  _api_base_url+'/usermgt/usrgrp'],
    api_role_applied : _api_base_url+'/usermgt/role_applied',
    api_usrgrp_applied: _api_base_url+'/usermgt/grps_applied',
    api_quotatype     : _api_base_url+'/usermgt/quota',
    api_username_edit : _api_base_url+'/usermgt/username/edit',
    api_password_edit :   _api_base_url+'/usermgt/password/edit',
    api_account_activate   : _api_base_url+'/usermgt/account/activate',
    api_account_deactivate : _api_base_url+'/usermgt/account/deactivate',
};
    ...
    if(command == 'edit') {
        ...
        if(ids.length > 0) {
            ...
            if(target.dataset.mode) {
                props.req_opt.mode = target.dataset.mode;
            }
            if(target.dataset.credentials) {
                props.req_opt.credentials = target.dataset.credentials;
            }
...
function perform_logout(evt) {
    ...
    props.url = base_url;
    props.req_opt = {method:"POST", body:"", headers:headers, mode:'cors', credentials:'include'};
    props.caller = target;
    props.callbacks = [prompt_api_resp_msg];
    perform_api_call(props);
}
...
function activate_login_account(kwargs) {
    ...
    var url = uri_dict['api_account_activate'];
    var caller = {dataset: {base_url:url},};
    var props  = {url: url+"?fields=profile", caller:caller, callbacks: [prompt_api_resp_msg]};
    props.req_opt = {method:"POST", body: JSON.stringify(users), mode:'cors', credentials:'include',
        headers:{'content-type':'application/json', 'accept':'application/json'},};
    perform_api_call(props);
}
...
function deactivate_login_account(kwargs) {
    ...
    var url = uri_dict['api_account_deactivate'];
    var caller = {dataset: {base_url:url},};
...
function toggle_auth_activation(evt) {
    ...
    if(target.checked) { // activate users and their login account
        ...
        kwargs.api = {req_opt: common_req_opt, url:uri_dict['api_usrprof'][0], load_api_fn:perform_api_call,
...
{ // must run as bound function
    for(var idx = 0; idx < this.apis.length; idx++) {
        ...
        var qualified_uri = api.url_template +'/'+ this.id;
...
function prompt_api_resp_msg(avail_data, props) {
    ...
    switch (base_url)
    {
        ...
        case "/usermgt/users/undel":
            ...
            break;
        case uri_dict['api_account_activate']:
        ...
        if(logout_flag) {
            setTimeout(
                function(){
                    ...
                    window.location.href = uri_dict['web_login'] +'?redirect='+ current_url;
                }, 3000
            );
        }
    } else {
...
function _get_edit_btns(container) {
    ...
    var btns = Array.from(container.querySelectorAll(query_string));
    return btns;
}
...
function init_toolbar_list_usrgrp(new_dom, api) {
    ...
    var btn_data = {};
    btn_data['add_items']  = {title:'add new groups', href:uri_dict['web_usrgrp_add'], };
    btn_data['edit_items'] = {title:'edit chosen groups',   onclick: run_action_bulk, api: api,
            dataset:{cls_name:'ugid', command:'edit', base_url:uri_dict['web_usrgrp_update'],},};
    btn_data['del_items']  = {title:'delete chosen groups', onclick: run_action_bulk, api: api,
            dataset:{cls_name:'ugid', command:'del',  base_url:uri_dict['api_usrgrp'][0],
            mode:'cors', credentials:'include'}, };
    btn_data['undel_item'] = {title:'undo last deleted groups', onclick: run_recovery, api:api, hidden:false,
            dataset:{command:'recovery', time_start:'', base_url:uri_dict['api_usrgrp'][0],
            mode:'cors', credentials:'include'},}
...
function init_toolbar_list_usrprof(new_dom, api)
{
    ...
    btn_data['add_items']  = {title:'add users', href: uri_dict['web_usrprof_add'], };
    btn_data['edit_items'] = { title:'edit chosen users',  onclick: run_action_bulk, api: api,
        dataset:{ cls_name:'upid', command:'edit', base_url: uri_dict['web_usrprof_update'], },
    };
    btn_data['del_items']  = {title:'delete chosen users', onclick: run_action_bulk,  api: api,
            dataset:{cls_name:'upid', command:'del',  base_url: uri_dict['api_usrprof'][0],
            mode:'cors', credentials:'include'}, };
    btn_data['undel_item'] = {title:'undo last deleted profiles', onclick: run_recovery, api:api, hidden:false,
            dataset:{command:'recovery', time_start:'', base_url: uri_dict['api_usrprof'][0],
            mode:'cors', credentials:'include'},}
    toolkit.update_doms_props(doms, btn_data);
    init_search_bar(new_dom, api, {textfield:'search_keywords', button:'search_submit'});
}
...
function init_toolbar_list_authrole(new_dom, api)
{
    var doms = _get_edit_btns(new_dom);
    var btn_data = {};
    btn_data['add_items']  = {title:'add roles', href: uri_dict['web_authrole_add'], };
    btn_data['edit_items'] = {title:'edit chosen roles',   onclick: run_action_bulk, api: api,
         dataset:{ cls_name:'rid', command:'edit', base_url: uri_dict['web_authrole_update'],}, };
    btn_data['del_items']  = {title:'delete chosen roles', onclick: run_action_bulk, api:api,
            dataset:{cls_name:'rid', command:'del',  base_url: uri_dict['api_authrole'][0],
            mode:'cors', credentials:'include'}, };
    toolkit.update_doms_props(doms, btn_data);
    init_search_bar(new_dom, api, {textfield:'search_keywords', button:'search_submit'});
}
...
function init_toolbar_list_quota(new_dom, api)
{
    var doms = _get_edit_btns(new_dom);
    var btn_data = {};
    btn_data['add_items']  = {title:'add quota usage type', href: uri_dict['web_quotatype_add'], };
    btn_data['edit_items'] = {title:'edit chosen quota usage type',   onclick: run_action_bulk, api:api,
            dataset:{cls_name:'qid', command:'edit', base_url: uri_dict['web_quotatype_update'],},};
    btn_data['del_items']  = {title:'delete chosen quota usage type', onclick: run_action_bulk, api:api,
            dataset:{cls_name:'qid', command:'del',  base_url: uri_dict['api_quotatype'],
            mode:'cors', credentials:'include'}, };
    toolkit.update_doms_props(doms, btn_data);
    init_search_bar(new_dom, api, {textfield:'search_keywords', button:'search_submit'});
}
...
{
    var detail_api = {
        req_opt: common_req_opt, url:'xxx/yyy', url_template: uri_dict['api_usrgrp'][1],
    ...
    var grp_to_profs_api = {
        req_opt: common_req_opt, url:'xxx/yyy', url_template: uri_dict['api_usrgrp_applied'],
...
function _init_listusrgrp_tab(tab, detail_render_container)
{
    ...
    usr_grp.api  = {
        req_opt: common_req_opt, url: uri_dict['api_usrgrp'][0],
...
function _init_listusrprof_tab(tab, detail_render_container)
{
    ...
    var detail_api = {
        req_opt: common_req_opt, url:'xxx/yyy', url_template: uri_dict['api_usrprof'][1],
    ...
    usr_prof.api = {
        req_opt: common_req_opt, url:uri_dict['api_usrprof'][0],
...
function init_listrole_tab(tab, templates)
{
    ...
    var detail_role_api = {
        req_opt: common_req_opt, url:'xxx/yyy', url_template: uri_dict['api_authrole'][1],
    ...
    var role_to_grps_api = {
        req_opt: common_req_opt, url:'xxx/yyy', url_template: uri_dict['api_role_applied'],
    ...
    auth_role.api = {
        req_opt: common_req_opt, url: uri_dict['api_authrole'][0],
...
function init_listquota_tab(tab, templates)
{
    ...
    content_wrapper.api = {
        req_opt: common_req_opt, url: uri_dict['api_quotatype'], load_api_fn:perform_api_call,
...
function on_page_load()
{
    ...
    global_tabs = {
        ...
        settings: {
            ...
            init_data: {
                edit_uname: {api_mthd:'PATCH', submit_url: uri_dict['api_username_edit'],},
                edit_passwd:{api_mthd:'PATCH', submit_url: uri_dict['api_password_edit'],},
            },
    ...
    btn_logout.dataset.base_url = uri_dict['api_logout'];
} // end of on_page_load
