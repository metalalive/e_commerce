...
class ExtendedTemplateResponseMixin(TemplateResponseMixin):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if chosen_template_idx >= template_names_len:
            raise IndexError('chosen template index : %s, total length of template name list: %s' \
...
class ExtendedLoginRequiredMixin(LoginRequiredMixin):
    def handle_no_permission(self):
        ...
        if auth_hosts and auth_hosts[0]:
            toward_url = '%s://%s%s' % (self.request.scheme, self.request._get_raw_host(), \
                    self.request.get_full_path())
            login_url  = '%s://%s:%s%s' % (self.request.scheme, auth_hosts[0][0], \
                    auth_hosts[0][1], self.get_login_url())
...
class BaseLoginHTMLView(BaseHTMLView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        redirect = request.GET.get(BaseAuthHTMLView.redirect_field_name , self.default_url_redirect)
        kwargs['formparams'] = {'non_field_errors':NON_FIELD_ERRORS_KEY, 'submit_url': self.submit_url,
                'redirect' : redirect, }
