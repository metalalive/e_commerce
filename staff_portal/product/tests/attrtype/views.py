...
from ..common import _fixtures, _MockTestClientInfoMixin, assert_view_permission_denied, listitem_rand_assigner, assert_view_bulk_create_with_response, SoftDeleteCommonTestMixin, app_code_product, priv_status_staff
...
class AttrTypeBaseViewTestCase(AttrTypeBaseViewTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.request_data = [random.choice(_fixtures['ProductAttributeType']) for _ in range(5)]

    def test_permission_denied(self):
        kwargs = { 'testcase':self, 'request_body_data':self.request_data, 'path':self.path,
            'permissions': ['view_productattributetype', 'add_productattributetype'], 'http_method':'post',
        }
        assert_view_permission_denied(**kwargs)
        kwargs['http_method'] = 'put'
        kwargs['permissions'] = ['view_productattributetype', 'change_productattributetype']
        assert_view_permission_denied(**kwargs)

    def test_bulk_ok_with_full_response(self):
        expect_shown_fields  = ['id', 'name', 'dtype']
        expect_usrprof = 321
        created_attrtypes = assert_view_bulk_create_with_response(testcase=self, path=self.path, method='post',
