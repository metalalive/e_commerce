...
from product.models.base import ProductAttributeType, ProductSaleableItem, ProductSaleablePackage
...
class AttrTypeDeletionTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        models_info = [ (ProductAttributeType, num_attrtypes), (ProductDevIngredient, num_ingredients),
                (ProductSaleableItem, num_saleitems), (ProductSaleablePackage, num_salepkgs) ]
        primitives = {}
        ...
        for attrtype in primitives['ProductAttributeType']:
            ...
            for cls in (ProductDevIngredient, ProductSaleableItem, ProductSaleablePackage):
                content_type = ContentType.objects.get_for_model(cls)
                objs = primitives[cls.__name__]
                for obj in objs:
                    attr_value = random.choice(_fixtures[manager.model.__name__])
                    _data = {'ingredient_type':content_type, 'ingredient_id':obj.id,
                            'value': attr_value}
                    manager.create(**_data)
        self._primitives = primitives
        expect_deleted_attrtypes = map(lambda _: random.choice(primitives['ProductAttributeType']), range(3))
        chosen_fields = ['id','ingredient_id','ingredient_type','value']
        self._expect_deleted_attrvals = dict(map(lambda obj: ((obj.id, obj.attr_val_set), \
                list(obj.attr_val_set.order_by('id').values(*chosen_fields))),  expect_deleted_attrtypes))
    ...
    def test_soft_delete(self):
        ...
        qset = ProductAttributeType.objects.filter(id__in=delete_ids)
        self.assertFalse(qset.exists())
        qset = ProductAttributeType.objects.get_deleted_set().filter(id__in=delete_ids).values_list('id', flat=True)
