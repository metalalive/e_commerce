...
from sqlalchemy import Column, Boolean, SmallInteger, Enum as sqlalchemy_enum, Float, String, Text, DateTime, Time, ForeignKey
...
class AppIdGapNumberFinder(IdGapNumberFinder):
    ...
    def is_db_err_recoverable(self, error) -> bool:
        mysql_pk_dup_error = lambda x : 'Duplicate entry' in x.args[0] and 'PRIMARY' in x.args[0]
...
class StoreProfile(Base):
    ...
    @classmethod
    def quota_stats(cls, objs, session, limit, attname):
        ...
        target_ids = limit.keys()
        query = cls.get_existing_items_stats(session, target_ids, attname)
        result = {}
        for target_id, maxnum in limit.items():
            max_items_limit = maxnum
            ...
            #assert max_items_limit >= num_items_used
            result[target_id] = {'max_items_limit':max_items_limit, 'num_new_items':num_new_items, 'num_existing_items':num_existing_items}
        return result

    @classmethod
    def bulk_save(cls, objs, session):
        ...
        def save_instance_fn():
            try:
                ...
                session.rollback()
                raise
        return cls._id_gap_finder.save_with_rand_id(save_instance_fn, objs=objs, session=session)
...
class StoreStaff(Base, TimePeriodValidMixin):
    ...
    store_id  = Column(MYSQL_INTEGER(unsigned=True), ForeignKey('store_profile.id'), primary_key=True)
    # come from GenericUserProfile in user_management app
