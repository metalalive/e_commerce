...
from pydantic import BaseModel as PydanticBaseModel, EmailStr, validator
...
class NewStoreProfilesReqBody(PydanticBaseModel):
    ...
    def _get_quota_arrangement(self, supervisor_data, profile_ids):
        ...
        quota_arrangement = {prof_id:{} for prof_id in profile_ids}
        for prof_id in profile_ids:
            ...
            if err_detail:
                err_content[prof_id] = err_detail
        if any(err_content):
            err_content = {'msg':'invalid supervisor ID found in the request',
                    'supervisor_id':err_content }
            raise FastApiHTTPException(
                    status_code=FastApiHTTPstatus.HTTP_400_BAD_REQUEST,
                    detail=err_content,  headers={}
                )
    ...
    def _storeemail_quota_check(self, quota_arrangement):
        ...
        for item in self.__root__:
            ...
            if max_limit < num_new_items:
                ...
                err_content[prof_id] = [err_msg]
        if any(err_content):
            raise  StoreProfileValidationError(content=err_content, field_name='emails')
    ...
    def _storephone_quota_check(self, quota_arrangement):
        ...
        for item in self.__root__:
            ...
            if max_limit < num_new_items:
                ...
                err_content[prof_id] = [err_msg]
        if any(err_content):
            raise  StoreProfileValidationError(content=err_content, field_name='phones')
    ...
    def _storeprofile_quota_check(self, db_engine, profile_ids, quota_arrangement):
        ...
        for prof_id in profile_ids:
            item = quota_chk_result[prof_id]
            num_existing_items = item['num_existing_items']
            ...
            if max_limit < curr_used:
                ...
                err_content[prof_id] = [err_msg]
        if any(err_content):
            raise  StoreProfileValidationError(content=err_content, field_name='supervisor_id')
        return new_stores
