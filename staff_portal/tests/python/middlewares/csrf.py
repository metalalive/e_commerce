...
from common.csrf.middleware import ExtendedCsrfViewMiddleware
...
class CsrfMiddlewareTestCase(SimpleTestCase):
    def setUp(self):
        ...
        self.base_mock_request = Mock(
                META=raw_headers, COOKIES={}, csrf_cookie_set=False, POST={},
                csrf_cookie_age=123, csrf_processing_done=False,
                _dont_enforce_csrf_checks=False, headers = {'accept':'application/json'}
            )
    ...
    def test_set_token_unauth_user(self):
        ...
        self._check_response_cookie(mock_request, response=response_after,
    ...
    def _check_response_cookie(self, mock_request, response, expect_max_age,
            expect_httponly_status):
    ...
    def test_verify_token_not_found(self):
        ...
        response = self.middleware.process_view(request=mock_request, callback=mock_callback,
