...
def init_test_database(dbs_engine, db_engine, metadata_objs, createdb_sql, keepdb):
    with dbs_engine.connect() as conn:
        conn.execute(createdb_sql)
    if not keepdb:
        tuple(map(lambda metadata: metadata.drop_all(db_engine), metadata_objs))
    # by default it skips to CREATE existing tables
    tuple(map(lambda metadata: metadata.create_all(db_engine), metadata_objs))


def deinit_test_database(dbs_engine, db_engine, metadata_objs, dropdb_sql, keepdb):
    if keepdb:
        return
    tuple(map(lambda metadata: metadata.drop_all(db_engine), metadata_objs))
    with dbs_engine.connect() as conn:
