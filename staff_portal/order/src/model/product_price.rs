...
impl ProductPriceModelSet {
    pub fn update(mut self, updating:Vec<ProductPriceEditDto>,
                  creating:Vec<ProductPriceEditDto>) -> DefaultResult<Self, AppError>
    {
        let num_updated = updating.iter().filter_map(|d| {
            let result = self.items.iter_mut().find(|obj| {
                    obj.product_type == d.product_type && obj.product_id == d.product_id
                    && !obj.is_create
            });
        ...
        if num_updated != updating.len() {
            return Err(AppError { code: AppErrorCode::InvalidInput,
