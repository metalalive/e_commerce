...
class RemoteAccessTokenAPIView(AuthCommonAPIView, GetProfileIDMixin):
    """
    API endpoint for authenticated users requesting a new access token
    if their refresh token (for this authentication server) hasn't expired yet.
    The token generated by this endpoint is used only in specific service
    but can be in different network domain
    """
