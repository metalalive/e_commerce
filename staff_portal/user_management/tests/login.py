...
class LoginTestCase(TransactionTestCase, _BaseMockTestClientInfoMixin, KeystoreMixin):
    ...
    def test_auth_failure(self):
        ...
        account_data = [
            # active guest
            {'username':'ImGuest', 'password':'hard2guess', 'is_active':True, 'is_staff':False, 'is_superuser':False},
            # inactive staff
            {'username':'ImStaff', 'password':'D0ntexpose', 'is_active':False, 'is_staff':True, 'is_superuser':False},
            # inactive superuser
            {'username':'ImSuperUsr', 'password':'cautious', 'is_active':False, 'is_staff':True, 'is_superuser':True},
        ]
        ...
        accounts = tuple(map(lambda d: LoginAccount.objects.create_user(**d), account_data))
        bodies = [
            None, {}, {'password': 'pig1234'}, {'username': 'shrimp_noodle'},
            {'username':'thisuser', 'password':'doesnotexist'},
        ]
        bodies.extend(tuple(map(lambda d: {'username':d['username'], 'password':d['password']}, account_data)))
        for body in bodies:
    ...
    def _verify_recv_jwt(self, encoded):
        ...
        with self.assertRaises(MissingRequiredClaimError):
            payld_verified = _jwt.verify(keystore=self._keystore, audience=['sale', 'payment'], raise_if_failed=True)
        ...
        self.assertIsNone(payld_verified.get('aud')) # refresh token doesn't include `aud` field
        return payld_verified
## end of class LoginTestCase


class RefreshAccessTokenTestCase(TransactionTestCase, _BaseMockTestClientInfoMixin, AuthenticateUserMixin, KeystoreMixin):
    ...
    def _setup_role(self, profile, approved_by):
        ...
        self.applied_perm_map = {AppCodeOptions.user_management.value: \
                list(roles[2].permissions.values_list('codename', flat=True)) }
        return role_data
    ...
    def test_ok(self):
        ...
        actual_perm_data = filter(lambda d: d['app_code'] == usrmgt_appcode, payld_verified['perms'])
        ...
        qset = qset.values_list('material__app_code', 'material__mat_code', 'maxnum',)
        expect_quota_data = list(map(lambda v: {'app_code':v[0], 'mat_code':v[1], 'maxnum':v[2]}, qset))
    ...
    def test_app_access_denied(self):
        ...
        result = response.json()
        self.assertEqual(int(response.status_code), 403)
        self.assertEqual(result[non_fd_err_key][0] , 'the user does not have access to these resource services listed in audience field')
    ...
    def _verify_recv_jwt(self, encoded, expect_audience):
        ...
        with self.assertRaises(InvalidAudienceError):
            payld_verified = _jwt.verify(keystore=self._keystore, audience=['delivery', 'payment'], raise_if_failed=True)
        payld_verified = _jwt.verify(keystore=self._keystore, audience=expect_audience, raise_if_failed=True)
        self.assertNotEqual(payld_verified , None)
        self.assertIsNotNone(payld_verified.get('aud')) # access token should include `aud` field
        return payld_verified
## end of class RefreshAccessTokenTestCase



class LogoutTestCase(TransactionTestCase, _BaseMockTestClientInfoMixin, AuthenticateUserMixin, KeystoreMixin):
    path = '/logout'
    def setUp(self):
        self._setup_keystore()
        with patch('django.conf.settings.AUTH_KEYSTORE', self._keystore_init_config) as mocked_obj:
