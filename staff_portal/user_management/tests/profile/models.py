...
class ProfileCommonTestCase(TransactionTestCase):
    def _setup_groups(self):
        ...
        group_closure_data = [
            ...
            {'id':5,  'depth':1, 'ancestor':grp_obj_map[3], 'descendant':grp_obj_map[4]},
            {'id':6,  'depth':1, 'ancestor':grp_obj_map[4], 'descendant':grp_obj_map[5]},
            {'id':7,  'depth':1, 'ancestor':grp_obj_map[5], 'descendant':grp_obj_map[6]},
            {'id':8,  'depth':2, 'ancestor':grp_obj_map[3], 'descendant':grp_obj_map[5]},
            {'id':9,  'depth':2, 'ancestor':grp_obj_map[4], 'descendant':grp_obj_map[6]},
            {'id':10, 'depth':3, 'ancestor':grp_obj_map[3], 'descendant':grp_obj_map[6]},
            # ---------
            {'id':11,  'depth':0, 'ancestor':grp_obj_map[7], 'descendant':grp_obj_map[7]},
            {'id':12,  'depth':0, 'ancestor':grp_obj_map[8], 'descendant':grp_obj_map[8]},
            {'id':13,  'depth':1, 'ancestor':grp_obj_map[3], 'descendant':grp_obj_map[7]},
            {'id':14,  'depth':1, 'ancestor':grp_obj_map[7], 'descendant':grp_obj_map[8]},
            {'id':15,  'depth':2, 'ancestor':grp_obj_map[3], 'descendant':grp_obj_map[8]},
            # ---------
            {'id':16,  'depth':0, 'ancestor':grp_obj_map[9], 'descendant':grp_obj_map[9]},
            {'id':17,  'depth':0, 'ancestor':grp_obj_map[10],'descendant':grp_obj_map[10]},
            {'id':18,  'depth':1, 'ancestor':grp_obj_map[9], 'descendant':grp_obj_map[10]},
        ]
    ...
    def _setup_roles(self):
        ...
        role_data = [
            ...
            {'id':6, 'name':'my third custom role',},
            {'id':7, 'name':'my fourth custom role',},
        ]
    ...
    def setUp(self):
        ...
        grp_rel_data = [
            {'group':self._groups[6], 'profile':self._profile, 'approved_by':self._profile_2nd},
            {'group':self._groups[8], 'profile':self._profile, 'approved_by':self._profile_2nd}
        ]
...
class ProfileCreationTestCase(ProfileCommonTestCase):
    def test_inherit_roles(self):
        ...
        role_rel_data = [
            {'expiry':_expiry_time, 'approved_by':self._profile_2nd, 'role': self._roles[2],
                'user_type':grp_ct, 'user_id': self._groups[3].id} ,
            {'expiry':_expiry_time, 'approved_by':self._profile_2nd, 'role': self._roles[3],
                'user_type':grp_ct, 'user_id': self._groups[5].id}
        ]
    ...
    def _get_default_quota_rel_data(self):
        ...
        prof_ct  = ContentType.objects.get_for_model(GenericUserProfile)
        out = [
            {'user_type':grp_ct, 'user_id':self._groups[3].id, 'material':self._quota_mat[0], 'maxnum':15},
            {'user_type':grp_ct, 'user_id':self._groups[4].id, 'material':self._quota_mat[0], 'maxnum':26},
            {'user_type':grp_ct, 'user_id':self._groups[5].id, 'material':self._quota_mat[0], 'maxnum':37},
            {'user_type':grp_ct, 'user_id':self._groups[7].id, 'material':self._quota_mat[0], 'maxnum':25},
            # ---------------------
            {'user_type':grp_ct, 'user_id':self._groups[3].id, 'material':self._quota_mat[1], 'maxnum':18},
            {'user_type':grp_ct, 'user_id':self._groups[4].id, 'material':self._quota_mat[1], 'maxnum':29},
            {'user_type':grp_ct, 'user_id':self._groups[5].id, 'material':self._quota_mat[1], 'maxnum':23},
            # ---------------------
            {'user_type':grp_ct, 'user_id':self._groups[5].id, 'material':self._quota_mat[2], 'maxnum':6},
            {'user_type':grp_ct, 'user_id':self._groups[7].id, 'material':self._quota_mat[2], 'maxnum':5},
            # ---------------------
            {'user_type':grp_ct, 'user_id':self._groups[3].id, 'material':self._quota_mat[3], 'maxnum':9},
            {'user_type':grp_ct, 'user_id':self._groups[4].id, 'material':self._quota_mat[3], 'maxnum':10},
            {'user_type':grp_ct, 'user_id':self._groups[5].id, 'material':self._quota_mat[3], 'maxnum':12},
            # ---------------------
            {'user_type':grp_ct, 'user_id':self._groups[7].id, 'material':self._quota_mat[4], 'maxnum':2},
            # ---------------------
            {'user_type':grp_ct, 'user_id':self._groups[4].id, 'material':self._quota_mat[5], 'maxnum':89},
            {'user_type':grp_ct, 'user_id':self._groups[5].id, 'material':self._quota_mat[5], 'maxnum':21},
            {'user_type':grp_ct, 'user_id':self._groups[7].id, 'material':self._quota_mat[5], 'maxnum':90},
            # ---------------------
    ...
    def test_activate_deactivate(self):
        verify_items = [
            {'role':self._roles[0], 'expect_is_staff':True,  'expect_is_superuser':True},
            {'role':self._roles[1], 'expect_is_staff':True,  'expect_is_superuser':False},
            {'role':self._roles[2], 'expect_is_staff':False, 'expect_is_superuser':False},
        ]
        ...
        edit_data = {'is_staff':True, 'is_superuser':True, 'is_active':True, 'profile':self._profile_2nd,
        ...
        edit_data = {'is_staff':False, 'is_active':False, 'password_last_updated': django_timezone.now()}
