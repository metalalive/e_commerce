...
static void
app_utest__rpc_cfg_setup(json_t *objs, size_t num_brokers, const char **data_str, uint32_t *data_uint) {
    size_t idx = 0;
    size_t jdx = 0;
    ...
    for (idx = 0; idx < num_brokers; idx++) {
        ...
        { // assume the hierarchy path is only 2 levels, first level is string, the second one is
          // integer
...
DECLARE_RPC_REPLY_RENDER_FN(utset_operation_efg__rpc_corr_id_render) { return APPRPC_RESP_ACCEPTED; }

DECLARE_RPC_REPLY_RENDER_FN(utset_operation_cde__rpc_qname_render) { return APPRPC_RESP_ACCEPTED; }

DECLARE_RPC_REPLY_RENDER_FN(utset_operation_ijk__rpc_qname_render) { return APPRPC_RESP_ACCEPTED; }
...
Ensure(rpc_caller_cfg_incomplete_setting_tests) {
    const char *data_str[] = {
        ...
        "utset_operation_efg__rpc_corr_id_render", // broker[1] bindings[0]
                                                   // reply.correlation_id.render_fn
        ...
        "utset_operation_ijk__rpc_corr_id_render", // broker[1] bindings[2]
                                                   // reply.correlation_id.render_fn , expect to
                                                   // report error from here
...
static void
utest_app_rpc__verify_broker_1(arpc_cfg_t *rpc_cfg, const char **data_str_ptr, uint32_t *data_uint_ptr) {
...
static void
utest_app_rpc__verify_broker_2(arpc_cfg_t *rpc_cfg, const char **data_str_ptr, uint32_t *data_uint_ptr) {
    arpc_cfg_bind_t *bind_cfg = NULL;
    assert_that(rpc_cfg->alias, is_equal_to_string(data_str_ptr[0]));
    assert_that(rpc_cfg->attributes.vhost, is_equal_to_string(data_str_ptr[3]));
    assert_that(rpc_cfg->attributes.max_channels, is_equal_to(data_uint_ptr[1]));
...
static void
utest_app_rpc__verify_broker_3(arpc_cfg_t *rpc_cfg, const char **data_str_ptr, uint32_t *data_uint_ptr) {
    arpc_cfg_bind_t *bind_cfg = NULL;
    assert_that(rpc_cfg->alias, is_equal_to_string(data_str_ptr[0]));
    assert_that(rpc_cfg->attributes.vhost, is_equal_to_string(data_str_ptr[3]));
    assert_that(rpc_cfg->attributes.max_kb_per_frame, is_equal_to(data_uint_ptr[2]));
...
Ensure(rpc_caller_cfg_reconfig_tests) {
    ...
    { // parse configuration for broker #2 and #3
        ...
        utest_app_rpc__verify_broker_3(
            &mock_app_cfg.rpc.entries[1], (const char **)&data_str[48], &data_uint[23]
        );
    }
    ...
    { // parse configuration for all three brokers
        ...
        utest_app_rpc__verify_broker_2(
            &mock_app_cfg.rpc.entries[1], (const char **)&data_str[20], &data_uint[10]
        );
        utest_app_rpc__verify_broker_3(
            &mock_app_cfg.rpc.entries[2], (const char **)&data_str[48], &data_uint[23]
        );
    }
    {
        ...
        assert_that(
            reply_cfg->correlation_id.render_fn, is_equal_to(utset_operation_efg__rpc_corr_id_render)
        );
    }
    json_decref(objs);
    utest_rpc_dellocate_app_cfg(&mock_app_cfg);
