...
RESTAPI_ENDPOINT_HANDLER(_test_service_xyz_action_add, POST, self, req) { // dummy for test
    return 0;
}

RESTAPI_ENDPOINT_HANDLER(_test_service_xyz_action_discard, DELETE, self, req) { return 0; }

RESTAPI_ENDPOINT_HANDLER(_test_service_antutu_action_edit, PATCH, self, req) { return 0; }

RESTAPI_ENDPOINT_HANDLER(_test_service_n22_melon_action_add, POST, self, req) { return 0; }
...
Ensure(setup_route_test) {
    ...
    assert_that(
        hostcfg->paths.entries[0]->handlers.size, is_equal_to(3)
    ); // plus default handler to return status 405
    assert_that(
        hostcfg->paths.entries[0]->handlers.entries[0]->on_req, is_equal_to(_test_service_xyz_action_add)
    );
    assert_that(
        hostcfg->paths.entries[0]->handlers.entries[1]->on_req, is_equal_to(_test_service_xyz_action_discard)
    );
    ...
    assert_that(hostcfg->paths.entries[0]->path.base, is_equal_to_string("/v3/service_XYZ/func123"));
    assert_that(
        hostcfg->paths.entries[1]->handlers.size, is_equal_to(2)
    ); // plus default handler to return status 405
    assert_that(
        hostcfg->paths.entries[1]->handlers.entries[0]->on_req, is_equal_to(_test_service_antutu_action_edit)
    );
    ...
    assert_that(
        hostcfg->paths.entries[0]->handlers.size, is_equal_to(2)
    ); // plus default handler to return status 405
    assert_that(
        hostcfg->paths.entries[1]->handlers.size, is_equal_to(3)
    ); // plus default handler to return status 405
    assert_that(
        hostcfg->paths.entries[0]->handlers.entries[0]->on_req,
        is_equal_to(_test_service_n22_melon_action_add)
    );
