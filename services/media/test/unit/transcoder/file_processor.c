...
#define NUM_CB_ARGS_ASAOBJ                        (UTEST_EXPECT_CONTENT_INDEX__IN_ASA_USRARG + 1)
    ...
    asa_op_localfs_cfg_t asaobj = {
        .loop = loop,
        .super =
            {.storage = &mock_storage, .cb_args = {.size = NUM_CB_ARGS_ASAOBJ, .entries = asaobj_usr_args}}
    };
    ASA_RES_CODE result;
    ...
    for (int chunk_seq = 0; chunk_seq < NUM_FILECHUNKS; chunk_seq++) {
        asaobj.super.cb_args.entries[UTEST_EXPECT_CONTENT_INDEX__IN_ASA_USRARG] =
            (void *)expect_f_content[chunk_seq];
        // in thhis application, sequential numbers in a set of ffile chunks start from one.
        result = atfp_open_srcfile_chunk(
            &asaobj.super, FULLPATH2, chunk_seq + 1, transcoder_utest__openfile_callback
        );
...
Ensure(transcoder_test__open_srcfile_chunk_error) {
    ...
    };
    {
        ...
        ASA_RES_CODE result = atfp_open_srcfile_chunk(
            &asa_cfg.super, FULLPATH2, chunk_seq + 1, transcoder_utest__openfile_error_callback
        );
        assert_that(result, is_equal_to(ASTORAGE_RESULT_ACCEPT));
        if (result == ASTORAGE_RESULT_ACCEPT)
            uv_run(loop, UV_RUN_ONCE);
...
#define NUM_CB_ARGS_ASAOBJ                        (UTEST_EXPECT_CONTENT_INDEX__IN_ASA_USRARG + 1)
    ...
    atfp_t      mock_fp = {
             .data = {.spec = spec, .storage = {.basepath = atfp_src_basepath, .handle = &asaobj.super}}
    };
    ...
    { // open the first chunk
        ...
        asaobj.super.cb_args.entries[UTEST_EXPECT_CONTENT_INDEX__IN_ASA_USRARG] =
            (void *)expect_f_content[chunk_seq];
        ASA_RES_CODE result = atfp_open_srcfile_chunk(
            &asaobj.super, atfp_src_basepath, chunk_seq + 1, transcoder_utest__openfile_callback
        );
        assert_that(result, is_equal_to(ASTORAGE_RESULT_ACCEPT));
        uv_run(loop, UV_RUN_ONCE);
    }
...
#define NUM_SWITCHES 8
        struct {
            ...
            {3, expect_f_content[3]},  {0, expect_f_content[0]},  {-1, expect_f_content[1]},
            {4, expect_f_content[4]},  {2, expect_f_content[2]},  {1, expect_f_content[1]},
            {-1, expect_f_content[2]}, {-1, expect_f_content[3]},
        };
        for (idx = 0; idx < NUM_SWITCHES; idx++) {
            asaobj.super.cb_args.entries[UTEST_EXPECT_CONTENT_INDEX__IN_ASA_USRARG] =
                (void *)switch_chunk_seq[idx].expect_content;
            ASA_RES_CODE result = atfp_switch_to_srcfile_chunk(
                &mock_fp, switch_chunk_seq[idx].num, transcoder_utest__openfile_callback
            );
            assert_that(result, is_equal_to(ASTORAGE_RESULT_ACCEPT));
...
Ensure(transcoder_test__estimate_src_filechunk_idx) {
    ...
    { // subcase #2
        ...
        size_t offset = 1;
        size_t pos =
            strlen(expect_f_content[0]) + strlen(expect_f_content[1]) + strlen(expect_f_content[2]) + offset;
        int actual_fchunk_idx = atfp_estimate_src_filechunk_idx(spec, chunk_idx_start, &pos);
...
#define NUM_CB_ARGS_ASAOBJ (ASAMAP_INDEX__IN_ASA_USRARG + 1)
    ...
    asa_op_localfs_cfg_t mock_asa_local = {
        .super = {.cb_args = {.size = NUM_CB_ARGS_ASAOBJ, .entries = asalocal_cb_args}}
    };
    uint8_t         num_dst = 0;
...
#define NUM_DESTINATIONS   7
    ...
    { // subcase 5
        ...
        asa_op_base_cfg_t *list_after_deleted[NUM_DESTINATIONS - 2] = {
            &mock_asa_dsts[0], &mock_asa_dsts[1], &mock_asa_dsts[2], &mock_asa_dsts[4], &mock_asa_dsts[5],
        };
        uint8_t err = atfp_asa_map_remove_destination(map, deleting_asa_dst);
        assert_that(err, is_equal_to(0));
        atfp_asa_map_reset_dst_iteration(map);
...
#define UTEST_TRANSCODER__START_TRANSFER_SEGMENT_SETUP \
    ...
    asa_op_localfs_cfg_t mock_asa_local = { \
        .super = \
            {.op = \
                 {.open = {.cb = utest_asa_local_openfile_cb}, \
                  .mkdir = {.path = {.origin = UTEST_ASA_LOCAL_BASEPATH}}}, \
             .storage = &mock_storage}, \
        .loop = loop \
    }; \
    atfp_segment_t mock_seg_cfg = { \
        .rdy_list = \
            {.capacity = NUM_READY_SEGMENTS, .size = NUM_READY_SEGMENTS, .entries = &mock_rdy_seg_num[0]}, \
        .filename = \
            {.prefix = {.data = UTEST_DATA_SEGMENT_PREFIX, .sz = strlen(UTEST_DATA_SEGMENT_PREFIX)}, \
             .pattern = \
...
static __attribute__((optimize("O0"))) void
utest_asa_local_openfile_cb(asa_op_base_cfg_t *asaobj, ASA_RES_CODE result) {
    mock(asaobj, result);
}

#define NUM_READY_SEGMENTS 9
#define READY_SEGMENT_NUMBERS \
    { 12, 4, 195, 26, 87, 994, 28, 3, 13 }
...
Ensure(transcoder_test__transfer_segment__missing_file) {
    ...
    ASA_RES_CODE result =
        atfp__segment_start_transfer(&mock_asa_remote, &mock_asa_local, &mock_seg_cfg, chosen_idx);
    assert_that(result, is_equal_to(ASTORAGE_RESULT_ACCEPT));
...
#define EXPECT_FILENAME UTEST_DATA_SEGMENT_PREFIX UTEST_DATA_SEGMENT_PREFIX UTEST_DATA_SEGMENT_PREFIX
    ASA_RES_CODE result = atfp__file_start_transfer(
        &mock_asa_remote, &mock_asa_local, &mock_seg_cfg, EXPECT_FILENAME, EXPECT_FILENAME
    );
    assert_that(result, is_equal_to(ASTORAGE_RESULT_DATA_ERROR));
...
#define EXPECT_SCANDIR_ENTRIES \
    { \
        {"charlie", ASA_DIRENT_FILE}, {"jordan", ASA_DIRENT_DIR}, {"mike", ASA_DIRENT_LINK}, { \
            "lima", ASA_DIRENT_FILE \
        } \
    }
Ensure(atfp_test__scandir_load_fileinfo__ok) {
    ...
    for (idx = 0; idx < EXPECT_SCANDIR_NUM_ENTRIES; idx++) {
        ...
        expect(
            utest__mock_fn_scandir_next, will_return(ASTORAGE_RESULT_COMPLETE),
