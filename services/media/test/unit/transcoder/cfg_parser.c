...
#define TEST_SERIALIZED_JSON \
    ...
    "{}}"
    {
        ...
        expect(av_find_input_format, will_return(&ifmts[0]), when(short_name, is_equal_to_string("m4a")));
        expect(
            avcodec_find_decoder_by_name, will_return(&mock_decoders[0]),
            when(name, is_equal_to_string("h263p"))
        );
        ...
        expect(
            avcodec_find_decoder_by_name, will_return(&mock_decoders[1]),
            when(name, is_equal_to_string("av1"))
        );
        expect(
            avcodec_find_decoder_by_name, will_return(&mock_decoders[2]),
            when(name, is_equal_to_string("aac"))
        );
...
#define TEST_SERIALIZED_JSON \
    "{\"input\": {\"demuxers\":[\"m4axxx\"], \"decoders\": {\"video\":[\"av1\"], " \
    "\"audio\":[\"ac3\"]}}, " \
    " \"output\": {\"muxers\":[\"hls\",\"mpegts\",\"webm\"]}}"
    {
        ...
        expect(
            avcodec_find_decoder_by_name, will_return(&mock_decoders[1]),
            when(name, is_equal_to_string("ac3"))
        );
...
#define TEST_SERIALIZED_JSON \
    "{\"input\": {\"demuxers\":[\"m4a\"], \"decoders\": {\"video\":[\"av1\"], " \
    "\"audio\":[\"ac3\"]}}, " \
    " \"output\": {\"muxers\":[\"hls\"], \"encoders\": {\"video\":[\"hevc\"], " \
    "\"audio\":[\"dca\",\"wmav1\"]}}}"
    {
        json_t *obj = json_loads(TEST_SERIALIZED_JSON, 0, NULL);
        expect(av_find_input_format, will_return(&ifmts[0]), when(short_name, is_equal_to_string("m4a")));
        ...
        expect(av_guess_format, will_return(&ofmts[0]), when(short_name, is_equal_to_string("hls")));
        expect(
            avcodec_find_encoder_by_name, will_return(&mock_decoders[2]),
            when(name, is_equal_to_string("hevc"))
        );
        expect(
            avcodec_find_encoder_by_name, will_return(&mock_decoders[3]),
            when(name, is_equal_to_string("dca"))
        );
...
static void _test_transcoder_cfg_test_invalid_resolution(const char *serialized_json) {
    ...
    expect(av_find_input_format, will_return(&ifmts[0]), when(short_name, is_equal_to_string("m4a")));
    expect(
        avcodec_find_decoder_by_name, will_return(&mock_decoders[0]), when(name, is_equal_to_string("av1"))
    );
    expect(
        avcodec_find_decoder_by_name, will_return(&mock_decoders[1]), when(name, is_equal_to_string("ac3"))
    );
    expect(av_guess_format, will_return(&ofmts[0]), when(short_name, is_equal_to_string("hls")));
    expect(
        avcodec_find_encoder_by_name, will_return(&mock_decoders[2]), when(name, is_equal_to_string("hevc"))
    );
    expect(
        avcodec_find_encoder_by_name, will_return(&mock_decoders[3]), when(name, is_equal_to_string("dca"))
    );
    int err = parse_cfg_transcoder(obj, &acfg);
    assert_that(err, is_not_equal_to(0));
    json_decref(obj);
...
#define TEST_SERIALIZED_JSON \
    ...
    " \"output\": {\"muxers\":[\"hls\"], \"encoders\": {\"video\":[\"hevc\"], " \
    "\"audio\":[\"dca\"]}, " \
    " \"video\":{\"pixels\":[[123,456],[78,9]], \"fps\":[12,15]}, " \
    "\"audio\":{\"bitrate_kbps\":[24, -96]}  }}"
    _test_transcoder_cfg_test_invalid_resolution(TEST_SERIALIZED_JSON);
#undef TEST_SERIALIZED_JSON
...
Ensure(transcoder_cfg_test_ok) {
    ...
    AVCodec        mock_decoders[4] = {0};
#define TEST_SERIALIZED_JSON \
    "{\"input\": {\"demuxers\":[\"mp4\"], \"decoders\": {\"video\":[\"h264\"], " \
    "\"audio\":[\"aac\"]}}, " \
    " \"output\": {" \
    "     \"muxers\":[\"webm\"], \"encoders\": {\"video\":[\"hevc\"], \"audio\":[\"ac3\"]}, " \
    "     \"video\":{\"pixels\":[[240,360],[78,49], [540,450]], \"fps\":[15,19]}," \
    "     \"audio\":{\"bitrate_kbps\":[48, 80]}," \
    "     \"image\":{\"pixel_limit\":{\"width\":960,\"height\":480}, " \
    "\"mask\":{\"basepath\":\"path/to/index/file\"}}" \
    "}}"
    json_t *obj = json_loads(TEST_SERIALIZED_JSON, 0, NULL);
    expect(av_find_input_format, will_return(&ifmts[0]), when(short_name, is_equal_to_string("mp4")));
    expect(
        avcodec_find_decoder_by_name, will_return(&mock_decoders[0]), when(name, is_equal_to_string("h264"))
    );
    expect(
        avcodec_find_decoder_by_name, will_return(&mock_decoders[1]), when(name, is_equal_to_string("aac"))
    );
    ...
    expect(
        avcodec_find_encoder_by_name, will_return(&mock_decoders[3]), when(name, is_equal_to_string("ac3"))
    );
    int err = parse_cfg_transcoder(obj, &acfg);
