...
#define PACKET_INDEX_ENTRY_VIDEO \
    { \
        {.pos = MDAT_ATOM_OFFSET, .size = 9}, {.pos = 47, .size = 10}, {.pos = 60, .size = 7}, \
            {.pos = 67, .size = 6}, {.pos = 85, .size = 11}, {.pos = 96, .size = 3}, \
            {.pos = 116, .size = 5}, {.pos = 121, .size = 4}, {.pos = 125, .size = 7}, \
            {.pos = 142, .size = 3}, {.pos = 158, .size = 9}, {.pos = 189, .size = 4}, \
            {.pos = 193, .size = 10}, \
    }
#define PACKET_INDEX_ENTRY_AUDIO \
    { \
        {.pos = 41, .size = 6}, {.pos = 57, .size = 3}, {.pos = 73, .size = 8}, {.pos = 81, .size = 4}, \
            {.pos = 99, .size = 9}, {.pos = 108, .size = 8}, {.pos = 132, .size = 10}, \
            {.pos = 145, .size = 6}, {.pos = 151, .size = 7}, {.pos = 167, .size = 13}, \
            {.pos = 180, .size = 9}, \
    }
...
static void utest_atfp_mp4__avctx_preload__done_cb(atfp_mp4_t *mp4proc) { mock(mp4proc); }
...
#define UNITTEST_AVCTX_INIT__SETUP \
    ...
    AVStream          mock_av_streams[NUM_STREAMS_FMTCTX] = { \
        {.nb_index_entries = NUM_PKTS_VIDEO_STREAM, \
                  .index_entries = &mock_idx_entry_video[0], \
                  .index = 0, \
                  .codecpar = &mock_codec_param}, \
        {.nb_index_entries = NUM_PKTS_AUDIO_STREAM, \
                  .index_entries = &mock_idx_entry_audio[0], \
                  .index = 1, \
                  .codecpar = &mock_codec_param}, \
    }; \
    ...
    atfp_mp4_t           mp4proc = { \
                  ...
                  .super = \
            {.data = {.storage = {.handle = &asa_cfg_src}, .spec = json_object(), .error = json_object()}}, \
                  .av = mock_av_ctx \
    }; \
...
Ensure(atfp_mp4_test__avctx_init_ok) {
    UNITTEST_AVCTX_INIT__SETUP;
    {
        ...
        expect(
            avio_alloc_context, will_return(&mock_avio_ctx), when(buffer, is_equal_to(mock_avio_ctx_buffer))
        );
        ...
        expect(
            av_mallocz_array, will_return(mock_dec_ctxs), when(nmemb, is_equal_to(NUM_STREAMS_FMTCTX)),
        ...
        for (int idx = 0; idx < NUM_STREAMS_FMTCTX; idx++) {
            ...
            expect(
                avcodec_parameters_to_context, will_return(0), when(codec_ctx, is_equal_to(expect_codec_ctx)),
...
#pragma GCC diagnostic ignored "-Wunused-variable"
    UNITTEST_AVCTX_INIT__SETUP;
    {
        ...
        expect(
            avformat_open_input, will_return(AVERROR(ENOMEM)), when(_fmt_ctx, is_equal_to(&mock_avfmt_ctx))
        );
        ...
        expect(
            av_freep, when(ptr, is_equal_to(&mock_avio_ctx.buffer))
        ); // the address to &mock_avio_ctx_buffer[0]
...
#define UNITTEST_AVCTX_VALIDATE_SOURCE__SETUP \
    ...
    atfp_mp4_t       mp4proc = { \
              .super = {.data = {.error = json_object(), .spec = json_object()}}, .av = &mock_av_ctx \
    }; \
    { \
        int idx = 0; \
...
#define UNITTEST_AVCTX__PRELOAD_PKT_SETUP \
    ...
    atfp_mp4_t           mp4proc = { \
                  .internal = \
            {.mdat = \
                           {.pos = MDAT_ATOM_OFFSET, \
                            .fchunk_seq = 0, \
                            .size = MDAT_ATOM_BODY_SZ, \
                            .pos_wholefile = MDAT_ATOM_OFFSET, \
                            .nb_preloaded = 0}}, \
                  .super = \
            {.data = {.storage = {.handle = &asa_cfg_src}, .spec = json_object(), .error = json_object()}}, \
                  .av = &mock_av_ctx \
...
Ensure(atfp_mp4_test__avctx_subsequent_preload_all_packets) {
    ...
    expect(utest_atfp_mp4__avctx_preload__done_cb);
    ASA_RES_CODE result = atfp_mp4__av_preload_packets(
        &mp4proc, MDAT_ATOM_BODY_SZ + 123, utest_atfp_mp4__avctx_preload__done_cb
    );
    assert_that(result, is_equal_to(ASTORAGE_RESULT_ACCEPT));
    for (int idx = 0; idx < NUM_STREAMS_FMTCTX; idx++) {
...
Ensure(atfp_mp4_test__avctx_subsequent_preload_some_packets) {
    ...
    for (idx = 0; proceed_idx_entries[idx] > 0; idx++) { // subcase #1, normal
        ...
        expect(utest_atfp_mp4__avctx_preload__done_cb);
        ASA_RES_CODE result = atfp_mp4__av_preload_packets(
            &mp4proc, expect_nbytes_preload, utest_atfp_mp4__avctx_preload__done_cb
        );
        assert_that(result, is_equal_to(ASTORAGE_RESULT_ACCEPT));
    ...
    { //  subcase #2, try overloading at the final preload operation
        ...
        for (idx = 0; idx < NUM_STREAMS_FMTCTX; idx++)
            assert_that(
                mock_av_stream_stats[idx].index_entry.preloading,
...
#define EXPECT_NUM_PKTS_AVAIL  (STREAM0_NUM_PKTS_AVAIL + STREAM1_NUM_PKTS_AVAIL)
    atfp_av_ctx_t mock_avctx = {
        ...
        .stats = &mock_av_stream_stats[0],
        .intermediate_data = {.decode = {.tot_num_pkts_avail = EXPECT_NUM_PKTS_AVAIL}}
    };
    mock_av_stream_stats[0] = (atfp_stream_stats_t
    ){.index_entry = {.preloaded = STREAM0_NUM_PRELOADED, .fetched = STREAM0_NUM_FETCHED}};
    mock_av_stream_stats[1] = (atfp_stream_stats_t
    ){.index_entry = {.preloaded = STREAM1_NUM_PRELOADED, .fetched = STREAM1_NUM_FETCHED}};
    ...
    for (idx = 0; idx < EXPECT_NUM_PKTS_AVAIL; idx++) {
        ...
        expect(
            av_read_frame, will_return(0),
            will_set_contents_of_parameter(pkt, &mock_pkts[idx], sizeof(AVPacket))
        );
        mock_avctx.intermediate_data.decode.num_decoded_frames =
            100 + idx; // assume some frames were decoded in previous packet
        int new_pkt_ready = 0;
        ...
        assert_that(
            mock_avctx.intermediate_data.decode.tot_num_pkts_avail,
...
#define STREAM1_NUM_FETCHED   11
    ...
    expect(av_packet_unref, when(pkt, is_equal_to(&mock_avctx.intermediate_data.decode.packet)));
    expect(
        av_read_frame, will_return(expect_err),
        will_set_contents_of_parameter(pkt, &mock_pkts[0], sizeof(AVPacket))
    );
    int err = atfp_ffmpeg__next_local_packet(&mock_avctx);
...
Ensure(atfp_mp4_test__packet_decode__send_error) {
    ...
    expect(av_packet_rescale_ts, when(pkt, is_equal_to(&mock_avctx.intermediate_data.decode.packet)));
    expect(
        avcodec_send_packet, will_return(expect_err),
        when(avpkt, is_equal_to(&mock_avctx.intermediate_data.decode.packet))
    );
    expect(av_log);
    err = atfp_mp4__av_decode_packet(&mock_avctx);
...
Ensure(atfp_mp4_test__monitor_progress) {
    atfp_av_ctx_t mock_avctx = {
        .intermediate_data =
            {.decode =
                 {.tot_num_pkts_fixed = 10000,
                  .tot_num_pkts_avail = 10000,
                  .percent_done = 0.0f,
                  .report_interval = 0.10f}}
    };
