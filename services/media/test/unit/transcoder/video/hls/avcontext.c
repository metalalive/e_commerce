...
#define EXPECT_SPEC_SERIALIZED \
    "{ \"elementary_streams\":{" \
    "\"video-st0\":{\"type\":\"video\",\"codec\":\"libx264\",\"attribute\":{\"height_pixel\":180," \
    "\"width_pixel\":240,\"framerate\":24}}," \
    "\"audio-st0\":{\"type\":\"audio\",\"codec\":\"aac\",\"attribute\":{\"bitrate_kbps\":55}}" \
    "}, \"outputs\":{" \
    "\"" EXPECT_VERSION "\":{\"container\":\"" EXPECT_MUX_FORMAT \
    "\",\"elementary_streams\":[\"video-st0\",\"audio-st0\"]}" \
    "}}"
...
#define UNITTEST_AVCTX_INIT__SETUP \
    ...
    json_t          *mock_spec = json_loadb( \
        (const char *)EXPECT_SPEC_SERIALIZED, sizeof(EXPECT_SPEC_SERIALIZED) - 1, (size_t)0, NULL \
    ); \
    ...
    atfp_hls_t    mock_fp_src = { \
           .av = &mock_avctx_src, \
           .super = \
            {.data = {.spec = mock_spec, .error = mock_errinfo, .storage = {.handle = &mock_asa_dst}}, \
...
#define EXPECT_NB_STREAMS_IFMT_CTX 3
    ...
    {
        expect(
            avformat_alloc_output_context2, will_return(0),
            when(fmt_name, is_equal_to_string(EXPECT_MUX_FORMAT)),
            will_set_contents_of_parameter(fmtctx_p, &mock_ofmt_ctx_p, sizeof(AVFormatContext **)),
        );
        expect(
            av_mallocz_array, will_return(&mock_st_encoder_ctxs[0]),
        ...
        for (idx = 0; idx < EXPECT_NB_STREAMS_IFMT_CTX; idx++) {
            expect(
                avformat_new_stream, will_return(&mock_av_streams_dst[idx]),
        ...
        expect(
            avcodec_parameters_from_context, will_return(0),
            when(codec_ctx, is_equal_to(&mock_encoder_ctxs[0]))
        );
        expect(
            avcodec_parameters_from_context, will_return(0),
            when(codec_ctx, is_equal_to(&mock_encoder_ctxs[1]))
        );
...
#define EXPECT_NB_STREAMS_IFMT_CTX 1
    ...
    int expect_err = AVERROR(ENOMEM);
    {
        ...
        expect(
            avformat_alloc_output_context2, will_return(expect_err),
            when(fmt_name, is_equal_to_string(EXPECT_MUX_FORMAT)),
            will_set_contents_of_parameter(fmtctx_p, &mock_ofmt_ctx_p, sizeof(AVFormatContext **)),
        );
...
#define EXPECT_NB_STREAMS_IFMT_CTX 3
    ...
    int                   expect_err = AVERROR(EIO);
    {
        ...
        expect(av_frame_unref, when(frame, is_equal_to(&mock_avctx_dst.intermediate_data.encode.frame)));
        expect(
            av_freep, when(ptr, is_equal_to(&mock_avctx_dst.stream_ctx.encode))
        ); // pointer to mock_st_encoder_ctxs
...
#define EXPECT_NB_STREAMS_IFMT_CTX 1
    ...
    int                   expect_err = AVERROR(EPERM);
    {
        ...
        mock_decoder_ctxs[0].time_base = (AVRational){num : 10, den : 180};
        expect(
            avformat_new_stream, will_return(&mock_av_streams_dst[0]), when(s, is_equal_to(&mock_ofmt_ctx))
        );
        expect(avcodec_find_encoder, will_return(&mock_codecs[0]));
        expect(avcodec_alloc_context3, will_return(&mock_encoder_ctxs[0]));
        expect(avcodec_open2, will_return(0), when(ctx, is_equal_to(&mock_encoder_ctxs[0])));
...
#define EXPECT_NB_STREAMS 3
    int ret = 0, idx = 0, ret_ok = 0, ret_nxt_frm_required = 1, expect_stream_idx = EXPECT_NB_STREAMS - 1,
    ...
    expect(
        avcodec_send_frame, will_return(0), when(codec_ctx, is_equal_to(&mock_enc_ctx[expect_stream_idx]))
    );
    for (idx = 0; idx < expect_num_encoded_pkts; idx++) {
        expect(
            avcodec_receive_packet, will_return(0),
    ...
    {
        expect(
            avcodec_receive_packet, will_return(AVERROR(EAGAIN)),
            when(codec_ctx, is_equal_to(&mock_enc_ctx[expect_stream_idx]))
        );
        ret = atfp_hls__av_encode_processing(&mock_avctx_dst);
...
#define EXPECT_NB_STREAMS 3
    ...
    );
    {
        ...
        assert_that(ret, is_equal_to(ret_ok));
        expect(
            avcodec_receive_packet, will_return(expect_err),
            when(codec_ctx, is_equal_to(&mock_enc_ctx[expect_stream_idx]))
        );
        expect(av_log);
        ret = atfp_hls__av_encode_processing(&mock_avctx_dst);
...
#define EXPECT_NUM_FILT_FRAMES_FROM_STREAMS \
    ...
    for (idx = 0; idx < EXPECT_NB_STREAMS; idx++) {
        expect(
            avcodec_send_frame, will_return(0), when(frame, is_equal_to(NULL)),
