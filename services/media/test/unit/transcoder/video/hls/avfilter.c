...
#define UNITTEST_AVFILT_INIT__SETUP \
    ...
    for (idx = 0; idx < EXPECT_NB_STREAMS_IFMT_CTX; mock_av_streams_src[idx].codecpar = &mock_codecpar[idx], \
        ...
        ; \
    AVFormatContext mock_ifmt_ctx = { \
        .nb_streams = EXPECT_NB_STREAMS_IFMT_CTX, .streams = mock_av_streams_src_p \
    }; \
    ...
    for (idx = 0; idx < EXPECT_NB_STREAMS_IFMT_CTX; \
         mock_st_encoder_ctxs[idx].enc_ctx = &mock_encoder_ctxs[idx], idx++) \
        ; \
    ...
    atfp_av_ctx_t mock_avctx_dst = { \
        .fmt_ctx = &mock_ofmt_ctx, .stream_ctx = {.encode = &mock_st_encoder_ctxs[0]} \
    }; \
    ...
    atfp_hls_t mock_fp_src = { \
        .av = &mock_avctx_src, .super = {.data = {.storage = {.handle = &mock_asa_dst}}} \
    }; \
    atfp_hls_t mock_fp_dst = { \
        .av = &mock_avctx_dst, .super = {.data = {.storage = {.handle = &mock_asa_dst}}} \
    }; \
    asasrc_cb_args[ATFP_INDEX__IN_ASA_USRARG] = &mock_fp_src; \
    asadst_cb_args[ATFP_INDEX__IN_ASA_USRARG] = &mock_fp_dst;

#define UNITTEST_AVFILT_INIT__TEARDOWN atfp_asa_map_deinit(mock_map);
...
#define EXPECT_NB_STREAMS_IFMT_CTX 3
    UNITTEST_AVFILT_INIT__SETUP;
    {
        ...
        for (idx = 1; idx < EXPECT_NB_STREAMS_IFMT_CTX; idx++) {
            ...
            expect(
                avfilter_graph_create_filter, will_return(0), when(name, is_equal_to_string("in")),
                will_set_contents_of_parameter(
                    filt_ctx_p, &mock_filt_ctx_src_p[idx], sizeof(AVFilterContext **)
                ),
                when(graph_ctx, is_equal_to(&mock_filt_graph[idx])),
                when(filt, is_equal_to(&mock_av_filt_src[idx]))
            );
            expect(
                avfilter_graph_create_filter, will_return(0), when(name, is_equal_to_string("out")),
                will_set_contents_of_parameter(
                    filt_ctx_p, &mock_filt_ctx_sink_p[idx], sizeof(AVFilterContext **)
                ),
                when(graph_ctx, is_equal_to(&mock_filt_graph[idx])),
                when(filt, is_equal_to(&mock_av_filt_sink[idx]))
            );
            expect(
                avfilter_graph_parse_ptr, will_return(0), when(graph, is_equal_to(&mock_filt_graph[idx])),
        ...
        { // video stream only
            ...
            expect(
                av_opt_set_bin, will_return(0), when(name, is_equal_to_string("pix_fmts")),
        ...
        { // audio stream only
            ...
            expect(
                av_opt_set_bin, will_return(0), when(name, is_equal_to_string("sample_fmts")),
                when(obj, is_equal_to(mock_filt_ctx_sink_p[2]))
            );
            expect(
                av_opt_set_bin, will_return(0), when(name, is_equal_to_string("channel_layouts")),
                when(obj, is_equal_to(mock_filt_ctx_sink_p[2]))
            );
            // av_opt_set_int_list will expand to `av_opt_set_bin` and `av_int_list_length_for_size`
            expect(
                av_opt_set_bin, will_return(0), when(name, is_equal_to_string("sample_rates")),
                when(obj, is_equal_to(mock_filt_ctx_sink_p[2]))
            );
        }
    }
...
#define EXPECT_NB_STREAMS_IFMT_CTX 1
    ...
    int expect_err = AVERROR(E2BIG);
    {
        ...
        expect(
            ...
            when(graph_ctx, is_equal_to(&mock_filt_graph[0])), when(filt, is_equal_to(&mock_av_filt_src[0]))
        );
        expect(
            avfilter_graph_create_filter, will_return(0), when(name, is_equal_to_string("out")),
            will_set_contents_of_parameter(filt_ctx_p, &mock_filt_ctx_sink_p[0], sizeof(AVFilterContext **)),
            when(graph_ctx, is_equal_to(&mock_filt_graph[0])), when(filt, is_equal_to(&mock_av_filt_sink[0]))
        );
        {
            AVRational mock_frame_ratio = {.num = 4, .den = 1};
            expect(av_mul_q, will_return(&mock_frame_ratio));
            expect(
                av_opt_set_bin, will_return(expect_err), when(name, is_equal_to_string("pix_fmts")),
...
#define EXPECT_NB_STREAMS_IFMT_CTX 1
    ...
    int expect_err = AVERROR(ENOSYS);
    {
        ...
        );
        {
            ...
            expect(av_get_default_channel_layout, will_return(4));
            expect(
                av_opt_set_bin, will_return(expect_err), when(name, is_equal_to_string("sample_fmts")),
                when(obj, is_equal_to(mock_filt_ctx_sink_p[0]))
            );
            expect(av_log);
        }
...
#define EXPECT_NB_STREAMS_IFMT_CTX 3
    ...
    atfp_av_ctx_t   mock_avctx_src = {
          .fmt_ctx = &mock_ifmt_ctx,
          .intermediate_data = {.decode = {.packet = {.stream_index = expect_pkt_stream_idx}}}
    };
    for (idx = 0; idx < expect_num_filtered_frames; idx++) {
        if (idx == 0)
            expect(
                av_buffersrc_add_frame_flags, will_return(0),
        ...
        expect(
            av_buffersink_get_frame, will_return(0),
...
#define EXPECT_NB_STREAMS_IFMT_CTX 3
    ...
    };
    {
        ...
        expect(av_frame_unref);
        expect(
            av_buffersink_get_frame, will_return(AVERROR(EAGAIN)),
            when(frm, is_equal_to(&mock_avctx_dst.intermediate_data.encode.frame))
        );
        ret = atfp_hls__av_filter_processing(&mock_avctx_src, &mock_avctx_dst);
...
#define EXPECT_NB_STREAMS_IFMT_CTX 3
    ...
    AVFormatContext       mock_ifmt_ctx = {.nb_streams = EXPECT_NB_STREAMS_IFMT_CTX};
    atfp_av_ctx_t         mock_avctx_dst = {
                .stream_ctx = {.encode = &mock_st_encode_ctx[0]},
    ...
    };
    {
        expect(av_frame_unref);
        expect(
            av_buffersink_get_frame, will_return(expect_err),
            when(frm, is_equal_to(&mock_avctx_dst.intermediate_data.encode.frame))
        );
        expect(av_log);
        ret = atfp_hls__av_filter_processing(&mock_avctx_src, &mock_avctx_dst);
...
#define EXPECT_NUM_FILT_FRAMES_FROM_STREAMS \
    { 8, 3, 17, 11 }
