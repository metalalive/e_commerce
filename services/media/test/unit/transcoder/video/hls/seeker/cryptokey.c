...
#define HLS__CRYPTO_KEY_ACQUIRE__SETUP \
    ...
    { \
        ...
        ){.callback = _utest_hls_cryptokey_req__common_cb, \
          .spec = mock_spec, \
          .error = mock_err_info, \
          .usr_id = MOCK_USER_ID, \
...
#define UTEST__CRYPTOKEY_MIN_CONTENT \
    ...
    "\":{\"key\":{\"nbytes\":8,\"data\":\"5D4AF8331751A309\"},\"alg\":\"aes\"}}"
    ...
    if (err_cnt == 0) {
        expect(
            _utest_hls_cryptokey_req__common_cb, when(err_cnt, is_equal_to(0)),
    ...
    expect(
        _utest_hls_cryptokey_req__common_cb, when(err_cnt, is_equal_to(0)), when(is_final, is_equal_to(1)),
        when(out_chunkbytes_sz, is_equal_to(UTEST__EXPECT_kEY_SZ)),
...
Ensure(atfp_hls_test__key_req__missing_file) {
    ...
    if (err_cnt == 0) {
        expect(
            _utest_hls_cryptokey_req__common_cb, when(err_cnt, is_greater_than(0)),
            when(is_final, is_equal_to(0)), when(out_chunkbytes_sz, is_equal_to(0))
        );
        while (!mock_done_flag)
            uv_run(loop, UV_RUN_ONCE);
