...
#define HLS__LVL2_PLIST_VALIDATE__SETUP \
    ...
    atfp_hls_t mock_fp = { \
        .super = \
            {.data = \
                 {.callback = _utest_hls_lvl2_plist__common_done_cb, \
                  .spec = mock_spec, \
                  .error = mock_err_info, \
                  .usr_id = MOCK_USER_ID, \
                  .upld_req_id = MOCK_UPLD_REQ_1_ID, \
                  .version = MOCK_VERSION_STR, \
                  .storage = {.handle = NULL}}}, \
        .asa_local = \
            {.super = \
        ...
        .internal = \
            {.op = \
                 {.build_secondary_playlist = atfp_hls_stream__build_lvl2_plist, \
...
#define UTEST__PLIST_ORIGIN_CONTENT \
...
#define UTEST__CRYPTOKEY_MIN_CONTENT \
    ...
    "\":{\"iv\":{\"nbytes\":8,\"data\":\"05D4A38F331751A3\"},\"alg\":\"aes\"}}"
...
Ensure(atfp_hls_test__l2_pl__validate_ok) {
    ...
    if (err_cnt == 0) {
        ...
        expect(
            _utest_hls_lvl2_plist__common_done_cb, when(asa_src, is_not_null), when(err_cnt, is_equal_to(0)),
        ...
        assert_that(
            mock_fp.internal.op.build_secondary_playlist,
...
Ensure(atfp_hls_test__l2_pl__validate_missing_plist) {
    ...
    if (err_cnt == 0) {
        expect(
            _utest_hls_lvl2_plist__common_done_cb, when(asa_src, is_not_null),
...
Ensure(atfp_hls_test__l2_pl__validate_missing_key) {
    ...
    if (err_cnt == 0) {
        expect(
            _utest_hls_lvl2_plist__common_done_cb, when(asa_src, is_not_null),
            when(err_cnt, is_greater_than(0)), when(is_final, is_equal_to(0)),
            when(eof_reached, is_equal_to(1)), when(out_chunkbytes_sz, is_equal_to(0)),
            when(out_chunkbytes, is_null)
        );
        while (!mock_done_flag)
            uv_run(loop, UV_RUN_ONCE);
...
#define UTEST__CRYPTOKEY_MIN_CONTENT \
    "{\"73724A57\":{\"iv\":{\"nbytes\":8,\"data\":\"296F986F0B7531A9\"},\"alg\":\"aes\"}," \
    " \"" UTEST__CRYPTOKEY_CHOSEN_ID \
    "\":{\"iv\":{\"nbytes\":8,\"data\":\"5D4A38331751A3\"},\"alg\":\"aes\"}}"
#define RD_BUF_MAX_SZ (sizeof(UTEST__PLIST_ORIGIN_CONTENT) + 1)
...
#define HLS__L2_PLIST_PARSE_HDR__SETUP \
    ...
    asa_op_base_cfg_t mock_asa_src = { \
        .op = {.read = {.dst_max_nbytes = RD_BUF_MAX_SZ, .dst = UTEST__PLIST_ORIGIN}} \
    }; \
    atfp_hls_t mock_fp = { \
        .super = \
            {.data = \
...
#define UTEST__INIT_MAP_URL \
    "https://" MOCK_HOST_DOMAIN MOCK_REST_PATH "?" API_QPARAM_LABEL__STREAM_DOC_ID "=" MOCK_ENCRYPTED_DOC_ID \
    "&" API_QPARAM_LABEL__DOC_DETAIL "=" MOCK_VERSION_STR "/" HLS_FMP4_FILENAME
#define UTEST__PLIST_TAG_MAP_PARSED "\n#EXT-X-MAP:USR_ATTR=987,URI=\"" UTEST__INIT_MAP_URL "\""
#define UTEST__MAX_TARGET_DURATION  "8299"
#define UTEST__PLIST_COMMON_HEADER \
    "#EXTM3U\n#EXT-X-VERSION:6\n#EXT-X-TARGETDURATION:" UTEST__MAX_TARGET_DURATION \
    "\n#EXT-X-MEDIA-SEQUENCE:0\n#EXT-X-PLAYLIST-TYPE:VOD"
#define UTEST__PLIST_ORIGIN \
    UTEST__PLIST_COMMON_HEADER "\n#EXT-X-MAP:USR_ATTR=987,URI=\"" HLS_FMP4_FILENAME "\"\n#EXTINF:0.123"
...
#define RD_BUF_MAX_SZ (sizeof(UTEST__PLIST_PARSED) + 1)
    ...
    expect(
        _utest_hls_lvl2_plist__common_done_cb, when(err_cnt, is_equal_to(0)), when(is_final, is_equal_to(0)),
        when(out_chunkbytes_sz, is_greater_than(0)), when(out_chunkbytes_sz, is_less_than(RD_BUF_MAX_SZ)),
        when(out_chunkbytes, begins_with_string(UTEST__PLIST_PARSED))
    );
    mock_fp.internal.op.build_secondary_playlist(&mock_fp);
    assert_that(
        mock_fp.internal.op.build_secondary_playlist, is_equal_to(atfp_hls_stream__lvl2_plist__parse_extinf)
    );
...
#define RD_BUF_MAX_SZ (sizeof(UTEST__PLIST_COMMON_HEADER) + 5)
    HLS__L2_PLIST_PARSE_HDR__SETUP
    expect(
        _utest_hls_lvl2_plist__common_done_cb, when(err_cnt, is_greater_than(0)),
...
#define HLS__L2_PLIST_PARSE_EXTINF__SETUP \
    ...
    asa_op_base_cfg_t mock_asa_src = { \
        .storage = &mock_src_storage_cfg, \
        .cb_args = {.entries = mock_asasrc_cb_args, .size = NUM_CB_ARGS_ASAOBJ} \
    }; \
    atfp_hls_t mock_fp = { \
        .super = \
            {.data = \
...
#define UTEST_DATASEG_PREFIX_URL \
    "https://" MOCK_HOST_DOMAIN MOCK_REST_PATH "?" API_QPARAM_LABEL__STREAM_DOC_ID "=" MOCK_ENCRYPTED_DOC_ID \
    "&" API_QPARAM_LABEL__DOC_DETAIL "=" MOCK_VERSION_STR "/" HLS_SEGMENT_FILENAME_PREFIX
#define SINGLE_EXTINF_PARSED_SZ \
...
#define UTEST__PLIST_ORIGIN_PART2 \
    ".70567,\n" HLS_SEGMENT_FILENAME_PREFIX STRINGIFY_SEG_NUM(4 \
    ) "xxxx" \
...
#define RD_BUF_MAX_SZ sizeof(UTEST__PLIST_ORIGIN_PART1)
    ...
    { // subcase #1
        ...
        expect(
            _utest_hls_lvl2_plist__common_done_cb, when(err_cnt, is_equal_to(0)),
            when(eof_reached, is_equal_to(0)), when(is_final, is_equal_to(0)),
            when(out_chunkbytes_sz, is_greater_than(0)), when(out_chunkbytes_sz, is_less_than(WR_BUF_MAX_SZ)),
            when(out_chunkbytes, begins_with_string(UTEST__PLIST_PARSED_PART1))
        );
        mock_fp.internal.op.build_secondary_playlist(&mock_fp);
        assert_that(
            atfp_hls_lvl2pl__load_curr_rd_ptr(&mock_fp),
            begins_with_string("\n" HLS_SEGMENT_FILENAME_PREFIX STRINGIFY_SEG_NUM(2) "\n#EXTINF:24.040561,")
        );
    }
    { // subcase #2
    ...
    { // subcase #3
        ...
        expect(
            utest_storage_read_fn, will_return(ASTORAGE_RESULT_ACCEPT),
            will_set_contents_of_parameter(
                read_dst_p, UTEST__PLIST_ORIGIN_PART2, sizeof(char) * expect_nread
            ),
...
#define RD_BUF_MAX_SZ sizeof(UTEST__PLIST_ORIGIN_PART1)
    ...
    { // subcase #1
        ...
        expect(
            ...
            when(out_chunkbytes_sz, is_greater_than(0)), when(out_chunkbytes_sz, is_less_than(WR_BUF_MAX_SZ)),
            when(out_chunkbytes, begins_with_string(UTEST__PLIST_PARSED_PART1))
        ); // CAUTION, string in begins_with_string() longer than actual buffer will cause undefined
           // behaviour
        mock_fp.internal.op.build_secondary_playlist(&mock_fp);
        assert_that(atfp_hls_lvl2pl__load_curr_rd_ptr(&mock_fp), begins_with_string(expect_unread_prev));
    }
