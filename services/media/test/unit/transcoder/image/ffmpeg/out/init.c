...
static void utest_img_ffo__avctx_init(
...
static void
utest_img_ffo__avfilt_init(atfp_av_ctx_t *src, atfp_av_ctx_t *dst, json_t *filt_spec, json_t *err_info) {
...
static void utest_img_ffo__avctx_deinit(atfp_av_ctx_t *dst) { mock(dst); }

static void utest_img_ffo__asaobj_deinit(asa_op_base_cfg_t *asaobj) { mock(asaobj); }

static void utest_atfp_img_ffo__usr_cb(atfp_t *processor) { mock(processor); }
...
#define UTEST_FFM_COMMON_SETUP \
    ...
    asa_op_base_cfg_t mock_asa_dst = { \
        .cb_args = {.size = NUM_CB_ARGS_ASAOBJ, .entries = asadst_cb_args}, \
        .deinit = utest_img_ffo__asaobj_deinit, \
        .op = {.mkdir = {.path = {.origin = UTEST_REMOTE_FILE_PATH}}} \
    }; \
    asa_op_localfs_cfg_t mock_asa_local = { \
        .super = {.op = {.open = {.dst_path = UTEST_SRC_FILE_LOCALBUF_PATH}}} \
    }; \
...
#define UTEST_FFM_INIT_SETUP \
    ...
    mock_fp_dst->super.data = (atfp_data_t \
    ){.storage = {.handle = &mock_asa_dst}, \
      .version = UTEST_FP_VERSION, \
      .error = mock_errinfo, \
      .spec = mock_spec, \
      .callback = utest_atfp_img_ffo__usr_cb}; \
...
static int utest_img_ffo__avctx_wirte_pkt_local(atfp_av_ctx_t *avobj) { return (int)mock(avobj); }

static int utest_img_ffo__avctx_encode(atfp_av_ctx_t *avobj) { return (int)mock(avobj); }

static int utest_img_ffo__avctx_filter(atfp_av_ctx_t *src, atfp_av_ctx_t *dst) { return (int)mock(src, dst); }

static int utest_img_ffo__avctx_final_filter(atfp_av_ctx_t *src, atfp_av_ctx_t *dst) {
    return (int)mock(src, dst);
}

static int utest_img_ffo__avctx_final_encode(atfp_av_ctx_t *avobj) { return (int)mock(avobj); }

static int utest_img_ffo__avctx_final_write_local(atfp_av_ctx_t *avobj) { return (int)mock(avobj); }

static int utest_img_ffo__avctx_has_done_flush_filter(atfp_av_ctx_t *avobj) { return (int)mock(avobj); }
...
Ensure(atfp_img_ffo_test__process_write_pkt_ok) {
    UTEST_FFM_PROCESS_SETUP // assume 2 packets will be encoded from the filtered frame
        expect(
            utest_img_ffo__avctx_filter, will_return(ATFP_AVCTX_RET__OK),
            when(src, is_equal_to(&mock_avctx_src)), when(dst, is_equal_to(&mock_avctx_dst))
        );
    expect(
        utest_img_ffo__avctx_encode, will_return(ATFP_AVCTX_RET__OK),
        when(avobj, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__avctx_wirte_pkt_local, will_return(ATFP_AVCTX_RET__OK),
        when(avobj, is_equal_to(&mock_avctx_dst))
    );
    ...
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(0),
...
Ensure(atfp_img_ffo_test__process_filter_error) {
    ...
    expect(
        utest_img_ffo__avctx_filter, will_return(expect_err_code), when(src, is_equal_to(&mock_avctx_src)),
        when(dst, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(0),
        when(processor, is_equal_to(&mock_fp_src.super))
...
Ensure(atfp_img_ffo_test__process_encode_error) {
    ...
    int expect_err_code = -3;
    expect(
        utest_img_ffo__avctx_filter, will_return(ATFP_AVCTX_RET__OK), when(src, is_equal_to(&mock_avctx_src)),
        when(dst, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__avctx_encode, will_return(expect_err_code), when(avobj, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(0),
        when(processor, is_equal_to(&mock_fp_src.super))
...
Ensure(atfp_img_ffo_test__process_need_more_frame) {
    UTEST_FFM_PROCESS_SETUP
    expect(
        utest_img_ffo__avctx_filter, will_return(ATFP_AVCTX_RET__NEED_MORE_DATA),
        when(src, is_equal_to(&mock_avctx_src)), when(dst, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(0),
...
Ensure(atfp_img_ffo_test__process_flush_filter) {
    ...
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(src_fp_done),
        when(processor, is_equal_to(&mock_fp_src.super))
    );
    expect(utest_img_ffo__avctx_has_done_flush_filter, will_return(0));
    atfp__image_ffm_out__proceeding_transcode(&mock_fp_dst.super);
    ...
    // ---------
    expect(
        utest_img_ffo__avctx_final_filter, will_return(ATFP_AVCTX_RET__OK),
        when(src, is_equal_to(&mock_avctx_src)), when(dst, is_equal_to(&mock_avctx_dst))
    );
    expect(utest_img_ffo__avctx_encode, will_return(ATFP_AVCTX_RET__OK));
    expect(utest_img_ffo__avctx_wirte_pkt_local, will_return(ATFP_AVCTX_RET__OK));
    expect(utest_img_ffo__avctx_encode, will_return(ATFP_AVCTX_RET__NEED_MORE_DATA));
    expect(
        utest_img_ffo__avctx_final_filter, will_return(ATFP_AVCTX_RET__NEED_MORE_DATA),
        when(src, is_equal_to(&mock_avctx_src)), when(dst, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(src_fp_done),
...
Ensure(atfp_img_ffo_test__process_save2storage_ok) {
    ...
    expect(
        utest_img_ffo__avctx_final_encode, will_return(ATFP_AVCTX_RET__OK),
        when(avobj, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__avctx_wirte_pkt_local, will_return(ATFP_AVCTX_RET__OK),
    ...
    expect(utest_img_ffo__avctx_has_done_flush_filter, will_return(dst_flush_filt_done));
    expect(
        utest_img_ffo__avctx_final_write_local, will_return(ATFP_AVCTX_RET__OK),
        when(avobj, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__avctx_save2storage, will_return(ASTORAGE_RESULT_ACCEPT),
...
Ensure(atfp_img_ffo_test__process_save2storage_err) {
    ...
    expect(
        utest_img_ffo__avctx_final_encode, will_return(ATFP_AVCTX_RET__END_OF_FLUSH_ENCODER),
        when(avobj, is_equal_to(&mock_avctx_dst))
    );
    expect(
        utest_img_ffo__fp_src__has_done_processing, will_return(src_fp_done),
    ...
    expect(
        utest_img_ffo__avctx_save2storage, will_return(ASTORAGE_RESULT_DATA_ERROR),
        when(fp, is_equal_to(&mock_fp_dst.super))
    );
    atfp__image_ffm_out__proceeding_transcode(&mock_fp_dst.super);
    assert_that(json_object_size(mock_errinfo), is_greater_than(0));
