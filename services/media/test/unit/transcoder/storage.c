...
#define EXPECT_COMMITTING_FILENAMES \
    { "let", "go", "calm", "down" }
#define EXPECT_DISCARDING_NFILES 5
#define EXPECT_DISCARDING_FILENAMES \
    { "Palau", "Guam", "Fiji", "Marshall", "Tonga" }
...
static void
_atfp_utest__commit_version__construct_path(char *out, size_t o_sz, const char *status, const char *fname) {
    memset(&out[0], 0, o_sz);
...
#define UTEST__COMMIT_VERSION_SETUP \
         ...
         .super = \
             {.storage = &mock_storage_cfg, \
              ...
              .op = { \
                  .mkdir = \
                      {.path = \
                           {.origin = &mkdir_path_origin[0], \
                            .prefix = &mkdir_path_prefix[0], \
    ...
    atfp_t  mock_fp = { \
         .data = \
            {.error = mock_errinfo, \
              .callback = utest__commit_new_version__done_cb, \
              .storage = {.handle = &mock_asa_dst.super}, \
              .usr_id = UTEST_USER_ID, \
              .upld_req_id = UTEST_UPLOAD_REQ_ID, \
              .version = UTEST_VERSION}, \
         .transfer = {.transcoded_dst = {.flags = {.version_exists = EXPECT_VERSION_EXIST_FLAG}}} \
    }; \
...
Ensure(atfp_test__commit_version__discard_old) {
    ...
    _atfp_utest__commit_version__files_setup(
        ATFP__TEMP_TRANSCODING_FOLDER_NAME, (const char **)committing_fnames, EXPECT_COMMITTING_NFILES
    );
    _atfp_utest__commit_version__files_setup(
        ATFP__COMMITTED_FOLDER_NAME, (const char **)discarding_fnames, EXPECT_DISCARDING_NFILES
    );
    {
        expect(
            utest__commit_new_version__done_cb, when(processor, is_equal_to(&mock_fp)),
    ...
    _atfp_utest__commit_version__files_teardown(
        ATFP__COMMITTED_FOLDER_NAME, (const char **)committing_fnames, EXPECT_COMMITTING_NFILES
    );
    _atfp_utest__commit_version__files_teardown(
        ATFP__DISCARDING_FOLDER_NAME, (const char **)discarding_fnames, EXPECT_DISCARDING_NFILES
    );
...
Ensure(atfp_test__commit_version__dup_error) {
    ...
    { // will fail to commit due to duplicate version folder in committed status
        ...
        while (!done_flag)
            uv_run(loop, UV_RUN_ONCE);
        _atfp_utest__commit_version__verify_existence(
            ATFP__TEMP_TRANSCODING_FOLDER_NAME, (const char **)committing_fnames, EXPECT_COMMITTING_NFILES, 0
        );
        _atfp_utest__commit_version__verify_existence(
            ATFP__COMMITTED_FOLDER_NAME, (const char **)discarding_fnames, EXPECT_DISCARDING_NFILES, 0
        );
    }
    _atfp_utest__commit_version__files_teardown(
        ATFP__TEMP_TRANSCODING_FOLDER_NAME, (const char **)committing_fnames, EXPECT_COMMITTING_NFILES
