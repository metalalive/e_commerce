...
static void mock_server_lowlevel_send(
    struct st_h2o_ostream_t *self, h2o_req_t *req, h2o_sendvec_t *bufs, size_t bufcnt, h2o_send_state_t state
) {
...
BeforeEach(MOCK_AUTH_PART) {
    ...
    mock_req.res.headers =
        (h2o_headers_t){.size = 0, .capacity = NUM_RESP_HEADERS, .entries = &mock_headers[NUM_REQ_HEADERS]};
    mock_req.headers =
        (h2o_headers_t){.size = NUM_REQ_HEADERS, .capacity = NUM_REQ_HEADERS, .entries = &mock_headers[0]};
...
Ensure(MOCK_AUTH_PART, auth_jwt_init_failure_tests) {
    ...
    *mock_req.headers.entries[1].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    *mock_req.headers.entries[2].name = (h2o_iovec_t){.len = 7, .base = "culture"};
    mock_req.headers.entries[1].value =
        (h2o_iovec_t){.len = sizeof(mock_encoded_token), .base = &mock_encoded_token[0]};
...
#define APPENDING_JUNK     "This-is-junk-data"
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    mock_req.headers.entries[0].value =
        (h2o_iovec_t){.len = sizeof(HDR_VAL_PREFIX MOCK_ENCODED_TOKEN) - 1, .base = &mock_encoded_token[0]};
    jwt_t  mock_jwt = {0};
    jwt_t *mock_jwt_ptr = &mock_jwt;
    expect(r_jwks_is_valid, will_return(RHN_OK));
    expect(
        r_jwt_init, will_return(RHN_OK), will_set_contents_of_parameter(jwt, &mock_jwt_ptr, sizeof(jwt_t **))
    );
    expect(
        r_jwt_parsen, when(token_len, is_equal_to(sizeof(MOCK_ENCODED_TOKEN) - 1)),
...
Ensure(MOCK_AUTH_PART, auth_jwt_incorrect_audience_1_tests) {
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    ...
    expect(
        r_jwt_get_claim_json_t_value, will_return(mock_aud_claim), when(jwt, is_equal_to(mock_jwt_ptr)),
...
Ensure(MOCK_AUTH_PART, auth_jwt_missing_keyid_tests) {
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    ...
    expect(
        r_jwt_get_header_str_value, will_return(NULL), when(jwt, is_equal_to(mock_jwt_ptr)),
...
Ensure(MOCK_AUTH_PART, auth_jwt_incorrect_pubkey_tests) {
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    ...
    expect(r_jwt_get_type, will_return(R_JWT_TYPE_SIGN));
    expect(
        r_jwt_get_header_str_value, will_return(mock_keyid), when(jwt, is_equal_to(mock_jwt_ptr)),
        when(key, is_equal_to_string("kid"))
    );
...
Ensure(MOCK_AUTH_PART, auth_jwt_verify_signature_failure_tests) {
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    ...
    expect(
        r_jwt_verify_signature, will_return(RHN_ERROR), when(jwt, is_equal_to(mock_jwt_ptr)),
        when(pubkey, is_equal_to(&mock_jwk)), with_side_effect(&_update_trace_point, (void *)&tracepoint)
    );
    expect(r_jwt_free, when(jwt, is_equal_to(mock_jwt_ptr)));
    app_authenticate_user(&mock_hdlr, &mock_req, &mock_mdchain_head);
    assert_that(mock_req.res.status, is_equal_to(401));
...
Ensure(MOCK_AUTH_PART, auth_jwt_expiry_tests) {
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    ...
    expect(r_jwk_is_valid, will_return(RHN_OK), when(jwk, is_equal_to(&mock_jwk)));
    expect(
        r_jwt_verify_signature, will_return(RHN_OK), when(jwt, is_equal_to(mock_jwt_ptr)),
        when(pubkey, is_equal_to(&mock_jwk))
    );
    expect(r_jwt_validate_claims, will_return(RHN_ERROR), when(jwt, is_equal_to(mock_jwt_ptr)));
    expect(r_jwt_free, when(jwt, is_equal_to(mock_jwt_ptr)));
    app_authenticate_user(&mock_hdlr, &mock_req, &mock_mdchain_head);
    assert_that(mock_req.res.status, is_equal_to(401));
...
#define APPENDING_JUNK     "This-is-junk-data"
    ...
    *mock_req.headers.entries[0].name = (h2o_iovec_t){.len = 13, .base = "authorization"};
    ...
    trcptr_t    tracepoint = {.data = NULL, .new_state = "reach_r_jwt_get_full_claims_json_t"};
    {
        ...
        expect(r_jwt_validate_claims, will_return(RHN_OK), when(jwt, is_equal_to(mock_jwt_ptr)));
        expect(
            r_jwt_get_full_claims_json_t, will_return(mock_full_claims), when(jwt, is_equal_to(mock_jwt_ptr)),
...
Ensure(rotate_jwks_remote_server_error_tests) {
    ...
    int    result = 0;
    for (int idx = 0; idx < 3; idx++) { // try re-entering the function and it should still get the same error
        expect(
            r_jwks_init, will_return(RHN_OK),
            will_set_contents_of_parameter(jwks, &new_handle_ptr, sizeof(jwks_t **))
        );
        expect(r_jwks_free, when(jwks, is_equal_to(new_handle_ptr)));
        expect(
            DEV_r_jwks_import_from_uri, will_return(RHN_ERROR), when(jwks, is_equal_to(new_handle_ptr)),
...
Ensure(rotate_jwks_succeed_tests) {
    ...
    };
    {
        ...
        expect(
            DEV_r_jwks_import_from_uri, will_return(RHN_OK), when(jwks, is_equal_to(new_handle_ptr)),
            when(uri, is_equal_to_string(src_url))
        );
        expect(r_jwks_is_valid, will_return(RHN_OK), when(jwks, is_equal_to(new_handle_ptr)));
