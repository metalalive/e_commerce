...
static db_conn_t *   mock_db_pool__acquire_free_conn_fn(db_pool_t *pool)
{ return (db_conn_t *)mock(pool); }

static DBA_RES_CODE  mock_db_pool__release_used_conn_fn(db_conn_t *conn)
...
#define EXPECT_RAW_SQL "SELECT col2, col3 FROM table56; INSERT INTO table56(col2, col3) VALUES('beard', 839274); UPDATE table356 SET logusr = 'wood';"
#define EXPECT_NUM_RS  2
Ensure(app_db_query_test_start_new_query_failure) {
    db_pool_t pool = { .cfg = {.bulk_query_limit_kb = 0},
    ...
    uv_loop_t loop = {0};
    db_query_cfg_t  qcfg = {
        .statements = {.entry = EXPECT_RAW_SQL, .num_rs = EXPECT_NUM_RS}, .pool = &pool,
        .usr_data = {.entry = (void **)NULL, .len = 0}, .loop = &loop,
        .callbacks = {
    ...
    { // error happened due to incorrect size of internal buffer in `db_conn_t` object
        expect(mock_db_pool__is_closing_fn, will_return(0), when(pool, is_equal_to(&pool)));
        expect(mock_db_pool__acquire_free_conn_fn, will_return(&free_conn),
             when(pool, is_equal_to(&pool)));
        expect(mock_db_pool__release_used_conn_fn, will_return(DBA_RESULT_OK),
...
#define NUM_USR_DATA_ITEMS  2
    ...
    db_pool_t pool = { .cfg = {.bulk_query_limit_kb = 1},  .is_closing_fn = mock_db_pool__is_closing_fn,
        .acquire_free_conn_fn = mock_db_pool__acquire_free_conn_fn,
        .release_used_conn_fn = mock_db_pool__release_used_conn_fn };
    uv_loop_t *_loop = uv_default_loop();
    db_query_cfg_t  qcfg = {
        .statements = {.entry = EXPECT_RAW_SQL, .num_rs = EXPECT_NUM_RS}, .pool = &pool,
        .usr_data = {.entry = (void **)db_async_usr_data, .len = NUM_USR_DATA_ITEMS}, .loop = _loop,
        .callbacks = {
            .result_rdy  = mock_db_query_cb__result_set_ready,
            .row_fetched = mock_db_query_cb__fetch_row,
            .result_free = mock_db_query_cb__result_set_free,
    ...
    db_conn_t free_conn = {.ops = {.try_process_queries = mock_db_conn__try_process_queries,
    ...
    }};
    {
        ...
        expect(mock_db_pool__acquire_free_conn_fn, will_return(&free_conn),
             when(pool, is_equal_to(&pool)));
        expect(mock_db_conn__add_new_query, will_return(DBA_RESULT_OK),
             when(conn, is_equal_to(&free_conn)));
        expect(mock_db_pool__release_used_conn_fn, will_return(DBA_RESULT_OK),
             when(conn, is_equal_to(&free_conn)));
        expect(mock_db_conn__try_process_queries, will_return(DBA_RESULT_OK),
