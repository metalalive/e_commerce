...
static int mock_app__timerpoll_deinit(app_timer_poll_t *handle) { return (int)mock(handle); }

static int mock_app__timerpoll_change_fd(app_timer_poll_t *handle, int fd) { return (int)mock(handle, fd); }

static int mock_app__timerpoll_start(
    app_timer_poll_t *handle, uint64_t timeout_ms, uint32_t event_flags, timerpoll_poll_cb poll_cb
) {
    return (int)mock(handle, timeout_ms, event_flags, poll_cb);
}

static int mock_app__timerpoll_stop(app_timer_poll_t *handle) { return (int)mock(handle); }
...
Ensure(app_mariadb_test_acquire_state_change) {
    ...
    { // assume 2 worker threads are contending to perform the state transition whenever
      // the connection state is valid
        enum _dbconn_async_state valid_states[] = {
            DB_ASYNC_INITED, DB_ASYNC_CONN_START, DB_ASYNC_QUERY_START
        };
...
Ensure(app_mariadb_test_start_connection_failure) {
    ...
    db_pool_t pool = {
        .cfg = {.bulk_query_limit_kb = CONN_BULK_QUERY_LIMIT_KB}, .is_closing_fn = mock_db_pool__is_closing_fn
    };
    db_conn_t conn = {
        .pool = &pool,
        .ops = {.timerpoll_stop = mock_app__timerpoll_stop},
        .processing_queries = NULL,
        .pending_queries = {.head = NULL},
    ...
    expect(
        mysql_real_connect_start, will_return(0),
...
Ensure(app_mariadb_test_connect_db_server_error) {
    ...
    db_conn_t conn = {
        ...
        .loop = &loop,
        .processing_queries = NULL,
        .pending_queries = {.head = NULL, .tail = NULL},
        .state = DB_ASYNC_INITED,
        .ops =
            {.timerpoll_stop = mock_app__timerpoll_stop,
             .timerpoll_start = mock_app__timerpoll_start,
    ...
    { // assume connection packet is sent successfully  on client side
        ...
        expect(
            mysql_real_connect_start, will_return(MYSQL_WAIT_READ),
            will_set_contents_of_parameter(ret, &mysql_conn_ret, sizeof(MYSQL *))
        );
        expect(mysql_get_socket, will_return(expect_lowlvl_fd));
        expect(mysql_get_timeout_value_ms, will_return(expect_timeout_ms));
        expect(
            mock_app__timerpoll_init, will_return(0), when(loop, is_equal_to(&loop)),
            when(handle, is_equal_to(&conn.timer_poll)), when(fd, is_equal_to(expect_lowlvl_fd))
        );
        expect(
            mock_app__timerpoll_start, will_return(0), when(timeout_ms, is_equal_to(expect_timeout_ms)),
    ...
    { // assume db sevrer returns connection error
        ...
        assert_that(conn.state, is_equal_to(DB_ASYNC_CONN_WAITING));
        expect(
            mysql_real_connect_cont, will_return(0),
            will_set_contents_of_parameter(ret, &mysql_conn_ret, sizeof(MYSQL *))
        );
        expect(mysql_errno, will_return(ER_TOO_MANY_USER_CONNECTIONS));
        expect(mysql_error, will_return("unit test mock error"));
        ...
        expect(
            mysql_close_start, will_return(MYSQL_WAIT_READ),
...
Ensure(app_mariadb_test_evict_all_queries_on_connection_failure) {
    ...
    db_conn_t conn = {
        .pool = &pool,
        .loop = &loop,
        .processing_queries = &mock_processing_nodes[0].node,
    ...
    { // init mock queries
        ...
        mock_pending_nodes[1].node =
            (db_llnode_t){.prev = &mock_pending_nodes[0].node, .next = &mock_pending_nodes[2].node};
        ...
        mock_processing_nodes[1].node =
            (db_llnode_t){.prev = &mock_processing_nodes[0].node, .next = &mock_processing_nodes[2].node};
        ...
        for (q_node = conn.processing_queries; q_node; q_node = q_node->next) {
            ...
            *q = (db_query_t
            ){.cfg = {.loop = &loop}, .notification = {.async_cb = mock_db_query__notify_callback}};
        }
    ...
    { // assume there are pending and processing queries in this case. Evict them
        ...
        expect(mock_app__timerpoll_stop, will_return(0));
        expect(
            mock_db_query__notify_callback, when(app_result, is_equal_to(DBA_RESULT_NETWORK_ERROR)),
        ...
        expect(
            mock_db_query__notify_callback, when(q_found, is_equal_to(&mock_processing_nodes[1].node.data))
        );
        expect(
            mock_db_query__notify_callback, when(q_found, is_equal_to(&mock_processing_nodes[2].node.data))
        );
        expect(
            mock_db_query__notify_callback, when(app_result, is_equal_to(DBA_RESULT_NETWORK_ERROR)),
            when(conn_state, is_equal_to(DB_ASYNC_CONN_DONE)), when(is_async, is_equal_to(0)),
...
Ensure(app_mariadb_test_query_failure_local_2) {
    ...
    expect(
        mysql_real_query_start, will_return(MYSQL_WAIT_READ),
        will_set_contents_of_parameter(ret, &mysql_query_ret, sizeof(int *))
    );
    expect(mysql_get_timeout_value_ms, will_return(expect_timeout_ms));
    expect(
        mock_app__timerpoll_start, will_return(UV_EPERM), when(timeout_ms, is_equal_to(expect_timeout_ms)),
        when(event_flags, is_equal_to(UV_READABLE))
    );
    expect(mock_app__timerpoll_stop, will_return(0));
    expect(
        mock_db_query__notify_callback, when(app_result, is_equal_to(DBA_RESULT_OS_ERROR)),
...
Ensure(app_mariadb_test_query_failure_remote) {
    ...
    uv_loop_t         loop = {0};
    db_pool_t         pool = {
                .cfg = {.bulk_query_limit_kb = CONN_BULK_QUERY_LIMIT_KB, .ops = {0}},
                .is_closing_fn = mock_db_pool__is_closing_fn
    };
    db_conn_t conn = {
        .pool = &pool,
    ...
    {                            // assume local app sent the queries successfully
        ...
        expect(
            mysql_real_query_cont, will_return(0),
            will_set_contents_of_parameter(ret, &mysql_query_ret, sizeof(int *))
        );
        ...
        expect(mock_app__timerpoll_stop, will_return(0));
        expect(
            mock_db_query__notify_callback, when(app_result, is_equal_to(DBA_RESULT_MEMORY_ERROR)),
            when(conn_state, is_equal_to(DB_ASYNC_QUERY_READY)), when(is_async, is_equal_to(0)),
            when(is_final, is_equal_to(1)), when(q_found, is_equal_to(&mock_processing_nodes[0].node.data))
        );
        expect(mock_db_conn__update_ready_queries, will_return(DBA_RESULT_SKIPPED));
...
Ensure(app_mariadb_test_query_resultset_no_rows) {
    ...
    db_pool_t         pool = {
                .cfg =
            {.bulk_query_limit_kb = CONN_BULK_QUERY_LIMIT_KB,
                     .ops = {.get_timeout_ms = app_db_mariadb_get_timeout_ms}}
    };
    db_conn_t conn = {
        .pool = &pool,
        .loop = &loop,
    ...
    {                             // assume local app sent the queries successfully
        ...
        expect(
            mock_db_query__notify_callback, when(app_result, is_equal_to(DBA_RESULT_END_OF_ROWS_REACHED)),
            when(conn_state, is_equal_to(DB_ASYNC_CHECK_CURRENT_RESULTSET)), when(is_async, is_equal_to(0)),
            when(is_final, is_equal_to(1)), when(q_found, is_equal_to(&mock_processing_nodes[0].node.data))
        );
        expect(
            mysql_next_result_start, will_return(MYSQL_WAIT_READ),
...
Ensure(app_mariadb_test_query_next_resultset_found) {
    ...
    { // assume there is next result set after the recent query is executed on DB server
        ...
        expect(
            mysql_next_result_cont, will_return(0),
            will_set_contents_of_parameter(ret, &mysql_query_ret, sizeof(int *))
        );
        expect(mock_app__timerpoll_stop, will_return(0));
        ...
        expect(
            mock_db_query__notify_callback, when(app_result, is_equal_to(DBA_RESULT_OK)),
            when(conn_state, is_equal_to(DB_ASYNC_CHECK_CURRENT_RESULTSET)), when(is_async, is_equal_to(0)),
            when(is_final, is_equal_to(0)), when(q_found, is_equal_to(&mock_processing_nodes[0].node.data))
        );
        expect(
            mysql_fetch_row_start, will_return(MYSQL_WAIT_READ),
...
Ensure(app_mariadb_test_query_reach_end_of_resultsets) {
    ...
    expect(mock_app__timerpoll_stop, will_return(0));
    expect(
        mock_db_conn__update_ready_queries, will_return(DBA_RESULT_OK)
    ); // assume there is new pending query
    expect(
        mysql_real_query_start, will_return(MYSQL_WAIT_READ),
        will_set_contents_of_parameter(ret, &mysql_query_ret, sizeof(int *))
    );
...
#define EXPECT_NUM_COLUMNS 3
    ...
    *q = (db_query_t
    ...
    expect(
        mysql_fetch_row_cont, will_return(0),
    ...
    for (size_t idx = 0; idx < EXPECT_NUM_COLUMNS; idx++) {
        expect(
            mock_db_query__notify_callback, when(col_value, is_equal_to_string(expect_mysql_rows[0][idx]))
        );
    }
    expect(
        mysql_fetch_row_start, will_return(MYSQL_WAIT_READ),
...
static void
_app_mariadb_test_free_resultset_iteration(db_conn_t *conn, db_query_t *expect_q, uint8_t _is_final) {
...
Ensure(app_mariadb_test_deinit_start) {
    ...
    conn.state = DB_ASYNC_QUERY_START;
    expect(
        mock_db_conn__update_ready_queries, will_return(DBA_RESULT_SKIPPED)
    ); // assume no more pending query
...
Ensure(app_mariadb_test_reconnecting) {
    ...
    assert_that(app_mariadb_acquire_state_change(&conn), is_equal_to(1));
    {
        ...
        expect(mysql_get_timeout_value_ms, will_return(expect_timeout_ms));
        expect(
            mock_app__timerpoll_change_fd, will_return(0), when(handle, is_equal_to(&conn.timer_poll)),
...
Ensure(app_mariadb_test_notify_query_callback) {
    db_query_t query = {
        .cfg =
            {.callbacks =
                 {.result_rdy = mock_db_query__resultset_rdy,
                  .row_fetched = mock_db_query__row_fetched,
                  .result_free = mock_db_query__resultset_free,
