...
static void mock_timerpoll_deinit_cb(app_timer_poll_t *target)
{
    mock(target);
}
...
Ensure(app_timerpoll_target_fd_working_test) {
    ...
    _app_timerpoll_test_setup(loop, &handle.super, pipe_fd_pair);
    {
        ...
        expect(mock_timerpoll_data_ready_cb, when(target, is_equal_to(&handle.super)),
        ...
        assert_that(&handle.rd_buf[0], is_equal_to_string("C10K" "DroneIT"));
        write(pipe_fd_pair[1], "NandGate", 8);
        expect(mock_timerpoll_data_ready_cb, when(target, is_equal_to(&handle.super)),
                when(status, is_equal_to(0)));
        uv_run(loop, UV_RUN_ONCE);
        assert_that(handle.nread, is_equal_to(19));
        assert_that(&handle.rd_buf[0], is_equal_to_string("C10K" "DroneIT" "NandGate"));
    }
    {
