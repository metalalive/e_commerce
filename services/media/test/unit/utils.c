...
Ensure(app_url_decode_queryparam__simple_test) {
    const char *kv[EXPECT_NUM_ITEMS][2] = {
        {"cumin", "clove+garlic"},        {"sesame", "coriander"}, {"wasabi", NULL},
        {"chilli", "bayan=-omega===zoo"}, {"dill", "mustard"},
    ...
    snprintf(
        &raw_query_param[0], (size_t)RAW_STRING_LEN, "%s=%s&%s=%s&%s&%s=%s&%s=%s&", kv[0][0], kv[0][1],
...
Ensure(app_url_decode_queryparam__blankchar_test) {
    ...
    json_t *map = json_object();
    snprintf(
        &raw_query_param[0], (size_t)RAW_STRING_LEN,
        "%s=%s&%s=%s HTTP/1.1\r\n"
        "mock request body starts",
        kv[0][0], kv[0][1], kv[1][0], kv[1][1]
    );
    int actual_num_items = app_url_decode_query_param(&raw_query_param[0], map);
    assert_that(actual_num_items, is_equal_to(EXPECT_NUM_ITEMS));
