...
static DBA_RES_CODE mock_dbpool_3pty_init_fn(db_conn_t *conn) { return DBA_RESULT_OK; }

static DBA_RES_CODE mock_dbpool_3pty_deinit_fn(db_conn_t *conn) { return DBA_RESULT_OK; }

static uint8_t mock_dbpool__can_change_state(db_conn_t *conn) { return 1; }
...
static void mock_dbpool_3pty_state_transition(app_timer_poll_t *target, int uv_status, int event_flags) {
    ...
    if (has_row) {
        size_t rs_node_sz = sizeof(db_llnode_t) + sizeof(db_query_result_t) + sizeof(db_query_row_info_t) +
        ...
        *rs = (db_query_result_t
          ...
          .app_result = DBA_RESULT_OK,
          .conn = {.state = UTEST_DB_ASYNC_FETCH_ROW_READY, .async = 0, .alias = conn->pool->cfg.alias},
          ._final = 0};
        {
    ...
    uv_run(curr_query->cfg.loop, UV_RUN_NOWAIT);
    {
        ...
        *rs = (db_query_result_t
          ...
          .app_result = DBA_RESULT_OK,
          .conn = {.state = UTEST_DB_ASYNC_FREE_RESULTSET_DONE, .async = 0, .alias = conn->pool->cfg.alias},
          ._final = 1};
        app_db_query_notify_with_result(curr_query, rs);
    }
    uv_run(curr_query->cfg.loop, UV_RUN_NOWAIT);
...
static int mock_dbpool__get_sock_fd(db_conn_t *conn) { return -1; }

static uint64_t mock_dbpool__get_timeout_ms(db_conn_t *conn) { return 123; }

static uint8_t mock_dbpool__is_conn_closed(db_conn_t *conn) { return 0; }

static db_pool_cfg_t utest_db_pool_cfg = {
    .alias = UTEST_DBPOOL_ALIAS,
    .capacity = 1,
    .idle_timeout = 13,
    .bulk_query_limit_kb = 3,
    .conn_detail = {.db_name = "x", .db_user = "x", .db_passwd = "x", .db_host = "x", .db_port = 4567},
    .ops =
        {.conn_init_fn = mock_dbpool_3pty_init_fn,
         .conn_deinit_fn = mock_dbpool_3pty_deinit_fn,
         .state_transition = mock_dbpool_3pty_state_transition,
         .notify_query = mock_dbpool__notify_query,
         .is_conn_closed = mock_dbpool__is_conn_closed,
         .get_sock_fd = mock_dbpool__get_sock_fd,
         .get_timeout_ms = mock_dbpool__get_timeout_ms,
         .can_change_state = mock_dbpool__can_change_state}
};

static void utest__db_async_err_cb(db_query_t *q, db_query_result_t *rs) { mock(q, rs); }
...
#define UTEST_NUM_ENTRIES_HASHMAP 6
    ...
    app_db_pool_init(&utest_db_pool_cfg);
    {
        ...
        expect(
            mock_dbpool_3pty_state_transition,
