...
Ensure(api_test_upload_part__singlechunk_ok) {
    ...
    usrarg_t cb_arg = {
        .resp_code = 200, .part = EXPECT_PART, .f_chksum = CHUNK_FILE_CHKSUM, .upld_req_ref = upld_req
    };
...
Ensure(api_test_upload_part__quota_exceed) {
    ...
    usrarg_t cb_args[NUM_PARTS] = {
        {.resp_code = 200,
         .part = 7,
         .f_chksum = CHUNK_FILE_CHKSUM_1,
         .filepath = CHUNK_FILE_PATH_1,
         .upld_req_ref = upld_req},
        {.resp_code = 200,
         .part = 3,
         .f_chksum = CHUNK_FILE_CHKSUM_2,
         .filepath = CHUNK_FILE_PATH_2,
         .upld_req_ref = upld_req},
        {.resp_code = 403,
         .part = 4,
         .f_chksum = CHUNK_FILE_CHKSUM_3,
         .filepath = CHUNK_FILE_PATH_3,
         .upld_req_ref = upld_req},
    };
...
Ensure(api_test_upload_part__multichunk_outoforder) {
    ...
    json_array_foreach(files_info, idx, file_info) {
        ...
        if (!upld_req) {
            break;
        } // not have enough upload requests
        ...
        } // end of inner loop
        {
            ...
            ssize_t accumulated_sz_bytes =
                json_integer_value(json_object_get(usr_upload_quota, &usr_id_str[0]));
