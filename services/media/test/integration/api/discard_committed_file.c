...
static int  _available_resource_lookup(json_t **upld_req, const char *ftype_in)
{
    ...
    if(req && resource_id_item) {
        ...
        *upld_req = NULL;
        fprintf(stderr, "[itest][discard_committed_file] no more ressource"
                " with the type:%s \n" , ftype_in);
    }
    return idx;
...
static void  _api_test__discard_committed_file__common(itest_usrarg_t *usrarg)
{
    ...
    test_setup_pub_t  setup_data = {
        .method="DELETE", .verbose=0, .url=&url[0], .req_body = {.serial_txt=NULL, .src_filepath=NULL},
...
{ // TODO, test consistency between storage and database
