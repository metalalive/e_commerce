...
#define  ITEST_URL_PATTERN  "https://" ITEST_STREAM_HOST "/file?" API_QPARAM_LABEL__RESOURCE_ID \
...
static void itest_verify__nonstream_init (CURL *handle, test_setup_priv_t *privdata, void *_usr_arg)
...
static  void  _itest_nonstream_init__filefetch (itest_usrarg_t *usr_arg)
{
    ...
    test_setup_pub_t  setup_data = {.method = "GET", .verbose = 0,  .url = &url[0],
...
#define  RUN_CODE(__upld_req, _usr_id, __expect_resp_code) { \
    ...
    if(__upld_req) { \
        ...
        if(ver_label) { \
            itest_usrarg_t  usr_arg = {._upld_req=__upld_req, .usr_id=_usr_id, \
                ._expect_resp_code=__expect_resp_code, .version_label=ver_label }; \
...
Ensure(api_test__filenonstream_init__nonexist_res_id)
{
    ...
    itest_usrarg_t  usr_arg = {._upld_req=upld_req, .usr_id=9876,
        ._expect_resp_code=404, .version_label="ab" };
    _itest_nonstream_init__filefetch(&usr_arg);
