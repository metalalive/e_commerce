    ...
    if (usr_args->expect_resp_code == 200) {
        if (usr_args->req_body_serialtxt) {
            ...
            if (existing_acl && resp_obj) {
                json_t *item_i = NULL, *item_j = NULL;
                int     idx = 0, jdx = 0;
                json_array_foreach(resp_obj, idx, item_i) {
                    ...
                    json_array_foreach(existing_acl, jdx, item_j) {
                        ...
                        if (id0 == id1) {
                            ...
                            uint8_t transocde_flg_0 =
                                json_boolean_value(json_object_get(i0_able, "transcode"));
                            uint8_t transocde_flg_1 =
                                json_boolean_value(json_object_get(i1_able, "transcode"));
...
static void _itest_edit_usrlvl_acl__common(
    json_t *upld_req, uint32_t auth_usr_id, const char *req_body_serialtxt, int expect_resp_code
) {
    ...
    itest_usrarg_t usr_args = {
        .upld_req = upld_req, .expect_resp_code = expect_resp_code, .req_body_serialtxt = req_body_serialtxt
    };
...
#undef REQ_BODY_SERIALTXT
    ...
    { // subcase 2, usr_id = 13, granted with edit-acl permission, should be able to edit ACL of the
      // resource
...
#undef REQ_BODY_SERIALTXT
    ...
    { // subcase 3, usr_id = 51, granted with edit-acl permission, should be able to edit ACL of the
      // resource
...
Ensure(api_edit_usrlvl_acl__test_permission_denied) { // usr_id = 13, hasn't been added to ACL of the resource
    json_t *upld_req = NULL;
    _available_resource_lookup(&upld_req, "ulvl_acl", NULL);
    if (!upld_req)
        return;
...
static void _itest_edit_filelvl_acl__common(
    json_t *upld_req, uint32_t auth_usr_id, const char *req_body_serialtxt, int expect_resp_code
) {
