...
static void test_verify__app_server_response(CURL *handle, test_setup_priv_t *privdata, void *usr_arg) {
    ...
    if (actual_resp_code < 300) {
        const char *valid_resource_id =
            json_string_value(json_object_get(resp_obj, API_QPARAM_LABEL__RESOURCE_ID));
...
static void _api_commit_upload_req__success_common(
    ...
    char req_body_raw[MAX_BYTES_REQ_BODY] = {0};
    {
        ...
        size_t nwrite =
            json_dumpb((const json_t *)req_body, &req_body_raw[0], MAX_BYTES_REQ_BODY, JSON_COMPACT);
    ...
    usrarg_t cb_arg = {
        ...
        .expect_resp_code = expect_resp_code,
        .err_field = err_field,
        .err_msg = err_msg
    };
...
Ensure(api_commit_upload_req__invalid_resource_id) {
    ...
    if (upld_req)
        _api_commit_upload_req__success_common(
            upld_req, "' OR 1;'", "video", 400, API_QPARAM_LABEL__RESOURCE_ID, "invalid format"
        );
...
Ensure(api_commit_upload_req__invalid_resource_typ) {
    json_t *upld_req = json_array_get(_app_itest_active_upload_requests, 0);
    if (upld_req) {
        _api_commit_upload_req__success_common(upld_req, "abcdefg", NULL, 400, "type", "invalid");
        _api_commit_upload_req__success_common(upld_req, "hijkmnop", "PDF", 400, "type", "invalid");
    }
...
Ensure(api_commit_upload_req__nonexistent_req) {
    ...
    _api_commit_upload_req__success_common(
        upld_req_dst, EXPECT_RESOURCE_ID, "video", 400, "req_seq", "not exists"
    );
...
Ensure(api_commit_upload_req__incomplete_chunks) {
    ...
    if (upld_req) {
        _api_commit_upload_req__success_common(
            upld_req, EXPECT_RESOURCE_ID, "video", 400, "req_seq",
            "part numbers of file chunks are not adjacent"
        );
    }
...
Ensure(api_commit_upload_req__add_resources) {
    ...
    if (upld_req) {
        ...
        _api_commit_upload_req__success_common(
            upld_req, SECONDARY_RESOURCE_ID, expect_ftyp, 400, "req_seq", "not exists"
        );
...
Ensure(api_commit_upload_req__resource_id_not_allowed) {
    ...
    if (upld_req) {
        ...
        if (!expect_ftyp)
            return;
        _api_commit_upload_req__success_common(
            upld_req, EXPECT_RESOURCE_ID, expect_ftyp, 403, API_QPARAM_LABEL__RESOURCE_ID,
...
Ensure(api_commit_upload_req__use_existing_resource_id) {
    ...
    if (upld_req_0 && upld_req_1) {
        ...
        _api_commit_upload_req__success_common(
            upld_req_0, EXPECT_RESOURCE_ID, expect_req0_ftyp, 200, NULL, NULL
        );
        // at this point, req-seq at idx=2 is rollbacked to `uncommitted state`, so it can be
        // committed
        //  again with different `resource ID`
        _api_commit_upload_req__success_common(
            upld_req_1, SECONDARY_RESOURCE_ID, expect_req1_ftyp, 201, NULL, NULL
        );
    }
...
Ensure(api_commit_upload_req__add_resources_2) {
    ...
    for (int idx = num_upld_reqs_preserved; idx < num_upld_reqs_total; idx++) {
        ...
        if (!expect_ftyp) {
            fprintf(
                stderr,
                "[itest][complete_multipart_upload] line:%d, upload request,"
        ...
        _api_commit_upload_req__success_common(
            upld_req, mock_res_id, expect_ftyp, _expect_resp_code, NULL, NULL
        );
