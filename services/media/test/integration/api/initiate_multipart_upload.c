...
static void
test_verify__initiate_multipart_upload_ok(CURL *handle, test_setup_priv_t *privdata, void *usr_arg) {
    CURLcode res;
    ...
    if (resp_obj) { // should return short-term token for upload request
        ...
        case 201: {
            ...
            if (req_seq > 0 && usr_id > 0) {
                ...
                json_object_set_new(
                    item, "type", json_null()
                ); // file type, will be referenced when transcoding files
...
TestSuite *api_initiate_multipart_upload_tests(json_t *root_cfg) {
    ...
    if (file_list && json_is_array(file_list)) {
        ...
        if (_itest_num_original_files < required)
            fprintf(
                stderr, "[itest][api][init_upld_req] line:%d, required:%d, _itest_num_original_files:%d \n",
                __LINE__, required, _itest_num_original_files
            );
    } else {
        fprintf(
            stderr, "[itest][api][init_upld_req] line:%d, no test file list in config object \n", __LINE__
        );
    }
