...
class NginxConfigGenerator:
    def start(self, argv:list):
        ...
        renderred = self._render_entry(template=env.get_template('entry.conf'), uname=uname, usrgrp=usrgrp,
            cdn_cfg_root=cdn_cfg_root, ups_param_gen=upstream_param_gen, curr_work_path=curr_work_path )
        self.write_renderred_to_file(dst_path=cfg_dst_path, wdata=renderred,
                filename='entry.conf')
        renderred = self.render_location_common(bkd_cfg_path=backends_cfg_path[0],
                template=env.get_template('loc_proxy_common.conf'), curr_work_path=curr_work_path )
        self.write_renderred_to_file(dst_path=cfg_dst_path, wdata=renderred,
                filename='loc_proxy_common.conf')
        print('rendering Nginx config file ...... done')

    def _render_entry(self, template, cdn_cfg_root:dict, curr_work_path:PyPath, uname:str,
            usrgrp:str, ups_param_gen:TyGenerator):
        ...
        kwargs = {'os_username':uname, 'os_user_group':usrgrp, 'logging_level':pxycfg['logging_level'],
                'max_num_conns':pxycfg['max_num_conns'], 'reqs_per_sec':qps, 'qps_minus1':qps - 1,
                'pxy_cch_basepath':cache_basepath, 'pxy_cch_inactive_mins':cache_inactive_minutes,
                'pxy_cch_max_space_mb':cache_sz_mb, 'pxy_cch_lock_timeout':cacheentry_lock_timeout,
                'pxy_srv_port':pxycfg['port'], 'pxy_srv_hostname':pxycfg['host'],
    ...
    def _generate_upstream_server(self, bkd_cfg_paths:list):
        ups_node_cls = type('upstream_node_class', (object,), {'hostname':None,'port':0,'max_conns':0,
            'max_fails':0,'retry_after_unavail_secs':0})
    ...
    def render_location_common(self, template, bkd_cfg_path:str, curr_work_path:PyPath):
        ...
        if bkd_cfg_root is not None:
            ...
            kwargs = {'pxy_upstream_ssl_ca_path':origsrv_ca_fullpath,
...
class NginxPathSetup:
    def start(self, argv:list):
        ...
        with open(cdn_cfg_path, 'r') as f:
            cdn_cfg_root = json.load(f)
        self._copypath(curr_work_path, ngx_install_path=ngx_install_path,
    ...
    def _copypath(self, curr_work_path:PyPath, ngx_install_path:PyPath, ngx_genfile_path:str):
        ...
        assert ngx_genfile_path.exists() and ngx_genfile_path.is_dir(), "ngx_genfile_path not exists"
