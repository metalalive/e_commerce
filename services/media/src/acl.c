...
static void appacl_db_common_error_cb(db_query_t *q, db_query_result_t *rs) {
    ...
    fprintf(
        stderr, "[acl] line:%d, error, resource ID:%s, user ID:%u \n", __LINE__, ctx->resource_id, ctx->usr_id
    );
    appacl_db_common_trigger_callback(ctx, usr_args);
}
...
static void appacl_loaddb_row_fetched(db_query_t *q, db_query_result_t *rs) {
    ...
    _result->data.entries[curr_num_rows++] = (aacl_data_t
    ){.usr_id = _usr_id, .capability = {.transcode = _transcode_enable, .edit_acl = _edit_acl_enable}};
...
#define SQL_PATTERN_1 \
    "EXECUTE IMMEDIATE 'SELECT `usr_id`,`transcode_flg`,`edit_acl_flg` FROM" \
    " `" USRLVL_ACL_TABLE "`  WHERE `file_id`=?' USING " SQL_BASE64_ENCODED_RESOURCE_ID ";"
#define SQL_PATTERN_2 \
    "EXECUTE IMMEDIATE 'SELECT `usr_id`,`transcode_flg`,`edit_acl_flg` FROM" \
    " `" USRLVL_ACL_TABLE "`  WHERE `file_id`=? AND `usr_id`=?' USING " SQL_BASE64_ENCODED_RESOURCE_ID \
    ", %u;"
...
#undef SQL_PATTERN_2
    ...
    memcpy(
        &db_async_usr_args[ctx->_num_internal_args], cfg->usr_args.entries,
        sizeof(void *) * ctx->_num_usr_args
    );
    db_query_cfg_t db_cfg = {
        ...
        .callbacks =
            {.result_rdy = appacl_db_dummy_cb,
             .row_fetched = appacl_loaddb_row_fetched,
             .result_free = appacl_loaddb_resultset_dealloc,
             .error = appacl_db_common_error_cb},
...
int app_acl_verify_resource_id(aacl_cfg_t *cfg) {
    ...
    if (!cfg || !cfg->resource_id || !cfg->callback)
        return err;
#define SQL1_PATTERN \
    "EXECUTE IMMEDIATE 'SELECT `usr_id`,HEX(`last_upld_req`),`type` FROM `uploaded_file` WHERE " \
    "`id`=?' USING FROM_BASE64('%s');"
#define SQL2_PATTERN \
    "EXECUTE IMMEDIATE 'SELECT `uf`.`usr_id`, " \
    "HEX(`uf`.`last_upld_req`),`uf`.`type`,`fac`.`visible_flg`" \
    " FROM `uploaded_file` AS `uf` LEFT JOIN `" FILELVL_ACL_TABLE "` AS `fac` ON `uf`.`id`=`fac`.`file_id`" \
    " WHERE `uf`.`id`=?' USING FROM_BASE64('%s');"
    size_t raw_sql_sz =
        strlen(cfg->resource_id) + (cfg->fetch_acl ? sizeof(SQL2_PATTERN) : sizeof(SQL1_PATTERN));
...
void app_acl__build_update_lists(
    aacl_result_t *existing_data, json_t *new_data, aacl_data_t **data_update, size_t *num_update,
    aacl_data_t **data_delete, size_t *num_deletion, aacl_data_t *data_insert, size_t *num_insertion
) {
    ...
    for (idx = 0; idx < existing_data->data.size; idx++) {
        ...
        json_array_foreach(new_data, jdx, new_item) {
            ...
            if (curr_usr_id == origin->usr_id) {
                ...
                origin->capability.edit_acl =
                    (uint8_t)json_boolean_value(json_object_get(new_detail, "edit_acl"));
                origin->capability.transcode =
                    (uint8_t)json_boolean_value(json_object_get(new_detail, "transcode"));
...
#define SAVE_ACL__RUN_SQL__COMMON_CODE \
    ...
    db_query_cfg_t db_cfg = { \
        ...
        .usr_data = {.entry = (void **)&db_async_usr_args[0], .len = tot_num_usr_args}, \
        .callbacks = \
            {.result_rdy = appacl_db_write_done_cb, \
             .row_fetched = appacl_db_dummy_cb, \
             .result_free = appacl_db_dummy_cb, \
             .error = appacl_db_common_error_cb}, \
...
#define SQL_EXEC_INSERT \
    "EXECUTE `" PREP_STMT_LABEL_INSERT "` USING %1u,%1u," SQL_BASE64_ENCODED_RESOURCE_ID ",%u;"
#define SQL_EXEC_UPDATE \
    "EXECUTE `" PREP_STMT_LABEL_UPDATE "` USING %1u,%1u," SQL_BASE64_ENCODED_RESOURCE_ID ",%u;"
#define SQL_EXEC_DELETE "EXECUTE `" PREP_STMT_LABEL_DELETE "` USING " SQL_BASE64_ENCODED_RESOURCE_ID ",%u;"
#define FINAL_SQL_PATTERN \
    "BEGIN NOT ATOMIC" \
    "  PREPARE `" PREP_STMT_LABEL_INSERT "` FROM 'INSERT INTO `" USRLVL_ACL_TABLE \
    "`(`transcode_flg`,`edit_acl_flg`,`file_id`,`usr_id`) VALUES (?,?,?,?)';" \
    "  PREPARE `" PREP_STMT_LABEL_UPDATE "` FROM 'UPDATE `" USRLVL_ACL_TABLE \
    "` SET `transcode_flg`=?,`edit_acl_flg`=? WHERE `file_id`=? AND `usr_id`=?';" \
    "  PREPARE `" PREP_STMT_LABEL_DELETE "` FROM 'DELETE FROM `" USRLVL_ACL_TABLE \
    "` WHERE `file_id`=? AND `usr_id`=?';" \
    "  START TRANSACTION;" \
    "    %s %s %s" \
    "  COMMIT;" \
    "END;"
    ...
    app_acl__build_update_lists(
        existing_data, new_data, &data_update[0], &num_update, &data_delete[0], &num_deletion,
        &data_insert[0], &num_insertion
    );
    ...
    size_t final_sql_sz =
        sizeof(FINAL_SQL_PATTERN) + delete_sql_tot_sz + update_sql_tot_sz + insert_sql_tot_sz;
    ...
    { // start sql queries construction, TODO: verify whether the SQL statement is corrupted
        ...
        if (num_insertion > 0) {
            ...
            for (idx = 0; idx < num_insertion; idx++) {
                ...
                nwrite = snprintf(
                    ptr, avail_buf_sz, SQL_EXEC_INSERT, data->capability.transcode, data->capability.edit_acl,
                    cfg->resource_id, data->usr_id
                );
        ...
        if (num_update > 0) {
            ...
            for (idx = 0; idx < num_update; idx++) {
                ...
                nwrite = snprintf(
                    ptr, avail_buf_sz, SQL_EXEC_UPDATE, data->capability.transcode, data->capability.edit_acl,
                    cfg->resource_id, data->usr_id
                );
                assert(nwrite < avail_buf_sz);
                ptr += nwrite;
        ...
        size_t tot_nwrite = snprintf(
            &final_sql[0], final_sql_sz, FINAL_SQL_PATTERN, &delete_sqls[0], &update_sqls[0], &insert_sqls[0]
        );
...
#define SQL_INSERT_PATTERN \
    "EXECUTE IMMEDIATE 'INSERT INTO `" FILELVL_ACL_TABLE "`(`visible_flg`,`file_id`) VALUES(?,?)'" \
    " USING %1u," SQL_BASE64_ENCODED_RESOURCE_ID ";"
#define SQL_UPDATE_PATTERN \
    "EXECUTE IMMEDIATE 'UPDATE `" FILELVL_ACL_TABLE "` SET `visible_flg`=? WHERE `file_id`=?'" \
    " USING %1u," SQL_BASE64_ENCODED_RESOURCE_ID ";"
