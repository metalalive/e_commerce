...
int app_url_decode_query_param(char *data, json_t *map) {
    ...
    size_t num_items = 0;
    for (tok = strtok_r(tok, "&", &ptr_kvpair); tok;
         tok = strtok_r(NULL, "&", &ptr_kvpair)) { // strtok_r is thread-safe
...
int app_hexstr_to_chararray(
    char *out, size_t out_sz, const char *in, size_t in_sz
