...
uint8_t app_timer_poll_is_closing(app_timer_poll_t *handle) {
    if (!handle) {
        ...
        return uv_is_closing((uv_handle_t *)&handle->poll) && uv_is_closing((uv_handle_t *)&handle->timeout);
    }
...
int app_timer_poll_start(
    ...
    int ret = 0;
    if (!poll_cb || !handle || !handle->timeout.loop || !handle->poll.loop || timeout_ms == 0 ||
        events == 0) {
