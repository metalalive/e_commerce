...
RESTAPI_ENDPOINT_HANDLER(fetch_file_streaming_element, GET, self, req) {
    ...
    if (!ret)
        json_object_set_new(
            err_info, API_QPARAM_LABEL__STREAM_DOC_ID, json_string("contains invalid character")
        );
    ret = app_verify_printable_string(detail, detail_sz);
    if (ret || strstr(detail, "../")) // prevent users from switching to parent folder
        json_object_set_new(
            err_info, API_QPARAM_LABEL__DOC_DETAIL, json_string("contains invalid character")
        );
    if (json_object_size(err_info) > 0)
        goto done;
...
#undef PATTERN
    ...
    ret = api_filefetch_start_caching(
        req, self, node, spec, err_info, atfp_streamcache_init, atfp_streamcache_proceed_datablock
    );
done:
    if (ret != 0) {
