...
static void _api_edit_usrlvl_acl__deinit_primitives(
...
static void _api_edit_usrlvl_acl__deinit_usrdata(api_usr_data_t *udata) {
    ...
    _api_edit_usrlvl_acl__deinit_primitives(
        udata->req, udata->hdlr, udata->node, udata->spec, udata->err_info
    );
    free(udata);
...
static void _api_load_saved_acl__done_cb(aacl_result_t *result, void **usr_args) {
    ...
    if (result->flag.error) {
        ...
        aacl_cfg_t aclcfg = {
            .usr_args = {.entries = &usr_args[0], .size = 1},
            .db_pool = app_db_pool_get_pool("db_server_1"),
            .resource_id = res_id_encoded,
            .loop = usrdata->req->conn->ctx->loop,
...
static __attribute__((optimize("O0"))) int _api_edit_acl_verify_otherusers_exist(api_usr_data_t *usrdata) {
    ...
    if (num_items_req == num_items_rpc) {
        ...
        if (ret) {
            h2o_error_printf(
                "[api][edit_acl] line:%d, unexpected result on user ID comparison: %d \n", __LINE__, ret
            );
        ...
        if (_usrdata->rpc_returned_usrprofs) { \
            ...
            if (!_err) { \
                ...
                aacl_cfg_t aclcfg = { \
                    ...
                    .resource_id = res_id_encoded, \
                    .db_pool = app_db_pool_get_pool("db_server_1"), \
                    .loop = _usrdata->req->conn->ctx->loop, \
...
static uint8_t
_api_verify_otherusers_exist__update_cb(arpc_reply_cfg_t *cfg, json_t *info, ARPC_STATUS_CODE result) {
...
static ARPC_STATUS_CODE _api_rpc__start_verify_usr_ids(
    void *mq_conn, json_t *spec, uint32_t curr_usr_id
...
#undef PY_CELERY_MSGBODY
    ...
    json_array_foreach(req_body, idx, item) { json_array_append(msg_ids, json_object_get(item, "usr_id")); }
...
#undef UUID_STR_SZ
    ...
    arpc_exe_arg_t rpc_arg = {
        ...
        .msg_body = {.len = msg_nb_required, .bytes = &msg_body_raw[0]},
        .routing_key = "rpc.user_management.get_profile",
        .usr_data = (void *)spec,
        .headers = {.size = 2, .entries = &_extra_headers[0]}
    };
    json_decref(msg_body);
    return app_rpc_start(&rpc_arg);
} // end of  _api_rpc__start_verify_usr_ids

static void api_edit_acl__verify_otherusers_exist(
    h2o_handler_t *hdlr, h2o_req_t *req, app_middleware_node_t *node, json_t *spec, json_t *err_info
    ...
    if (result != APPRPC_RESP_ACCEPTED) {
        ...
        *usrdata =
        ...
        arpc_reply_cfg_t rpc_cfg = {
            ...
            .timeout_ms = TIMER_EVENT_INTERVAL_MS,
            .on_error = _api_verify_otherusers_exist__err_cb,
...
static void _api_abac_pdp__try_match_rule(
    aacl_result_t *result, void **usr_args
...
static void _api_abac_pdp__verify_resource_owner(
    aacl_result_t *result, void **usr_args
