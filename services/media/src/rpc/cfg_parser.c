...
static int parse_cfg_rpc__broker_credential(json_t *in, arpc_cfg_t *out) {
    ...
    json_array_foreach(hierarchy, idx, hier_tag) {
        if (json_is_string(hier_tag)) {
            ...
            h2o_error_printf(
                "[parsing] invalid hierarchy in the AMQP-broker credential file : %s \n", filepath
            );
            goto error;
        }
        if (!dst) {
            h2o_error_printf(
                "[parsing] invalid json object in the AMQP-broker credential file : %s \n", filepath
            );
            goto error;
        }
    } // end of loop
...
static int parse_cfg_rpc__broker_bindings(json_t *objs, arpc_cfg_t *cfg) {
    ...
    json_array_foreach(objs, idx, obj) {
        ...
        if (!queue || !exchange || !routing_key) {
            h2o_error_printf(
                "[parsing] missing binding parameters (idx=%lu) in message broker configuration "
                "(%s:%hu) \n",
        ...
        if (max_msgs_pending > RPC_QUEUE_MAX_NUM_MSGS) {
            h2o_error_printf(
                "[parsing] max_msgs_pending (%lu) exceeds system limit (%d) in queue (%s), binding "
                "configuration (idx=%lu)\n",
        ...
        *bcfg = (arpc_cfg_bind_t
        ){.q_name = strdup(queue),
          .exchange_name = strdup(exchange),
          .routing_key = strdup(routing_key),
          .max_msgs_pending = max_msgs_pending,
...
static int parse_cfg_rpc_common(json_t *objs, app_cfg_t *app_cfg) {
    ...
    json_array_foreach(objs, idx, obj) {
        ...
        if (!credential || !attributes || !alias) {
            h2o_error_printf(
                "[parsing] missing credential or attributes message broker configuration "
                "(idx=%lu)\n",
                idx
            );
            goto error;
        ...
        if (!json_is_object(credential) || !json_is_object(attributes)) {
            h2o_error_printf(
                "[parsing] invalid format in credential or attributes of message broker "
                "configuration (idx=%lu)\n",
                idx
            );
            goto error;
...
#define PARSE_CFG_RPC__COMMON_SANITY(objs, app_cfg) \
    { \
        ...
        json_array_foreach(objs, idx, obj) { \
            ...
            if (!bindings) { \
                h2o_error_printf( \
                    "[parsing] missing bindings in message broker configuration (idx=%lu)\n", idx \
                ); \
                goto error; \
            } \
            if (!json_is_array(bindings) || json_array_size(bindings) == 0) { \
                h2o_error_printf( \
                    "[parsing] invalid format in binding field of message broker configuration " \
                    "(idx=%lu)\n", \
                    idx \
                ); \
                goto error; \
