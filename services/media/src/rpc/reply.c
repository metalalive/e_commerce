...
void apprpc_reply_deinit_start(void *ctx) { _apprpc_reply_deinit_start((_arpc_reply_ctx_t *)ctx); }
...
static void _apprpc_replyq_identify_corr_id(arpc_cfg_t *cfg, arpc_exe_arg_t *arg) {
    if (arg->job_id.len == 0 || !arg->job_id.bytes)
        return; // discard due to lack of job ID
                // NOTE, in this app, the prefix of name pattern of a reply queue has to be identifiable by
                // each user the prefix of name pattern of correlation ID has to be identifiable by each RPC
                // function
...
void *apprpc_recv_reply_restart(void *in_ctx) {
    ...
    int                ret = uv_timer_start(
        &_rpc_ctx->timer, _apprpc_reply_update_timeout_cb, _rpc_ctx->super.timeout_ms, TIMER_DONT_REPEAT
    );
    if (ret != 0) {
        _apprpc_reply_deinit_start(_rpc_ctx);
        in_ctx = NULL;
