...
static ARPC_STATUS_CODE apprpc__translate_status_from_lowlvl_lib(amqp_rpc_reply_t *reply) {
    ...
    case AMQP_RESPONSE_SERVER_EXCEPTION:
        ...
        case AMQP_CHANNEL_CLOSE_METHOD: // see detail usage of API  amqp_get_rpc_reply(...) in
                                        // rabbitmq-c
        {
...
static amqp_status_enum apprpc_msgq_conn_auth(struct arpc_ctx_t *item
    ...
    // given connection is active, some of primary API functions will return
    // AMQP_STATUS_HEARTBEAT_TIMEOUT in case that the connection is inactive (closed) before
    // invoking this function
    {
        int              max_nbytes_per_frame = (int)cfg->attributes.max_kb_per_frame << 10;
        amqp_rpc_reply_t _reply = amqp_login(
            item->conn, cfg->attributes.vhost, cfg->attributes.max_channels, max_nbytes_per_frame,
            cfg->attributes.timeout_secs, AMQP_SASL_METHOD_PLAIN, cfg->credential.username,
            cfg->credential.password
        );
        if (_reply.reply_type != AMQP_RESPONSE_NORMAL) {
            fprintf(
                stderr, "[RPC] authentication failure %s@%s:%hu \n", cfg->credential.username,
                cfg->credential.host, cfg->credential.port
            );
    ...
    if (!chn_res || !chn_res->channel_id.bytes) {
        fprintf(
            stderr, "[RPC] line:%d, failed to open default channel %s:%hu \n", __LINE__, cfg->credential.host,
            cfg->credential.port
        );
...
static void apprpc_declare_q_report_error(
    amqp_rpc_reply_t *reply, char *q_name
    ...
    if (reply->reply_type == AMQP_RESPONSE_SERVER_EXCEPTION) {
        ...
        amqp_channel_close_t *m = (amqp_channel_close_t *)reply->reply.decoded;
        fprintf(
            stderr, ", reason: server channel error %hu, message: %.*s ", m->reply_code,
...
static void apprpc_ensure_send_queue(struct arpc_ctx_t *item) {
    ...
    for (idx = 0; idx < cfg->bindings.size; idx++) {
        ...
        if (bind_cfg->skip_declare) {
            fprintf(
                stderr,
                "[RPC][core] this app should NOT declare the queue:%s"
                " where the other app will consume the message from \n",
                bind_cfg->q_name
            );
            continue;
        }
        // AMQP broker does NOT allow  unsigned number as argument when declaring a queue ? find out
        // the source(TODO)
        amqp_table_entry_t q_arg_n_elms = {
            .key = amqp_cstring_bytes("x-max-length"),
        ...
        amqp_queue_declare(
            item->conn, item->curr_channel_id, amqp_cstring_bytes(bind_cfg->q_name),
            (amqp_boolean_t)bind_cfg->flags.passive, (amqp_boolean_t)bind_cfg->flags.durable,
            (amqp_boolean_t)bind_cfg->flags.exclusive, (amqp_boolean_t)bind_cfg->flags.auto_delete,
            q_arg_table
        );
        ...
        if (_reply.reply_type != AMQP_RESPONSE_NORMAL) {
            fprintf(
                stderr, "[RPC] fail to bind the routing key (%s) with the queue (%s) \n",
...
static ARPC_STATUS_CODE
apprpc_ensure_reply_queue(struct arpc_ctx_t *_ctx, arpc_cfg_bind_reply_t *reply_cfg, char *q_name) {
    // TODO:
    ...
    amqp_queue_declare(
        _ctx->conn, _ctx->curr_channel_id, amqp_cstring_bytes(q_name),
        (amqp_boolean_t)reply_cfg->flags.passive, (amqp_boolean_t)reply_cfg->flags.durable,
        (amqp_boolean_t)reply_cfg->flags.exclusive, (amqp_boolean_t)reply_cfg->flags.auto_delete,
        amqp_empty_table
    );
...
#define RPC_MAX_REPLY_QNAME_SZ 128
    ...
    ARPC_MSGQ__ENSURE_RPC_REPLY_PARAM(
        app_status, &bind_cfg->reply, queue, args, &reply_req_queue[0], RPC_MAX_REPLY_QNAME_SZ
    );
    if (app_status != APPRPC_RESP_OK)
        goto done;
    // job_id here should be the same as correlation ID within the message
    ARPC_MSGQ__ENSURE_RPC_REPLY_PARAM(
        app_status, &bind_cfg->reply, correlation_id, args, args->job_id.bytes, args->job_id.len
    );
    if (app_status != APPRPC_RESP_OK)
        goto done;
    ...
    if (args->headers.size > 0 && args->headers.entries) {
        ...
        for (uint8_t idx = 0; idx < args->headers.size; idx++) {
            ...
            extra_headers[idx] = (amqp_table_entry_t
            ){.value =
    ...
    amqp_basic_properties_t properties = {
        ._flags = AMQP_BASIC_CONTENT_TYPE_FLAG | AMQP_BASIC_DELIVERY_MODE_FLAG |
                  AMQP_BASIC_CORRELATION_ID_FLAG | AMQP_BASIC_REPLY_TO_FLAG | AMQP_BASIC_TIMESTAMP_FLAG |
                  AMQP_BASIC_CONTENT_ENCODING_FLAG | header_enable_flag,
        .content_encoding = amqp_cstring_bytes("utf-8"),
        ...
        .correlation_id = {.bytes = args->job_id.bytes, .len = _job_id_fit_sz},
        .timestamp = args->_timestamp,
        .delivery_mode = 0x2, // means persistent, explanation in RabbitMQ AMQP 0.9.1 complete guide
    };
    amqp_status_enum mq_status = amqp_basic_publish(
        mq_ctx->conn, mq_ctx->curr_channel_id, amqp_cstring_bytes(bind_cfg->exchange_name), routekey,
...
static ARPC_STATUS_CODE apprpc_consumer_set_read_queues(struct arpc_ctx_t *_ctx) {
    ...
    for (idx = 0; idx < rpc_cfg->bindings.size; idx++) {
        ...
        if (bindcfg->skip_declare) {
            fprintf(
                stderr, "[RPC][core] this app should NOT consume message from the queue:%s \n",
                bindcfg->q_name
            );
            continue;
        }
        ...
        amqp_basic_consume(
            _ctx->conn, _ctx->curr_channel_id, queue, amqp_empty_bytes, no_local, no_ack, exclusive,
            amqp_empty_table
        );
...
static ARPC_STATUS_CODE _apprpc_consume_handler__send2reply_q(arpc_receipt_t *r, char *out, size_t out_sz) {
    ...
    amqp_basic_properties_t properties = {
        ...
        .correlation_id = {.bytes = r->job_id.bytes, .len = r->job_id.len},
        .delivery_mode = 0x2, // magic number
...
#define RUN_PUBLISH_CMD \
    amqp_basic_publish( \
        _ctx->conn, _ctx->curr_channel_id, AMQP_ANON_EXCHANGE, evp->message.properties.reply_to, mandatory, \
        immediate, (amqp_basic_properties_t const *)&properties, body \
    );
    ...
    if (mq_status == AMQP_STATUS_OK) {
        ...
        default:
            fprintf(
                stderr, "[RPC][consumer] unclassified error (%d) when returning value to reply queue\n",
                app_status
            );
            break;
...
ARPC_STATUS_CODE
app_rpc_consume_message(void *ctx, void *loop) { // consume one message at a time in non-blocking manner
    ARPC_STATUS_CODE   res = APPRPC_RESP_OK;
    ...
    if (!bind_cfg) {
        fprintf(
            stderr, "[RPC consumer] unknown routing key (%s) within the RPC message\n",
    ...
    if (entry_fn) {
        ...
        *r = (arpc_receipt_t){
            .ctx = ctx,
            .loop = loop,
            .return_fn = apprpc_consume_handler_finalize,
            .send_fn = _apprpc_consume_handler__send2reply_q,
            ._msg_obj = malloc(sizeof(amqp_envelope_t)),
            .routing_key = envelope.routing_key.bytes,
            .job_id = {.len = corr_id->len, .bytes = corr_id->bytes},
            ._timestamp = (uint64_t)time(NULL),
        ...
        *(amqp_envelope_t *)r->_msg_obj = envelope;
        ...
        fprintf(
            stderr, "[RPC consumer] missing task handler (%s) that wasn't found at parsing phase \n",
            (char *)envelope.routing_key.bytes
        );
...
ARPC_STATUS_CODE
app_rpc_fetch_replies(arpc_exe_arg_t *args, size_t max_nread, arpc_reply_corr_identify_fn cb) {
#define RPC_MAX_REPLY_QNAME_SZ 128
    ...
    for (int idx = 0; (max_nread > 0) && (idx < mq_ctx->ref_cfg->bindings.size); idx++) {
        ...
        ARPC_MSGQ__ENSURE_RPC_REPLY_PARAM(
            res, &bind_cfg->reply, queue, args, &reply_q_name[0], RPC_MAX_REPLY_QNAME_SZ
        );
        if (res != APPRPC_RESP_OK)
            continue; // -------------
...
#define CMD(_res) \
    { \
        amqp_basic_consume( \
            mq_ctx->conn, mq_ctx->curr_channel_id, _queue, _con_tag, no_local, no_ack, exclusive, \
            amqp_empty_table \
        ); \
...
#undef CMD
        if (res == APPRPC_RESP_OK) {
            ...
            if (channel_closed) { // RPC reply queue for current user does not exist
                ...
                if (!chn_res || !chn_res->channel_id.bytes) {
                    fprintf(
                        stderr, "[RPC] line:%d, failed to open channel:%hu \n", __LINE__,
                        mq_ctx->curr_channel_id
                    );
        ...
        do {
            ...
            reply = amqp_consume_message(
                mq_ctx->conn, &envelope, (const struct timeval *)&timeout, 0
            ); // non-blocking
            res = apprpc__translate_status_from_lowlvl_lib(&reply);
            if (res == APPRPC_RESP_OK) {
                amqp_bytes_t *corr_id = &envelope.message.properties.correlation_id;
