...
int atfp_encrypt_document_id(
    EVP_CIPHER_CTX *ctx, atfp_data_t *fp_data, json_t *kitem, unsigned char **out, size_t *out_sz
) { // the RFC8216 only accpet aes-128-cbc
    int            success = 0;
    unsigned char *ciphertext = NULL, *key_buf = NULL, *iv_buf = NULL;
    if (!fp_data || !kitem || !out || !out_sz || fp_data->usr_id == 0 || fp_data->upld_req_id == 0)
        goto done;
        ...
        goto done;
    {
        ...
        success = EVP_EncryptInit_ex(
            ctx, NULL, NULL, (const unsigned char *)&key_buf[0], (const unsigned char *)&iv_buf[0]
        );
        if (!success)
            goto done;
    }
...
#define PATTERN "%d/%08x"
        ...
        size_t pt_sz = snprintf(&plaintext[0], pre_cal_pt_sz, PATTERN, fp_data->usr_id, fp_data->upld_req_id);
        ...
        success =
            EVP_EncryptUpdate(ctx, ciphertext, &num_encrypt, (const unsigned char *)&plaintext[0], pt_sz);
        if (!success)
            goto done;
#undef PATTERN
