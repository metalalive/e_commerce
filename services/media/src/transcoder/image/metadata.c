...
static void atfp_image__dst_update_metadata__db_async_err(db_query_t *target, db_query_result_t *detail) {
    ...
    json_object_set_new(
        processor->data.error, "model", json_string("[image] failed to update metadata of output file")
    );
    fprintf(
        stderr, "[transcoder][image][metadata] line:%d, job_id:%s, unknown db error \n", __LINE__,
        processor->data.rpc_receipt->job_id.bytes
    );
...
#define SQL_PATTERN_INSERT \
    "BEGIN NOT ATOMIC" \
    "  PREPARE `" PREP_STMT_LABEL_INSERT "` FROM 'INSERT INTO `" DB_TABLE_NAME \
    "`(`file_id`,`version`,`size_bytes`" \
...
#define SQL_PATTERN_UPDATE \
    ...
    "    ,`scale_w`=?,`crop_h`=?,`crop_w`=?,`crop_x`=?,`crop_y`=?,`mask_patt`=? WHERE " \
    "`file_id`=? AND `version`=?';" \
...
void atfp_image__dst_update_metadata(atfp_t *processor, void *loop) {
    ...
    size_t  raw_sql_sz =
        UINT32_STR_SIZE + UINT16_STR_SIZE * 6 + MAX_SZ_MSK_PATT_LABEL + version_sz + res_id_encoded_sz;
    raw_sql_sz += (_is_update) ? sizeof(SQL_PATTERN_UPDATE) : sizeof(SQL_PATTERN_INSERT);
    char   raw_sql[raw_sql_sz];
    size_t nb_rawsql_used = 0;
    if (_is_update) {
        nb_rawsql_used = snprintf(
            &raw_sql[0], raw_sql_sz, SQL_PATTERN_UPDATE, total_nbytes, scale_height, scale_width, crop_height,
            crop_width, crop_x, crop_y, msk_patt_label, res_id_encoded, _version
        );
    } else {
        nb_rawsql_used = snprintf(
            &raw_sql[0], raw_sql_sz, SQL_PATTERN_INSERT, res_id_encoded, _version, total_nbytes, scale_height,
            scale_width, crop_height, crop_width, crop_x, crop_y, msk_patt_label
        );
    }
    ...
    if (db_result != DBA_RESULT_OK) {
        ...
        fprintf(
            stderr, "[transcoder][video][metadata] line:%d, job_id:%s, db_result:%d \n", __LINE__,
            processor->data.rpc_receipt->job_id.bytes, db_result
        );
    }
} // end of  atfp_image__dst_update_metadata

const char *atfp_image__metadata_dbtable_name(void) { return DB_TABLE_NAME; }
