...
static void _atfp_img_src__avctx_init_decoder(atfp_av_ctx_t *_avctx, json_t *err_info) {
    ...
    if (_fmt_ctx->nb_streams != 1) {
        ...
        fprintf(
            stderr,
            "[atfp][img][ff-in] line:%d, currently this processor only support input"
    ...
    if (ret < 0) {
        ...
        if (!decoder) {
            json_object_set_new(
                err_info, "transcoder", json_string("[ff_in] failed to find decoder for the stream")
            );
            json_object_set_new(err_info, "err_code", json_integer(ret));
            return;
        }
        ...
        if (!codec_ctx) {
            json_object_set_new(
                err_info, "transcoder", json_string("[ff_in] failed to create decoder context of the stream")
            );
            return;
        }
        ...
        if (ret < 0) {
            json_object_set_new(
                err_info, "transcoder",
                json_string("[ff_in] failed to copy parameters from stream to decoder context")
            );
            json_object_set_new(err_info, "err_code", json_integer(ret));
            return;
        }
        ...
        if (ret < 0) {
            json_object_set_new(
                err_info, "transcoder", json_string("[ff_in] failed to open decoder for stream")
            );
            json_object_set_new(err_info, "err_code", json_integer(ret));
        }
    }
...
int atfp__image_src__avctx_fetch_next_packet(atfp_av_ctx_t *_avctx) {
    ...
    if (ret == AVERROR_EOF) {
        ...
        ret = (pkt->stream_index < 0) || (is_corrupted != 0) || (pkt->stream_index >= fmt_ctx->nb_streams);
        if (ret)
...
int atfp__image_src__avctx_decode_curr_packet(atfp_av_ctx_t *_avctx) {
    ...
    if (ret < 0) {
        av_log(
            NULL, AV_LOG_ERROR,
            "[atfp][img][ff-in][av-ctx] line:%d, Failed to send packet to"
        ...
        if (ret == ATFP_AVCTX_RET__OK) {
            ...
            av_log(
                NULL, AV_LOG_ERROR,
                "[atfp][img][ff-in][av-ctx] line:%d, Failed to get decoded"
