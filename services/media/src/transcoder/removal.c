...
static void _atfp_discard__remove_version_done(atfp_t *processor) {
    ...
    if (json_object_size(err_info) == 0) {
        ...
        fprintf(
            stderr,
            "[atfp][removal] line:%d, failed to remove version in remote storage, status:%s"
...
static void
_atfp_discard__status_gather_versions(asa_op_base_cfg_t *asa_remote, json_t *err_info, json_t *spec) {
    size_t  idx = 0;
    ...
    for (idx = 0; idx < num_files; idx++) {
        ...
        if (result == ASTORAGE_RESULT_COMPLETE) {
            ...
            fprintf(
                stderr,
                "[atfp][removal] line:%d, result:%d, status:%s, version-name:%s"
                ", version-type:%d \n",
                __LINE__, result, _curr_status, entry.name, entry.type
            );
            json_object_set_new(
                err_info, "transcode",
                json_string("[storage] failed"
