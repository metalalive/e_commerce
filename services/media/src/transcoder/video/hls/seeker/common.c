...
void atfp_hls_stream_seeker__init_common(
    atfp_hls_t *hlsproc, ASA_RES_CODE (*usr_asa_cmd)(asa_op_base_cfg_t *, atfp_t *)
) {
    ...
    if (!usr_asa_cmd || !err_info || !spec || _usr_id == 0 || _upld_req_id == 0) {
        ...
        fprintf(
            stderr, "[hls][stream_seeker][common] line:%d, asa_src field reserved for internal use. \r\n",
            __LINE__
        );
        goto error;
    }
    const char *storage_alias = json_string_value(json_object_get(spec, "storage_alias"));
    size_t      _rdbuf_max_sz =
        json_integer_value(json_object_get(spec, "buf_max_sz")); // TODO, rename to  rdbuf_max_sz
    ...
    if (result != ASTORAGE_RESULT_ACCEPT) {
        fprintf(
            stderr, "[hls][stream_seeker][common] line:%d, failed to perform storage operation \r\n", __LINE__
        );
        goto error;
    }
