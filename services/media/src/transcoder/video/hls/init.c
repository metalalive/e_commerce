...
static void atfp_hls__create_local_workfolder_cb(
...
static void _atfp__hls_init__dst_version_folder_cb(asa_op_base_cfg_t *asa_dst, ASA_RES_CODE result) {
    ...
    if (result != ASTORAGE_RESULT_COMPLETE) {
        fprintf(
            stderr, "[transcoder][hls][init] line:%d, job_id:%s, result:%d \n", __LINE__,
    ...
    {
        // NOTE, if multiple destination file-processors work concurrently,  there should be
        // multiple local storage handles , each of which handles transcoded file for specific
        // version
        const char *local_tmpfile_basepath = asa_local_srcdata->super.op.mkdir.path.origin;
        const char *_version = processor->data.version;
        size_t path_sz = strlen(local_tmpfile_basepath) + 1 + sizeof(ATFP__TEMP_TRANSCODING_FOLDER_NAME) + 1 +
                         strlen(_version) + 1; // include NULL-terminated byte
        char   fullpath[path_sz];
        size_t nwrite = snprintf(
            &fullpath[0], path_sz, "%s/%s/%s", local_tmpfile_basepath, ATFP__TEMP_TRANSCODING_FOLDER_NAME,
            _version
        );
    ...
    }
    {
        ...
        hlsproc->internal.segment = (atfp_segment_t){
            ...
            .checksum = {0},
            .transfer = {0} // implicitly reset `curr_idx` field to 0
        };
    }
    {
...
void atfp__video_hls__init_transcode(atfp_t *processor) {
    ...
    if (json_object_size(err_info) > 0) {
        fprintf(
            stderr, "[atfp][hls][init] line:%d, job_id:%s, avinput or validation error \n", __LINE__,
...
void atfp__video_hls__proceeding_transcode(atfp_t *processor) {
    ...
    while (!ret) {
        ...
        if (ret) {
            if (ret < ATFP_AVCTX_RET__OK)
                json_object_set_new(
                    processor->data.error, "transcoder", json_string("[hls] error when filtering")
                );
            continue;
        ...
        while (!ret2) {
            ...
            if (ret2 < ATFP_AVCTX_RET__OK) {
                json_object_set_new(
                    processor->data.error, "transcoder", json_string("[hls] error when encoding")
                );
                ret = ret2;
            }
        }
    ...
    uint8_t flush_filt_done =
        hlsproc_dst->internal.op.has_done_flush_filter(hlsproc_src->av, hlsproc_dst->av);
