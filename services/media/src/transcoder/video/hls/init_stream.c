...
static int _atfp_hls__stream__crypto_key_rotation(json_t *keyinfo, json_t *err_info) {
    ...
    json_object_foreach(keyinfo, key_id, item) {
        ...
        // currently, HLS encryption only accept AES-128-CBC, would refactor code when there's more
        // to support Also note that `nbytes` above means the size of original key bytes, not the
        // hex string
    ...
    if (!ret) { // FIXME, bn-rand returns zero, debug
        ...
        fprintf(
            stderr, "[hls][init_stream] line:%d, failed to rotate key, reason:%s \r\n", __LINE__, &buf[0]
        );
    ...
    if ((actual_key_hex_sz == (HLS__NBYTES_KEY << 1)) && (actual_iv_hex_sz == (HLS__NBYTES_IV << 1)) &&
        ...
        fprintf(
            stderr,
            "[hls][init_stream] line:%d, key:sz=%u,data=0x%s, IV:sz=%u,data=0x%s, "
            "keyID:sz=%u,data=0x%s \r\n",
...
#undef RUNNER
    ...
    chosen_key_id =
        hlsproc->internal.op.get_crypto_key(keyinfo, ATFP__CRYPTO_KEY_MOST_RECENT, &chosen_keyitem);
    if (!chosen_key_id || !chosen_keyitem) {
        json_object_set_new(err_info, "transcoder", json_string("[hls] internal error"));
        fprintf(stderr, "[hls][init_stream] line:%d, failed to get crypto key \r\n", __LINE__);
        goto done;
    }
    hlsproc->internal.op.encrypt_document_id(
        &processor->data, chosen_keyitem, &encrypted_doc_id, &enc_doc_id_sz
    );
...
static ASA_RES_CODE atfp_hls__init_stream__crypto_keyfile(atfp_hls_t *hlsproc
    ...
    size_t nwrite = snprintf(
        &filepath[0], filepath_sz, "%s/%s", _asa_local->op.mkdir.path.origin, HLS_CRYPTO_KEY_FILENAME
    );
...
static void _atfp_hls__ensure_local_basepath_cb(
...
void atfp__video_hls__init_stream(atfp_t *processor) {
    ...
    if (!_err_info || !_spec || _usr_id == 0 || _upld_req_id == 0) {
        _http_resp_code = 400;
        json_object_set_new(
            _err_info, "transcoder", json_string("[hls] missing argument during init stream")
        );
        goto done;
    }
    ...
    if (!loop || !update_interval || keyfile_update_interval < 1.0f || !host_domain || !res_id_label ||
        !detail_label) {
        _http_resp_code = 400;
        json_object_set_new(
            _err_info, "transcoder", json_string("[hls] missing arguments in spec for constructing playlist")
        );
        goto done;
    }
...
#undef ASA_SRC_BASEPATH_PATTERN
    if (asa_result != ASTORAGE_RESULT_ACCEPT) {
        _http_resp_code = 500;
        json_object_set_new(
            _err_info, "storage", json_string("[hls] unable to send command for updating master playlist")
        );
...
static int
atfp_hls__encrypt_document_id(atfp_data_t *fp_data, json_t *kitem, unsigned char **out, size_t *out_sz) {
    int             success = 0;
...
#define CHECK_ELEMENT_FILE(fn0, fn1_name, _pattern, _prefix_sz, _version_required, _extra_cond) \
    if (!fn0) { \
        ...
        if (fn0 && _version_required && !processor->data.version) { \
            ...
            size_t alloc_sz = \
                uint_sz + APP_TRANSCODED_VERSION_SIZE - (APP_TRANSCODED_VERSION_SIZE % uint_sz); \
    ...
    if (detail) {
        ...
        CHECK_ELEMENT_FILE(
            _fn, build_secondary_playlist, HLS_PLAYLIST_FILENAME, APP_TRANSCODED_VERSION_SIZE + 1, 1,
            isalnum(detail[0]) && isalnum(detail[1]) && detail[APP_TRANSCODED_VERSION_SIZE] == '/'
        )
        CHECK_ELEMENT_FILE(
            _fn, encrypt_segment, HLS_SEGMENT_FILENAME_PREFIX, APP_TRANSCODED_VERSION_SIZE + 1, 0,
            isalnum(detail[0]) && isalnum(detail[1]) && detail[APP_TRANSCODED_VERSION_SIZE] == '/'
        )
        CHECK_ELEMENT_FILE(
            _fn, encrypt_segment, HLS_FMP4_FILENAME, APP_TRANSCODED_VERSION_SIZE + 1, 0,
            isalnum(detail[0]) && isalnum(detail[1]) && detail[APP_TRANSCODED_VERSION_SIZE] == '/'
        )
    }
    if (_fn) {
