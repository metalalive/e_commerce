...
static void _db_async_dummy_cb(db_query_t *target, db_query_result_t *detail) { (void *)detail; }
...
static void atfp_video__dst_update_metadata__db_async_err(db_query_t *target, db_query_result_t *detail) {
    ...
    json_object_set_new(
        processor->data.error, "model", json_string("[video] failed to update metadata of transcoded video")
    );
    fprintf(
        stderr, "[transcoder][video][metadata] line:%d, job_id:%s, unknown db error \n", __LINE__,
...
#define SQL_PATTERN__METADATA_INSERT \
    ...
    ",`height_pixel`,`width_pixel`,`framerate`) VALUES (?,?,?,?,?,?)' USING " \
    "FROM_BASE64('%s'),'%s',%u,%u,%u,%u;"

#define SQL_PATTERN__METADATA_UPDATE \
    "EXECUTE IMMEDIATE 'UPDATE `" DB_TABLE_NAME "` SET `height_pixel`=?,`width_pixel`=?,`framerate`=?" \
    " ,`size_bytes`=? WHERE `file_id`=? AND `version`=?' USING " \
    "%u,%u,%u,%u,FROM_BASE64('%s'),'%s';"
...
void atfp_video__dst_update_metadata(atfp_t *processor, void *loop) {
    ...
    if (_is_update) {
        nb_rawsql_used = snprintf(
            &raw_sql[0], raw_sql_sz, SQL_PATTERN__METADATA_UPDATE, height, width, framerate, total_nbytes,
            res_id_encoded, version
        );
    } else {
        nb_rawsql_used = snprintf(
            &raw_sql[0], raw_sql_sz, SQL_PATTERN__METADATA_INSERT, res_id_encoded, version, total_nbytes,
    ...
    if (db_result != DBA_RESULT_OK) {
        json_object_set_new(
            processor->data.error, "model", json_string("failed to send SQL command for transcoded video")
        );
        fprintf(
...
const char *atfp_video__metadata_dbtable_name(void) { return DB_TABLE_NAME; }
