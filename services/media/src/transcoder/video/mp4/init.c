...
static void atfp_mp4__avinput_init_done_cb(atfp_mp4_t *mp4proc) {
    ...
    if (json_object_size(err_info) == 0) {
        ...
        fprintf(
            stderr, "[transcoder][mp4][init] line:%d, job_id:%s, avinput or validation error \n", __LINE__,
...
static void atfp_mp4__preload_stream_info__done(atfp_mp4_t *mp4proc) {
    ...
    if (json_object_size(err_info) == 0) {
        ...
        if (result != ASTORAGE_RESULT_ACCEPT) {
            ...
            fprintf(
                stderr, "[transcoder][mp4][init] line:%d, job_id:%s, result:%d \n", __LINE__,
                processor->data.rpc_receipt->job_id.bytes, result
            );
        }
    } else {
        fprintf(
            stderr, "[transcoder][mp4][init] line:%d, job_id:%s, error in preload buffer \n", __LINE__,
            processor->data.rpc_receipt->job_id.bytes
        );
    }
    if (json_object_size(err_info) > 0)
...
static void atfp__video_mp4__open_local_tmpbuf_cb(
...
static void atfp__video_mp4__init(atfp_t *processor) {
    ...
    //       asa_local->super.op.open.dst_path);
...
static void atfp_mp4__asalocal_closefile_cb(
    asa_op_base_cfg_t *asaobj, ASA_RES_CODE result
    ...
    fprintf(
        stderr, "[transcoder][mp4][init] line:%d, local buffer path:%s \n", __LINE__,
        asa_local->super.op.unlink.path
    );
...
static void atfp_mp4__close_async_handle_cb(uv_handle_t *handle) { // close source chunkfile if still open
    atfp_t            *processor = handle->data;
...
static void _atfp_mp4__processing_one_frame(atfp_mp4_t *mp4proc) {
    ...
    do {
        ...
        if (!err) {
            ...
            if (!err) {
                ...
                ASA_RES_CODE result = mp4proc->internal.op.preload_pkt(
                    mp4proc, ATFP_MP4__DEFAULT_NBYTES_BULK, _atfp_mp4__processing_one_frame
                );
                ...
                json_object_set_new(
                    err_info, "transcoder",
                    json_string("[mp4] error when getting next packet from local temp buffer")
                );
                break;
            }
        } else {
    ...
    if (frame_avail) {
        ...
        if (err < 0)
            json_object_set_new(
                err_info, "transcoder", json_string("[mp4] failed to send async event with decoded frame")
            );
    }
