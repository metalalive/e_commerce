...
#undef NUM_UINT16_COLUMNS
    ...
    switch (is_editing) {
    case 1:
        // message-queue consumer (in later step) check this field and optionally rename exising
        // version folder (to stale state, so it would be deleted after new version is transcoded)
...
#define SQL_PATT_VIDEO \
    "EXECUTE IMMEDIATE 'SELECT `version`,`height_pixel`,`width_pixel`,`framerate` FROM" \
    " `transcoded_video_metadata` WHERE `file_id` = ? and `version` IN (%s)' USING " \
    "FROM_BASE64('%s'), %s;"
#define SQL_PATT_IMAGE \
    "EXECUTE IMMEDIATE 'SELECT " \
    "`version`,`scale_h`,`scale_w`,`crop_h`,`crop_w`,`crop_x`,`crop_y`,`mask_patt`" \
    " FROM `transformed_image_metadata` WHERE `file_id` = ? and `version` IN (%s)' USING " \
    "FROM_BASE64('%s'), %s;"
    const char *out = NULL;
...
static void _validate_video_req__output_v(json_t *elm, json_t *err_info) { // TODO, improve err-info structure
    size_t     idx = 0;
...
static void _atfp_validate_video_req__outputs_elm_st_map(json_t *output, json_t *elm_st_dict, json_t *err) {
    ...
    if (audio_stream_included == 1 && video_stream_included == 1) {
        ...
        json_object_set_new(output, "__internal__", internal);
             ...
             // ?
        json_object_set_new(
            err, "elementary_streams",
            json_string("each output item should have exact one audio stream and exact one video "
                        "stream to mux")
        );
    }
    if (audio_stream_key) {
        free(audio_stream_key);
    }
    if (video_stream_key) {
        free(video_stream_key);
    }
...
static void _atfp_validate_video_request(json_t *spec, json_t *err_info) {
    ...
    if (json_object_size(err_info) == 0)
        _validate_video_req__outputs(
            json_object_get(spec, "outputs"), json_object_get(spec, "elementary_streams"), err_info
        );
}
...
static int _validate_image_req__scaling(
    json_t *_crop_item, json_t *_scale_item, json_t *err_info, aav_cfg_img_t *img_cfg
) {
    int     err = 0;
    json_t *err_detail = json_object();
