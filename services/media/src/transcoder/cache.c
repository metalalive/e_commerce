...
static void _atfp_cachecommon_localfile2_closed_cb(asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result) {
...
static void _atfp_cachecommon_localfile1_closed_cb(
    asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result
) { // close the file specified in `existing_cached_fd` of object `asa_cch_usrdata_t`
    asa_op_localfs_cfg_t *asa_cch_local = (asa_op_localfs_cfg_t *)_asa_cch_local;
    asa_cch_usrdata_t    *usrdata = asa_cch_local->file.data;
    int                   fd = usrdata->existing_cached_fd;
    if (fd > -1) {
        asa_cch_local->file.file = fd;
        _asa_cch_local->op.close.cb = _atfp_cachecommon_localfile2_closed_cb;
        result = _asa_cch_local->storage->ops.fn_close(_asa_cch_local);
        if (result != ASTORAGE_RESULT_ACCEPT)
            _atfp_cachecommon_localfile2_closed_cb(_asa_cch_local, result);
    } else {
        _atfp_cachecommon_localfile2_closed_cb(_asa_cch_local, result);
    }
}
...
static void atfp_cachecommon_deinit(asa_op_base_cfg_t *_asa_cch_local) {
    ...
    if (fd >= 0) {
        ...
        _asa_cch_local->op.close.cb = _atfp_cachecommon_localfile1_closed_cb;
        ASA_RES_CODE result = _asa_cch_local->storage->ops.fn_close(_asa_cch_local);
        if (result != ASTORAGE_RESULT_ACCEPT)
            _atfp_cachecommon_localfile1_closed_cb(_asa_cch_local, result);
    } else {
        _atfp_cachecommon_localfile1_closed_cb(_asa_cch_local, ASTORAGE_RESULT_COMPLETE);
    }
...
int atfp_cache_save_metadata(
    ...
    json_t     *cacheable_flag = json_object_get(fp_data->spec, "http_cacheable");
    if (!key_id)
        return 1;
    int skipped = 0;
#define PATTERN "%s/%s"
...
#undef PATTERN
    ...
    if (ret != 0) { // not exist
        ...
        if (cacheable_flag != NULL)
            json_object_set(info, "http_cacheable", cacheable_flag);
    ...
    if (result == ASTORAGE_RESULT_COMPLETE) {
        ...
        if (ret == 0) {
            ...
            if (errno == EWOULDBLOCK) {
                ...
                fprintf(
                    stderr, "[atfp][cache] line:%d, error (%d) when locking cache file \r\n", __LINE__, errno
                );
            }
        }
    } else {
...
static void _atfp_cachecommon_ensure_detailpath_cb(
    asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result
) { // TODO, reuse
    json_t *err_info = _asa_cch_local->cb_args.entries[ERRINFO_INDEX__IN_ASA_USRARG];
    DEINIT_IF_EXISTS(_asa_cch_local->op.mkdir.path.origin, free);
...
static ASA_RES_CODE
_atfp_streamcache_new_detailpath(asa_op_base_cfg_t *_asa_cch_local, const char *_path_end_pos) {
    json_t     *spec = _asa_cch_local->cb_args.entries[SPEC_INDEX__IN_ASA_USRARG];
...
static void _atfp_cache_processor_setup_ready_cb(atfp_t *processor) {
    ...
    asa_op_base_cfg_t *_asa_cch_local =
        (asa_op_base_cfg_t *)json_integer_value(json_object_get(spec, "_asa_cache_local"));
...
static void _atfp_streamcache_metadata_close_cb(asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result) {
    ...
    asa_op_localfs_cfg_t *asa_cch_local = (asa_op_localfs_cfg_t *)_asa_cch_local;
    asa_cch_usrdata_t    *cch_usrdata = asa_cch_local->file.data;
    if (cch_usrdata->existing_cached_fd > -1) {
        asa_cch_local->file.file = cch_usrdata->existing_cached_fd;
        cch_usrdata->existing_cached_fd = -1;
    }
    if (result == ASTORAGE_RESULT_COMPLETE && json_object_size(err_info) == 0) {
        json_t *_metadata = json_object_get(spec, "metadata");
        // ensure cacheable flag is always set
        uint8_t _cchable = json_boolean_value(json_object_get(_metadata, "http_cacheable"));
        json_object_set_new(spec, "http_cacheable", json_boolean(_cchable));
        if (_cchable) {
            json_t *pxyobj = json_object_get(spec, "proxy_host_domain");
            if (pxyobj != NULL) {
                const char *pxy_srv_host = json_string_value(pxyobj);
                json_object_del(spec, "host_domain");
                json_object_set_new(spec, "host_domain", json_string(pxy_srv_host));
            }
        }
        if (cch_usrdata->flags.already_exists) {
            INVOKE_INIT_USR_CALLBACK(_asa_cch_local, result);
        } else {
            ...
            processor->data = (atfp_data_t
              ...
              .spec = spec,
              .callback = _atfp_cache_processor_setup_ready_cb,
              .usr_id = res_owner_id,
            ...
            if (json_object_size(err_info) > 0) {
                fprintf(
                    stderr,
                    "[atfp][cache] line:%d, failed to setup file processor"
                    ", type:%s \r\n",
                    __LINE__, label
                );
                INVOKE_INIT_USR_CALLBACK(_asa_cch_local, result);
...
static void _atfp_streamcache_existence_check(asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result) {
    ...
    if (result == ASTORAGE_RESULT_COMPLETE) {
        asa_op_localfs_cfg_t *asa_cch_local = (asa_op_localfs_cfg_t *)_asa_cch_local;
        assert(asa_cch_local->file.file > -1);
        asa_cch_usrdata_t *_usrdata = asa_cch_local->file.data;
        _usrdata->flags.already_exists = 1;
        _usrdata->existing_cached_fd = asa_cch_local->file.file;
        asa_cch_local->file.file = -1; // temporarily reset for opening metadata at later time
    } // then always open and load metadata (for http_cacheable flag)
#define PATTERN "%s/%s"
    ...
    char        filepath[filepath_sz];
    size_t      nwrite =
        snprintf(&filepath[0], filepath_sz, PATTERN, _cached_path, ATFP_ENCRYPT_METADATA_FILENAME);
    assert(filepath_sz >= nwrite);
#undef PATTERN
...
static void _atfp_nonstreamcache_remotesrc_open_cb(
...
static void _atfp_nonstreamcache_existence_check(asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result) {
    json_t *spec = _asa_cch_local->cb_args.entries[SPEC_INDEX__IN_ASA_USRARG];
    json_t *err_info = _asa_cch_local->cb_args.entries[ERRINFO_INDEX__IN_ASA_USRARG];
    if (result == ASTORAGE_RESULT_COMPLETE) {
        INVOKE_INIT_USR_CALLBACK(_asa_cch_local, result);
        ...
        asa_op_base_cfg_t *asa_src_remote =
            (asa_op_base_cfg_t *)app_storage__init_asaobj_helper(storage, _asa_cch_local->cb_args.size, 0, 0);
...
#undef PATTERN
        ...
        if (result != ASTORAGE_RESULT_ACCEPT) {
            json_object_set_new(err_info, "storage", json_string("internal error"));
            fprintf(
                stderr, "[atfp][cache][storage] line:%d, failed to open file, path:%s \r\n", __LINE__,
                &fullpath[0]
            );
            INVOKE_INIT_USR_CALLBACK(_asa_cch_local, result);
        }
...
static asa_op_localfs_cfg_t *_atfp_cache_init_common(
    void *loop, json_t *spec, json_t *err_info, uint8_t num_cb_args, uint32_t buf_sz,
    asa_open_cb_t usr_init_cb, asa_close_cb_t usr_deinit_cb, asa_open_cb_t nxt_evt_cb
) {
...
asa_op_localfs_cfg_t *atfp_streamcache_init(
    ...
    asa_op_localfs_cfg_t *out = _atfp_cache_init_common(
        loop, spec, err_info, num_cb_args, buf_sz, usr_init_cb, usr_deinit_cb,
        _atfp_streamcache_existence_check
    );
    if (out)
...
asa_op_localfs_cfg_t *atfp_cache_nonstream_init(
    ...
    asa_op_localfs_cfg_t *out = _atfp_cache_init_common(
        loop, spec, err_info, num_cb_args, buf_sz, usr_init_cb, usr_deinit_cb,
        _atfp_nonstreamcache_existence_check
    );
    if (out)
        out->super.deinit = atfp_cachecommon_deinit;
    return out;
...
static void
_atfp_read_from_cachedfile_cb(asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result, size_t nread) {
...
static void
_atfp_write_to_cachedfile_cb(asa_op_base_cfg_t *_asa_cch_local, ASA_RES_CODE result, size_t nwrite) {
...
static void _atfp_s_proceed_cachedata_ready_cb(atfp_t *processor) {
    ...
    if (json_object_size(err_info) == 0) {
        ...
        fprintf(
            stderr,
            "[atfp][cache] line:%d, file processor failed to produce "
            "response data \r\n",
            __LINE__
        );
...
static void
_atfp_ns_proceed_cachedata_ready_cb(asa_op_base_cfg_t *_asa_src, ASA_RES_CODE result, size_t nread) {
    json_t               *err_info = _asa_src->cb_args.entries[ERRINFO_INDEX__IN_ASA_USRARG];
    atfp_asa_map_t       *_map = _asa_src->cb_args.entries[ASAMAP_INDEX__IN_ASA_USRARG];
    asa_op_localfs_cfg_t *tmp = atfp_asa_map_get_localtmp(_map);
    asa_op_base_cfg_t    *_asa_cch_local = &tmp->super;
    if (result == ASTORAGE_RESULT_COMPLETE) {
        ...
        if (result != ASTORAGE_RESULT_ACCEPT) {
            fprintf(
                stderr,
                "[atfp][cache] line:%d, failed to issue write opeation to"
                "local file \r\n",
                __LINE__
            );
            json_object_set_new(err_info, "storage", json_string("internal error"));
        ...
        fprintf(
            stderr,
            "[atfp][cache] line:%d, failed to load data block from source file"
            "\r\n",
            __LINE__
        );
        INVOKE_PROCEED_USR_CALLBACK(_asa_cch_local, ASTORAGE_RESULT_UNKNOWN_ERROR, NULL, 1);
