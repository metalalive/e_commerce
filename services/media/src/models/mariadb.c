...
static void _app_mariadb_error_notify_queries(db_conn_t *conn, db_llnode_t *q_head, DBA_RES_CODE app_result) {
    ...
    for (q_node = q_head; q_node; q_node = q_node->next) {
        ...
        q_ret->_final = 1;
#if 0
        fprintf(stderr, "[mariaDB] line:%d, conn_state:%d, app_result:%d, sql:%s \n",
                __LINE__, conn->state, app_result, q->cfg.statements.entry );
#endif
...
static void _app_mariadb_error_reset_all_pending_query(
    db_conn_t *conn, DBA_RES_CODE app_result
...
static DBA_RES_CODE _app_mariadb_convert_error_code(MYSQL *handle) {
    ...
    if (error_code) { // use mysql_error() to check detail description
        fprintf(
            stderr, "[db][mariaDB] line:%d, error code: %d, detail:%s \n", __LINE__, error_code,
            mysql_error(handle)
        );
    }
    if (!error_code) { // pass
...
static DBA_RES_CODE app_db_mariadb_conn_connect_start(db_conn_t *conn, int *evt_flgs) {
    ...
    MYSQL         *my_ret = NULL;
    int            my_evts = mysql_real_connect_start(
        &my_ret, (MYSQL *)conn->lowlvl.conn, credential->db_host, credential->db_user, credential->db_passwd,
        ...
        if (my_evts) { // needs more time to wait on
            *evt_flgs = _app_mariadb_convert_evt_to_uv(my_evts);
            ...
            if (my_ret) {
                ...
                result = _app_mariadb_convert_error_code(conn->lowlvl.conn);
            }
        }
    }
    return result;
...
static DBA_RES_CODE app_db_mariadb_conn_send_query_start(db_conn_t *conn, int *evt_flgs) {
    ...
    int my_evts = mysql_real_query_start(
        &my_err, (MYSQL *)conn->lowlvl.conn, &conn->bulk_query_rawbytes.data[0],
        conn->bulk_query_rawbytes.wr_sz
    );
...
static void _app_mariadb_row_ready_helper(db_conn_t *conn, DBA_RES_CODE app_result) {
    ...
    *rs = (db_query_result_t
    ){.free_data_cb = free,
      .app_result = app_result,
      .conn =
          {.state = conn->state, .alias = conn->pool->cfg.alias, .async = (conn->loop != curr_query->cfg.loop)
          },
      ._final = 0};
    {
...
#if 0
    if(app_result != DBA_RESULT_OK)
        fprintf(stderr, "[mariaDB] line:%d, conn_state:%d, app_result:%d, sql:%s \n",
                __LINE__, conn->state, app_result, curr_query->cfg.statements.entry );
#endif
...
void app_mariadb_async_state_transition_handler(app_timer_poll_t *target, int uv_status, int event_flags) {
    ...
    do {
        ...
        case DB_ASYNC_CONN_WAITING:
            ...
            // signal that the file descriptor is writable or readable even when it is not,
            // application caller should be prepared to double-check this by using low-level
            // (database-specific) function to check the status of current connection or running
            // query
        ...
        case DB_ASYNC_CONN_DONE:
            ...
            if (result == DBA_RESULT_OK) {
                ...
                _app_mariadb_error_reset_all_processing_query(
                    conn, result
                ); // TODO, retry before evicting processing queries
        ...
        case DB_ASYNC_QUERY_READY:
            ...
            if (result == DBA_RESULT_OK) {
                ...
                conn->state =
                    (result == DBA_RESULT_NETWORK_ERROR) ? DB_ASYNC_CLOSE_START : DB_ASYNC_QUERY_START;
                continue_checking = 1;
                break;
            }
        case DB_ASYNC_CHECK_CURRENT_RESULTSET:
            ...
            if (conn->processing_queries) {
                ...
                if (conn->lowlvl.resultset) {
                    rs_node_sz += sizeof(db_query_rs_info_t);
                }
                db_llnode_t       *rs_node = malloc(rs_node_sz);
                db_query_result_t *rs = (db_query_result_t *)&rs_node->data[0];
                *rs = (db_query_result_t
                  ...
                  .conn = {
                      .state = conn->state,
                      .alias = conn->pool->cfg.alias,
                      .async = (conn->loop != curr_query->cfg.loop)
                  }};
                if (conn->lowlvl.resultset) {
                    db_query_rs_info_t *cloned_rs_info = (db_query_rs_info_t *)&rs->data[0];
                    cloned_rs_info->num_rows.affected =
                        (size_t)mysql_affected_rows((MYSQL *)conn->lowlvl.conn);
                ...
                rs->_final = (curr_query->db_result.num_rs_remain == 0) &&
                             (rs->app_result == DBA_RESULT_END_OF_ROWS_REACHED);
#if 0
                    if(rs->app_result != DBA_RESULT_OK && rs->app_result != DBA_RESULT_END_OF_ROWS_REACHED) {
                        const char *sql = curr_query ? curr_query->cfg.statements.entry: NULL;
                        fprintf(stderr, "[mariaDB] line:%d, conn_state:%d, app_result:%d, sql:%s \n",
                                __LINE__, conn->state, rs->app_result, sql );
                    }
#endif
                app_db_query_notify_with_result(curr_query, rs);
        ...
        case DB_ASYNC_MOVE_TO_NEXT_RESULTSET_DONE:
            ...
            if (result == DBA_RESULT_OK) {
                ...
                // if all the result sets has been visited, the result should be
                // DBA_RESULT_END_OF_RSETS_REACHED
                conn->state = DB_ASYNC_QUERY_START;
            } else { // other errors
                // still switch to this state even we're pretty sure there won't be new result set
                // available because this handling function still needs to respond to currently
                // processing query added
        ...
        case DB_ASYNC_FREE_RESULTSET_DONE:
            ...
            if (conn->processing_queries) {
                ...
                *rs = (db_query_result_t){
                    ...
                    if(result != DBA_RESULT_OK)
                        fprintf(stderr, "[mariaDB] line:%d, conn_state:%d, app_result:%d, sql:%s \n",
                                __LINE__, conn->state, result, curr_query->cfg.statements.entry );
#endif
                app_db_query_notify_with_result(curr_query, rs);
            }
            conn->state = DB_ASYNC_MOVE_TO_NEXT_RESULTSET_START;
        ...
        case DB_ASYNC_CLOSE_DONE:
...
#if 0
                fprintf(stderr, "[mariaDB] line:%d, is_app_closing:%hu, continue_checking:%hu \n",
                        __LINE__, is_app_closing, continue_checking);
#endif
            if (is_app_closing) {
            ...
            break; // keep looping and transitting between the states until all queries are
                   // processed
