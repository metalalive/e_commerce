...
struct addrinfo *resolve_net_addr(int socktype, int protocol, const char *host, uint16_t port) {
    ...
    if (error) {
        h2o_error_printf(
            "[parsing] failed to resolve address, host:%s, port:%d, reason:%s\n", host, port,
            gai_strerror(error)
        );
    }
    if (res == NULL) {
...
uv_tcp_t *create_network_handle(
    uv_loop_t *loop, struct addrinfo *ainfo, uv_connection_cb cb_on_accept, unsigned int tfo_q_len
    ...
    if ((flgs & 0xff) == AF_INET) {
        ...
        inet_ntop(AF_INET, &((struct sockaddr_in *)sock_addr)->sin_addr, (void *)&iplog[0], sizeof(iplog));
        h2o_error_printf("[network][create-handle] ipv4: %s:%hu \n", &iplog[0], port_hsb);
    } else if ((flgs & 0xff) == AF_INET6) {
        char iplog[INET6_ADDRSTRLEN] = {0};
        inet_ntop(AF_INET6, &((struct sockaddr_in6 *)sock_addr)->sin6_addr, (void *)&iplog[0], sizeof(iplog));
    ...
    if (ret != 0) {
        h2o_error_printf(
            "[network][create-handle] failed to initialize handle, flags:%x , reason:%s \n", flgs,
            uv_strerror(ret)
        );
        goto error;
    }
    ...
    if (ret < 0 || fd == -1) {
        h2o_error_printf(
            "[network][create-handle] failed to get sockfd from handle, reason:%s \n", uv_strerror(ret)
        );
        goto error;
    }
    int optval = 1;
    ...
    if (ret != 0) {
        h2o_error_printf(
            "[network][create-handle] failed to reuse port %u, reason:%s \n", port_hsb, uv_strerror(ret)
        );
        goto error;
    }
    ...
    if (ret != 0) {
        h2o_error_printf(
            "[network][create-handle] failed to bind the address, \
                reason:%s \n",
            uv_strerror(ret)
        );
        goto error;
    ...
    if (tfo_q_len > 0) {
        ...
        if (ret != 0) {
            h2o_error_printf(
                "[network][create-handle] failed to configure TCP \
                    fastopen, reason:%s \n",
                uv_strerror(ret)
            );
            goto error;
...
h2o_socket_t *init_client_tcp_socket(uv_stream_t *server, uv_close_cb on_close) {
    ...
    if (ret != 0) {
        h2o_error_printf(
            "[network] server failed to initialize client connection, reason:%s \n", uv_strerror(ret)
        );
        goto error;
    }
    ...
    if (ret != 0) {
        ...
        if (ret != UV_EAGAIN) {
            h2o_error_printf(
                "[network] server failed to accept client connection, reason:%s \n", uv_strerror(ret)
            );
        }
        goto error;
    }
...
int atomic_num_connections(app_cfg_t *app_cfg, int delta) {
    ...
    if (delta == 0) {
        ...
        prev = atomic_fetch_add_explicit(&app_cfg->state.num_curr_connections, delta, memory_order_acq_rel);
    }
    return prev;
}
