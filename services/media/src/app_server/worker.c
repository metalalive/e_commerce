...
static void on_tcp_accept(uv_stream_t *server, int status) {
    ...
    // TLS handshake takes about 1 ms, this limits the latency induced by TLS handshakes to 10 ms
    // per event loop
    ...
    do { // TODO, figure out how to use libh2o to monitor and de-init connections which hang
        ...
        if (curr_num_conns >= acfg->max_connections) {
            ...
            h2o_error_printf(
                "[worker] ID = %lu, number of connections exceeds the limit %d \n",
...
static void worker_dup_network_handle(
    app_ctx_listener_t *ctx, const app_cfg_t *cfg, uv_loop_t *loop, unsigned int thread_index
) {
    ...
    for (idx = 0; (listener = cfg->listeners[idx]) != NULL; idx++) {
        ...
        struct addrinfo ai = {
            ...
            .ai_next = NULL,
            .ai_family = nt_attr->ai_family,
            .ai_flags = nt_attr->ai_flags,
            .ai_socktype = nt_attr->ai_socktype,
            .ai_protocol = nt_attr->ai_protocol
        };
        nt_handle = (uv_handle_t *)create_network_handle(loop, &ai, on_tcp_accept, cfg->tfo_q_len);
...
static void worker_init_accept_ctx(app_ctx_listener_t *ctx, const app_cfg_t *cfg, h2o_context_t *http_ctx) {
    ...
    http_ctx->storage.entries[1] = (h2o_context_storage_item_t
    ){.dispose = app_rpc_conn_deinit, .data = app_rpc_conn_init(cfg->rpc.entries, cfg->rpc.size)};
...
static void migrate_worker_to_cpu(uv_thread_t tid, unsigned int cpu_id) {
    ...
    if (ret != 0) {
        h2o_error_printf(
            "[system] failed to migrate thread (id = %ld) to CPU core %u \n", (long int)tid, cpu_id
        );
    }
...
static void run_loop(void *data) {
    ...
    app_ctx_worker_t           ctx_worker = {
                  .listeners = &ctx_listeners[0], .num_listeners = app_cfg->num_listeners, .thread_id = uv_thread_self()
    };
    ...
    h2o_multithread_register_receiver(
        server_ctx.queue, &ctx_worker.server_notifications, on_server_notification
    );
    ...
    do {
        ...
        // if not waiting for 1000 milliseconds, that means all internal buffers / allocators are
        // cleaned
    } while (max_wait == 1000);
