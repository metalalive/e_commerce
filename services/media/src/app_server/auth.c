...
static char *extract_header_auth_token(
    h2o_req_t *req, size_t found_idx, size_t *o_sz
) { // extract encoded JWT, e.g. `Authorization` header should contain : Bearer
    ...
    *o_sz = rawdata->len - hdr_val_prefix_sz;
    ...
    if (out == NULL)
        fprintf(
            stderr, "[auth] line:%d, found_idx:%lu, header name:%s, value:%s, value-sz:%lu \n", __LINE__,
            found_idx, req->headers.entries[found_idx].orig_name, rawdata->base, rawdata->len
        );
    return out;
...
static json_t *perform_jwt_authentication(
    jwks_t *keyset, const char *in, size_t in_sz
) { // In this application , auth server (user_management) always issue new access token with `kid`
    // field
    jwk_t  *pubkey = NULL;
    jwt_t  *jwt = NULL;
    json_t *claims = NULL;
    ...
    if (result != RHN_OK) {
        fprintf(
            stderr, "[auth] line:%d, failed to claim audience, result:%d, in_sz:%lu \n", __LINE__, result,
            in_sz
        );
        goto done;
    }
    ...
    if (!pubkey) {
        fprintf(
            stderr,
            "[auth] line:%d, failed to get key ID object,"
            " result:%d, kid:%s \n",
            __LINE__, result, kid
        );
        goto done;
    ...
    if (result != RHN_OK) {
        fprintf(
            stderr,
            "[auth] line:%d, invalid pub key, result:%d, kid:%s "
            "\n",
            __LINE__, result, kid
        );
        goto done;
    ...
    if (result != RHN_OK) {
        fprintf(
            stderr,
            "[auth] line:%d, fail to verify sign, result:%d, kid:%s "
            "\n",
            __LINE__, result, kid
        );
        goto done;
    }
    result = r_jwt_validate_claims(
        jwt, R_JWT_CLAIM_EXP, R_JWT_CLAIM_NOW, R_JWT_CLAIM_IAT, R_JWT_CLAIM_NOW, R_JWT_CLAIM_NOP
    );
    if (result != RHN_OK)
        goto done;
...
#define AUTH_HEADER_NAME "authorization"
    size_t name_len =
        sizeof(AUTH_HEADER_NAME) - 1; // exclude final byte which represent NULL-terminating character
...
int app_authenticate_user(RESTAPI_HANDLER_ARGS(self, req), app_middleware_node_t *node) {
    ...
    if (hsearch_r(e, ENTER, &e_ret, node->data)) {
        ...
        h2o_send_error_500(req, "internal error", "", H2O_SEND_ERROR_KEEP_HEADERS);
        h2o_error_printf(
            "[auth] failed to save JWT claims (0x%lx) to given hash map \n", (unsigned long int)decoded
        );
        json_decref(decoded);
    }
    goto done;
...
int app_rotate_jwks_store(struct app_jwks_t *jwks) {
    ...
    if (!atomic_flag_test_and_set_explicit(
            &jwks->is_rotating, memory_order_acquire
        ...
        if (load_result != RHN_OK) {
            h2o_error_printf(
                "[parsing] failed to preload JWKS from given URI: %s , result:%d \n", url, load_result
            );
