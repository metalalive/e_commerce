...
impl AbsProductPriceRepo for MockRepository {
    ...
    async fn delete(&self, _store_id:u32, _ids:ProductPriceDeleteDto) -> DefaultResult<(), AppError>
    { self._mocked_del_subset.clone() }
    
    async fn fetch(&self, store_id:u32, ids:Vec<(ProductType,u64)>)
        -> DefaultResult<ProductPriceModelSet, AppError>
    {
        ...
        match &self._mocked_fetch {
            Ok(m) => Ok(ProductPriceModelSet {store_id: m.store_id,
    ...
    { // embed verification logic at here, the use case invokes `fetch()` first
      // then `save()`, just ensure the call arguments are correct.
        if let (Ok(guard), Some(expect)) = (self._callargs_fetch_actual.lock(),
...
impl MockRepository {
        ...
        ) -> Self
    {
        ...
        Self {_mocked_save, _mocked_del_all, _mocked_del_subset, _mocked_fetch,
            _callargs_fetch_expect: None, _callargs_fetch_actual }
    }
    fn _clone_fetched_items(&self, src:&Vec<ProductPriceModel>) -> Vec<ProductPriceModel>
    {
        src.iter().map(|g| {
            ProductPriceModel {
                price: g.price, start_after: g.start_after, end_before: g.end_before,
                product_id: g.product_id, product_type: g.product_type.clone(),
...
async fn create_ok ()
{
    ...
    let creating_products = vec![
        ProductPriceEditDto { price:389, product_type:ProductType::Item, product_id:2379,
            start_after: DateTime::parse_from_rfc3339("2022-11-25T09:13:39+05:00").unwrap().into(),
            end_before: DateTime::parse_from_rfc3339("2023-09-12T21:23:00+06:00").unwrap().into(),
        },
        ProductPriceEditDto { price:51, product_type:ProductType::Package, product_id:2642,
            start_after: DateTime::parse_from_rfc3339("2022-11-24T09:13:39+05:00").unwrap().into(),
            end_before: DateTime::parse_from_rfc3339("2023-09-12T21:25:01+11:00").unwrap().into(),
        },
    ];
...
async fn update_ok ()
{
    ...
    let mocked_ppset = ProductPriceModelSet {
        store_id: mocked_store_id, items: vec![
            ProductPriceModel { price:1009, product_type:ProductType::Package, product_id:2613, is_create:false,
            ...
            ProductPriceModel { price:1010, product_type:ProductType::Item, product_id:3072, is_create:false,
    ...
    let updating_products = vec![
        ProductPriceEditDto { price:1322, product_type:ProductType::Item, product_id:3072,
            start_after: DateTime::parse_from_rfc3339("2021-03-29T10:58:39-06:00").unwrap().into(),
            end_before: DateTime::parse_from_rfc3339("2023-05-15T04:18:07+08:00").unwrap().into(),
        },
        ProductPriceEditDto { price:1155, product_type:ProductType::Package, product_id:2613,
            start_after: DateTime::parse_from_rfc3339("2022-04-27T07:15:47+01:00").unwrap().into(),
            end_before: DateTime::parse_from_rfc3339("2024-08-29T20:34:25-08:00").unwrap().into(),
        },
    ];
    let creating_products = vec![
        ProductPriceEditDto { price:452, product_type:ProductType::Item, product_id:8299,
        ...
        ProductPriceEditDto { price:6066, product_type:ProductType::Package, product_id:1712,
    ...
    repo.expect_callargs_fetch((mocked_store_id,
...
async fn fetch_error ()
{
    ...
    let repo = MockRepository::_new(Ok(()), Ok(()),
        Err(AppError{code:AppErrorCode::DataTableNotExist, detail: Some(expect_errmsg.to_string()) })
        , Ok(()) );
    let creating_products = vec![
        ProductPriceEditDto { price:389, product_type:ProductType::Item, product_id:2379,
            start_after: DateTime::parse_from_rfc3339("2022-11-25T09:13:39+05:00").unwrap().into(),
...
async fn save_error ()
{
    ...
    let mocked_ppset = ProductPriceModelSet {store_id:mocked_store_id, items:vec![
        ProductPriceModel { is_create:false, price:4810, product_type:ProductType::Package, product_id:9914,
    ...
    let updating_products = vec![
        ProductPriceEditDto { price:183, product_type:ProductType::Package, product_id:9914,
            start_after: DateTime::parse_from_rfc3339("2022-11-23T09:13:41+07:00").unwrap().into(),
            end_before: DateTime::parse_from_rfc3339("2023-10-12T21:23:00+08:00").unwrap().into(),
        },
    ];
...
async fn delete_subset_ok ()
{
    ...
    let data = ProductPriceDto { s_id: mocked_store_id, rm_all: false,
        deleting: ProductPriceDeleteDto {item_type:ProductType::Item, pkg_type:ProductType::Package,
...
async fn delete_subset_error ()
{
    ...
    let (mocked_store_id, expect_errmsg) = (12345, "unit-test-set-error-1");
    let mocked_ppset = ProductPriceModelSet {store_id: mocked_store_id, items:Vec::new()};
    let repo = MockRepository::_new( Ok(()),
        Err(AppError{code:AppErrorCode::DataTableNotExist, detail: Some(expect_errmsg.to_string()) })
        , Ok(mocked_ppset), Ok(())
    );
    let data = ProductPriceDto { s_id: mocked_store_id, rm_all: false,
