...
impl AbsOrderStockRepo for MockStockRepo {
    async fn fetch(
        &self,
        _pids: Vec<ProductStockIdentity>,
    ...
    async fn try_reserve(
        &self,
        _cb: AppStockRepoReserveUserFunc,
        _order_req: &OrderLineModelSet,
    ...
    async fn try_return(
        &self,
        _cb: AppStockRepoReturnUserFunc,
...
impl MockOrderRepo {
    fn build(
        stk_save_r: DefaultResult<(), AppError>,
    ...
    async fn acquire(&self, _num_retry: u8) -> TestAcquireClientResult {
        if let Ok(guard) = self._mock_acquire_c.lock() {
            ...
            if let Some(mocked) = objref.take() {
                ...
                Err(AppError {
                    detail: Some("client-acquire-empty".to_string()),
...
impl AbsRpcServerCtx for MockRpcContext {
    async fn server_start(
        ...
        if let Some(req) = guard.take() {
            ...
            Err(AppError {
                detail: Some("server-recv-empty-req".to_string()),
                code: AppErrorCode::Unknown,
            })
        }
    }
...
impl Default for MockRpcHandler {
    fn default() -> Self {
        Self {
            _mock_client_publish: None,
            _mock_client_rreply: None,
        }
    }
}
impl MockRpcHandler {
    fn mock_c_pub(mut self, m: TestClientPublishResult) -> Self {
        self._mock_client_publish = Some(m);
        self
    }
    fn mock_c_reply(mut self, m: TestClientReplyResult) -> Self {
        self._mock_client_rreply = Some(m);
        self
    }
}
...
async fn client_run_rpc_ok() {
    ...
    let ctx: Arc<Box<dyn AbstractRpcContext>> = {
        ...
        let hdlr = {
            ...
            let m2 = AppRpcReply {
                body: UTEST_REPLY_BODY_SERIAL.to_vec(),
            };
...
async fn client_run_rpc_publish_error() {
    ...
    let ctx: Arc<Box<dyn AbstractRpcContext>> = {
        ...
        let hdlr = {
            let h = MockRpcHandler::default();
            let m1 = AppError {
                code: AppErrorCode::RpcPublishFailure,
                detail: Some(ut_error_detail.clone()),
            };
            h.mock_c_pub(Err(m1))
        };
...
async fn mock_route_handler_ok(
...
fn mock_route_handler_ok_wrapper(
    req: AppRpcClientReqProperty,
    shr_state: AppSharedState,
) -> Pin<Box<dyn Future<Output = DefaultResult<Vec<u8>, AppError>> + Send>> {
    Box::pin(async move { mock_route_handler_ok(req, shr_state).await })
}
#[tokio::test]
...
async fn mock_route_handler_fail(
    req: AppRpcClientReqProperty,
    _ss: AppSharedState,
) -> DefaultResult<Vec<u8>, AppError> {
    // request and error handling
...
fn mock_route_handler_fail_wrapper(
    req: AppRpcClientReqProperty,
    shr_state: AppSharedState,
) -> Pin<Box<dyn Future<Output = DefaultResult<Vec<u8>, AppError>> + Send>> {
    Box::pin(async move { mock_route_handler_fail(req, shr_state).await })
}
#[tokio::test]
async fn server_run_rpc_receive_request_error() {
    ...
    let result = rctx
        .server_start(shr_state, mock_route_handler_fail_wrapper)
        .await;
