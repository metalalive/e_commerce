...
async fn in_mem_save_fetch_ok_1() {
    ...
    if let Ok(fetched) = result {
        ...
        let exists = fetched
            .items
            .iter()
            .any(|m| m.product_type == fetching_ids[1].0 && m.product_id == fetching_ids[1].1);
    ...
    if let Ok(fetched) = result {
        ...
        let exists = fetched
            .items
            .iter()
            .any(|m| m.product_type == fetching_ids[2].0 && m.product_id == fetching_ids[2].1);
        assert_eq!(exists, false);
    }
...
async fn in_mem_save_fetch_ok_2() {
    ...
    let new_5th_elm = ProductPriceModel {
        is_create: false,
        price: 7811,
        product_type: pprice_data[5].product_type.clone(),
...
async fn in_mem_save_fetch_ok_3() {
    ...
    let fetching_ids = {
        ...
        let iter = pprice_data[0..2]
            .iter()
            .map(|d| (mocked_store_ids[0], d.product_type.clone(), d.product_id));
        out.extend(iter);
        let iter = pprice_data[2..]
            .iter()
            .map(|d| (mocked_store_ids[1], d.product_type.clone(), d.product_id));
        out.extend(iter);
        out
    };
    ...
    if let Ok(fetched) = result {
        ...
        if let Some(ppset) = result {
            ...
            let pp = ppset
                .items
                .iter()
                .find(|d| d.product_id == 1002 && d.product_type == ProductType::Package);
            assert!(pp.is_some());
        }
