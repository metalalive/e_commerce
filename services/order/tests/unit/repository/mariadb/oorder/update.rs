...
use order::model::{
    StockLevelModelSet, OrderLineModelSet, OrderLineModel, OrderLineIdentity
};
...
async fn update_payment_ok()
{
    ...
    {
        let lines = vec![
            (1032, ProductType::Package, 9010, 13, 99, create_time.clone()),
    ...
    let data = {
        let lines = vec![
            OrderLinePaidUpdateDto { seller_id: 1031, product_id: 9003,  qty: 4,
                product_type: ProductType::Item, time: create_time + Duration::seconds(5)  },
            OrderLinePaidUpdateDto { seller_id: 1032, product_id: 9010,  qty: 1,
                product_type: ProductType::Package, time: create_time + Duration::seconds(6)  },
        ];
        OrderPaymentUpdateDto { oid: mock_oid.to_string(), lines }
    };
    let result = o_repo.update_lines_payment(data, OrderLineModel::update_payments).await;
    assert!(result.is_ok());
    ...
    let data = {
        let lines = vec![
            OrderLinePaidUpdateDto { seller_id: 1032, product_id: 9010,  qty: 3,
                product_type: ProductType::Package, time: create_time + Duration::seconds(7)  },
            OrderLinePaidUpdateDto { seller_id: 1032, product_id: 9011,  qty: 5,
                product_type: ProductType::Item, time: create_time + Duration::seconds(10)  },
        ];
        OrderPaymentUpdateDto { oid: mock_oid.to_string(), lines }
    };
    let result = o_repo.update_lines_payment(data, OrderLineModel::update_payments).await;
    ...
    let pids = vec![
        OrderLineIdentity {store_id:1031, product_id:9003, product_type:ProductType::Item},
        OrderLineIdentity {store_id:1032, product_id:9011, product_type:ProductType::Item},
        OrderLineIdentity {store_id:1032, product_id:9010, product_type:ProductType::Package},
    ];
