...
async fn save_fetch_ok()
{
    ...
    assert!(result.is_ok());
    let result = repo.fetch(store_id, vec![(ProductType::Package, 1002),
                                           (ProductType::Item, 1003) ]).await;
    assert!(result.is_ok());
    if let Ok(ms) = result {
        ...
        ms.items.into_iter().map(|m| {
            let expect = match &m.product_id {
                ...
                _others => (0u32, "1997-07-31T23:59:59+00:00", "1997-07-31T23:59:59+00:00"),
            };
    ...
    if let Ok(ms) = result {
        ...
        ms.items.into_iter().map(|m| {
            let expect = match &m.product_id {
                1002 => (515,  "2023-09-10T11:14:54+07:00", "2023-10-07T09:01:30+06:00"),
                1005 => (1389, "2023-07-29T10:17:04+05:00", "2023-10-06T09:01:32-06:30"),
                _others => (0u32, "1997-07-31T23:59:59+00:00", "1997-07-31T23:59:59+00:00"),
            };
            let actual = (m.price, m.start_after, m.end_before);
            assert_eq!(expect.0, actual.0);
