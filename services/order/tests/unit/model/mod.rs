    ...
    -> Option<&'a ProductStockModel>
{
    ...
    let result = actual.stores.iter().find(|m| {m.store_id == expect_store.store_id});
    assert!(result.is_some());
    if let Some(actual_st) = result {
        let expect_product = &expect_store.products[idx[1]];
        let result = actual_st.products.iter().find(|m| {
            m.type_ == expect_product.type_ &&  m.id_ == expect_product.id_
                && m.expiry_without_millis() == expect_product.expiry_without_millis()
        });
        assert!(result.is_some());
        if let Some(actual_prod) = result {
            if use_eq_op { assert_eq!(actual_prod, expect_product); }
            else { assert_ne!(actual_prod, expect_product); }
            Some(actual_prod)
        } else { None }
    } else { None }
