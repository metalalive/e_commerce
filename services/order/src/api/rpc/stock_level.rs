...
pub(super) async fn inventory_edit(req:AppRpcClientReqProperty, shr_state:AppSharedState)
    -> Vec<u8>
{
    common_setup!(Vec<InventoryEditStockLevelDto>, shr_state,
                  req.msgbody.as_slice(), StockLevelUseCase::try_edit)
}

pub(super) async fn inventory_return_cancelled(req:AppRpcClientReqProperty, shr_state:AppSharedState)
    -> Vec<u8>
{
    common_setup!(StockLevelReturnDto, shr_state,
                  req.msgbody.as_slice(), StockLevelUseCase::try_return)
}
