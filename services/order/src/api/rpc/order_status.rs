...
use crate::usecase::{
    ...
    OrderDiscardUnpaidItemsUseCase, OrderReplicaRefundUseCase
};

use super::build_error_response;
use super::dto::{
    OrderReplicaPaymentReqDto, OrderReplicaInventoryReqDto, OrderPaymentUpdateDto, OrderReplicaRefundReqDto
};
...
{
    match common_setup!(OrderReplicaPaymentReqDto, shr_state,
...
pub(super) async fn read_cancelled_refund (req:AppRpcClientReqProperty,
                                           shr_state:AppSharedState) -> Vec<u8>
{
    match common_setup!(OrderReplicaRefundReqDto, shr_state,
                        app_repo_order_return, req.msgbody.as_slice()) {
        Ok((v, repo)) => {
                                             ...
                                             shr_state:AppSharedState) -> Vec<u8>
{
    ...
    match common_setup!(OrderReplicaInventoryReqDto, shr_state,
                        app_repo_order, req.msgbody.as_slice())
    {
        Ok((v, o_repo)) => {
...
pub(super) async fn update_paid_lines (req:AppRpcClientReqProperty,
                                       shr_state:AppSharedState) -> Vec<u8>
{
    match common_setup!(OrderPaymentUpdateDto, shr_state,
                        app_repo_order, req.msgbody.as_slice())
    {
        Ok((v, repo)) => {
...
{ // it is invoked by scheduled job, no message in the RPC request
    match common_setup!(JsnVal, shr_state,
                        app_repo_order, req.msgbody.as_slice())
    {
        Ok((_v, repo)) => {
            let logctx = shr_state.log_context().clone();
