...
use sqlx::{MySql, Executor, IntoArguments, Arguments, Statement, Acquire, Row};
...
impl InsertLineArg {
    fn sql_pattern(num_batch:usize) -> String {
        let col_seq = (0..num_batch).into_iter().map(
            |_| "(?,?,?,?,?,?)"
        ).collect::<Vec<_>>().join(",") ;
        format!("INSERT INTO `cart_line_detail`(`usr_id`,`seq`,`store_id`,`product_type`,\
...
impl<'q> IntoArguments<'q, MySql> for InsertLineArg {
    fn into_arguments(self) -> <MySql as sqlx::database::HasArguments<'q>>::Arguments
    {
        ...
        lines.into_iter().map(|line| {
            ...
            let BaseProductIdentity { store_id, product_type, product_id } = id_ ;
...
impl UpdateLineArg {
    fn sql_pattern(num_batch:usize) -> String {
        let case_op = (0..num_batch).into_iter().map(
            |_| "WHEN (`store_id`=? AND `product_type`=? AND `product_id`=?) THEN ? "
        ).collect::<Vec<_>>().join("");
        let where_op = (0..num_batch).into_iter().map(
            |_| "(`store_id`=? AND `product_type`=? AND `product_id`=?)"
        ).collect::<Vec<_>>().join("OR");
        // `usr_id`,`seq`,`store_id`,`product_type`,`product_id`
        format!("UPDATE `cart_line_detail` SET \
...
impl TryFrom<MySqlRow> for CartModel {
    ...
    fn try_from(row: MySqlRow) -> DefaultResult<Self, Self::Error> {
        ...
        Ok(Self { owner, seq_num, title, saved_lines: Vec::new(),
            new_lines: Vec::new() })
    }
}
impl TryFrom<MySqlRow> for CartLineModel {
    ...
    fn try_from(row: MySqlRow) -> DefaultResult<Self, Self::Error> {
        ...
        Ok(Self { id_: BaseProductIdentity { store_id, product_type,
            product_id }, qty_req })
    }
}
...
impl AbsCartRepo for CartMariaDbRepo
{
    ...
    async fn fetch_cart(&self, owner: u32, seq:u8) -> DefaultResult<CartModel, AppError>
    {
        ...
        let out = self.fetch_common(owner, seq, rawsql_toplvl, rawsql_line).await?;
        Ok(out)
...
impl CartMariaDbRepo {
    ...
    async fn fetch_common(
        &self, owner: u32, seq:u8, rawsql_toplvl: (String, MySqlArguments),
    ...
    ) -> DefaultResult<CartModel, AppError>
    {
        ...
        if let Some(row) = result {
            ...
            let mut errors = rows.into_iter().filter_map(|row| {
                match CartLineModel::try_from(row) {
                    Ok(v) => { cart.saved_lines.push(v); None },
                    Err(e) => Some(e),
                }
            }).collect::<Vec<_>>();
            if errors.is_empty() { Ok(cart) }
            else { Err(errors.remove(0)) }
        } else {
            Ok(CartModel { owner, seq_num: seq, title: "Untitled".to_string(),
                saved_lines: Vec::new(), new_lines: Vec::new() })
        }
    }
} // end of impl CartMariaDbRepo
