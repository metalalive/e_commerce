...
mod CartLineTable {
    ...
    fn pkey(usr_id:u32, seq:u8, id_:BaseProductIdentity) -> String
    {
        ...
        format!("{}-{}-{}-{}-{}", usr_id, seq, id_.store_id,
                prod_typ_num, id_.product_id)
    }

    impl Into<AppInMemFetchedSingleTable> for UpdateArg {
        fn into(self) -> AppInMemFetchedSingleTable {
            let (usr_id, seq, mut saved_lines, new_lines) = (
                self.0.owner, self.0.seq_num, self.0.saved_lines, self.0.new_lines
            );
...
impl AbsDStoreFilterKeyOp for InnerFilterKeyOp {
    fn filter(&self, k:&String, _v:&Vec<String>) -> bool {
        ...
        if let Some(prod_ids) = self.pids.as_ref() {
            ...
            let curr_p_id = BaseProductIdentity {store_id, product_type, product_id};
...
impl TryFrom<(String, Vec<String>)> for CartLineModel {
    ...
    fn try_from(value: (String, Vec<String>)) -> DefaultResult<Self, Self::Error>
    {
        ...
        let out = CartLineModel { id_: BaseProductIdentity { store_id, product_type,
            product_id }, qty_req };
        Ok(out)
    }
}
...
impl From<(String, Vec<String>, Vec<CartLineModel>)> for CartModel {
    fn from(value: (String, Vec<String>, Vec<CartLineModel>))-> Self
    {
        ...
        CartModel { owner, seq_num, title: row.remove(0),
            saved_lines, new_lines:Vec::new() }
    }
}
...
impl CartInMemRepo
{
    ...
    async fn filter_keys(&self, usr_id: u32, seq_num:u8, pids:Option<Vec<BaseProductIdentity>>)
        -> DefaultResult<AppInMemFetchKeys, AppError>
    {
        let mut op = InnerFilterKeyOp {usr_id, seq_num, pids};
    ...
    async fn fetch_common(&self, owner: u32, seq:u8, keys:AppInMemFetchKeys)
        -> DefaultResult<CartModel, AppError>
    {
        ...
        let m_lines = rows_lines.into_iter().filter_map(|(k, v)|
            match CartLineModel::try_from((k, v)) {
                Ok(m) => Some(m),
        ...
        if errors.is_empty() {
            let obj = if let Some((k,v)) = rows_toplvl.into_iter().next() {
                ...
                CartModel {owner, seq_num:seq, title:"Untitled".to_string(),
                   new_lines:Vec::new(), saved_lines:Vec::new() }
            };
            Ok(obj)
        } else {
            Err(errors.remove(0))
