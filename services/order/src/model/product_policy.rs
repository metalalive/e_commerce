...
use crate::api::web::dto::{
    ProductPolicyDto, ProductPolicyClientErrorDto, ProductPolicyClientLimitDto, ProductPolicyNumRsvLimitDto
};
...
impl ProductPolicyModelSet
{
    pub fn validate (newdata:&Vec<ProductPolicyDto>) -> DefaultResult<(), Vec<ProductPolicyClientErrorDto>>
    {
        if newdata.is_empty() {
            let ce = ProductPolicyClientErrorDto {product_type:ProductType::Unknown(0),
                product_id: 0u64, auto_cancel_secs: None, warranty_hours:None, num_rsv:None,
        ...
        let detected_invalid = newdata.iter().filter_map(|item| {
            let auto_cancel_secs = if item.auto_cancel_secs > HARD_LIMIT_AUTO_CANCEL_SECS {
                Some(ProductPolicyClientLimitDto { given:item.auto_cancel_secs,
                    limit:HARD_LIMIT_AUTO_CANCEL_SECS })
            } else { None };
            let warranty_hours = if item.warranty_hours > HARD_LIMIT_WARRANTY_HOURS {
                Some(ProductPolicyClientLimitDto { given:item.warranty_hours,
            ...
            let num_rsv = if min_num_rsv > max_num_rsv {
                Some(ProductPolicyNumRsvLimitDto { min_items: min_num_rsv.clone(),
            ...
            if num_rsv.is_some() || auto_cancel_secs.is_some() || warranty_hours.is_some()
            {
                let ce = ProductPolicyClientErrorDto {
                    ...
                    auto_cancel_secs, warranty_hours, num_rsv, 
    ...
    {
        let zero_num_rsv = 0u16;
        let mut _new_objs = newdata.into_iter().filter_map(|mut item| {
            ...
            if let Some(obj) = result {
                ...
                Some(ProductPolicyModel {
                    is_create: true, product_id: item.product_id, max_num_rsv,
                    product_type: item.product_type, min_num_rsv,
