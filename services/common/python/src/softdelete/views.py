...
class RecoveryModelMixin:
    ...
    def _get_time_range(self, first_cset, time_start=""):
        ...
        if time_start in EMPTY_VALUES:  # only recover last deletion
            ...
            try:  # the acceptable format is YYYY-MM-DD-hh-mm
                ...
                clean_time_start = {
                    self.TIME_UNIT[idx]: clean_time_start[idx]
                    for idx in range(len(self.TIME_UNIT))
                }
                ...
                if diff < self.ZERO_TIME_DELTA:  # negative delta
                    ...
                    errmsg = "the range must be less than {td}".format(
                        td=str(self.MAX_TIME_DELTA)
                    )
                    raise ValueError(errmsg)
                ...
                if isinstance(e, IndexError):
                    ...
                    errmsg = str(e)
                msg = "invalid start time {tf}, {errmsg}".format(
                    tf=time_start, errmsg=errmsg
                )
                log_args.extend(["msg", msg])
                _logger.info(None, *log_args, request=self.request)
                raise ValidationError(
                    message=msg,
                    params={"status": RestStatus.HTTP_400_BAD_REQUEST},
                )
    ...
    def recovery(
        ...
        profile_id,
        *args,
        status_ok=RestStatus.HTTP_200_OK,
            ...
            if isinstance(body, dict):
                ...
                if any(ids):
                    status, return_msg, affected_items, loglevel = self._recover_by_id(
                        ...
                        status_ok=status_ok,
                        return_data_after_done=return_data_after_done,
                        log_args=log_args,
                    )
                else:
                    status, return_msg, affected_items, loglevel = (
                        self._recover_by_newest_deletion(
    ...
    def _recover_by_newest_deletion(
        self,
        model_cls,
        ...
        affected_items = None
        first_cset = (
            self.SOFTDELETE_CHANGESET_MODEL.objects.filter(
                content_type=resource_content_type.pk, done_by=profile_id
            )
        ...
        if first_cset:
            ...
            _id_list = []
            try:
                delta_start, delta_end = self._get_time_range(
                    first_cset=first_cset, time_start=time_start
                )
                # recover items in opposite order in which they were deleted
                cset = self.SOFTDELETE_CHANGESET_MODEL.objects.filter(
                    content_type=resource_content_type.pk,
                    time_created__lt=delta_end,
                    done_by=profile_id,
                    time_created__gt=delta_start,
                ).order_by("-time_created")
                _id_list = cset.values_list("object_id", flat=True)
                _id_list = list(
                    _id_list
                )  # do not do this after un-delete the instances
                ...
                status = (
                    RestStatus.HTTP_400_BAD_REQUEST
                )  ## e.params.get('status', status)
    ...
    def _recover_by_id(
        self, model_cls, ids, profile_id, status_ok, return_data_after_done, log_args
    ):
        status = status_ok
        loglevel = logging.INFO
        affected_items = None
