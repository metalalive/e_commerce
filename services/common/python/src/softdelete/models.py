...
class ChangeSet(models.Model, _SoftDeleteIdSerializerMixin):
    ...
    content_type = models.ForeignKey(
        ContentType, db_column="content_type", on_delete=models.CASCADE
    )
...
class SoftDeleteObjectMixin(models.Model):
    ...
    _changeset_not_found_err_msg = (
        "changeset not found and NOT allowed to create new one"
    )
    ...
    def _delete_relations(self, related_fields, *args, **kwargs):
        ...
        for f in related_fields:
            ...
            if not hasattr(
                self, rel_attr
            ):  # some fields exist in the model instance but not accessible
                continue
    ...
    def delete(self, *args, **kwargs):
        ...
        if hard:
            ...
            cs = kwargs.get("changeset", None) or self.determine_change_set(
                profile_id=profile_id
            )
            kwargs["changeset"] = cs
            self.SOFTDELETE_RECORD_MODEL.objects.get_or_create(
                changeset=cs,
    ...
    def undelete(self, *args, **kwargs):
        ...
        for d in discarded:
            model_cls = (
                d.content_type.model_class()
            )  # model_cls must inherit SoftDeleteObjectMixin
    ...
    def determine_change_set(self, profile_id, create=True):
        ...
        loglevel = logging.DEBUG
        try:
            serialized_object_id = self.SOFTDELETE_RECORD_MODEL.serialize_obj_id(
                value=self.pk
            )
            qs = self.SOFTDELETE_RECORD_MODEL.objects.filter(
                content_type=content_type,
                object_id=serialized_object_id,
            ...
            serialized_object_id = self.SOFTDELETE_CHANGESET_MODEL.serialize_obj_id(
                value=self.pk
            )
            try:
                qs = self.SOFTDELETE_CHANGESET_MODEL.objects.filter(
                    content_type=content_type,
                    object_id=serialized_object_id,
                    done_by=profile_id,
                )
                ...
                if create:
                    qs = self.SOFTDELETE_CHANGESET_MODEL.objects.create(
                        content_type=content_type,
                        object_id=serialized_object_id,
                        done_by=profile_id,
                    )
