...
def _render_perms_sql(model_cls):
    ...
    insert_pattern = insert_pattern.format(
        db_table=ContentType._meta.db_table, app_label=model_cls._meta.app_label
    )
    delete_pattern = delete_pattern.format(
        db_table=ContentType._meta.db_table, app_label=model_cls._meta.app_label
    )
    ops = []
...
class Migration(migrations.Migration):
    ...
    def _load_ops(cls):
        ...
        insert_contenttype_pattern = insert_contenttype_pattern.format(
            db_table=ContentType._meta.db_table,
            app_label=ProductSaleableItem._meta.app_label,
        )
        delete_contenttype_pattern = "DELETE FROM `{db_table}` WHERE `app_label` = '{app_label}' AND `model` = '%s'"
        delete_contenttype_pattern = delete_contenttype_pattern.format(
            db_table=ContentType._meta.db_table,
            app_label=ProductSaleableItem._meta.app_label,
        )
        extra_ops = [
