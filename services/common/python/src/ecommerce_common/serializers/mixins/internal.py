...
class SerializerExcludeFieldsMixin:
            ...
            for fd in getattr(self, exc_fd_name):
                ...
                if len(hier_names) == 1:
                    ...
                    if _field is None or not isinstance(_field, BaseSerializer):
                        continue
                    _field = (
                        _field.child if isinstance(_field, ListSerializer) else _field
                    )
                    if not hasattr(_field, exc_fd_name) or not getattr(
                        _field, exc_fd_name
                    ):
                        setattr(_field, exc_fd_name, [])
                    getattr(_field, exc_fd_name).append(
                        self.DELIMITOR.join(hier_names[1:])
                    )
    ...
    def exclude_read_fields(self):
        ...
        if not allowed_fields:
            return
        self._common_exclude_fields(
            allowed_fields=allowed_fields, exc_fd_name="exc_rd_fields"
        )
    ...
    def exclude_write_fields(self):
        ...
        self._common_exclude_fields(
            allowed_fields=allowed_fields, exc_fd_name="exc_wr_fields"
        )
...
class ValidationErrorCallbackMixin:
    def run_validation(self, data=empty):
        try:
            ...
            if (
                hasattr(target, "_validation_error_callback")
                and target._validation_error_callback
            ):
