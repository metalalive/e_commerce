        ...
        if fetch_all:
            ...
            if IDs:
                try:
                    ...
                    FieldError,
                    ValueError,
                ) as e:  # invalid data type in ID list, or invalid field name
...
class BulkCreateModelMixin(CreateModelMixin, UserEditViewLogMixin):
    ...
    def create(
        ...
        **kwargs
        ...
        self.log_action(
            action_type="create", request=request, many=many, serializer=serializer
        )
        response = RestResponse(
            return_data, status=RestStatus.HTTP_201_CREATED, headers=headers
        )
        return response
    ...
    def update(
        ...
        **kwargs
        ...
        if many:
            ...
            instance = self.get_object(
                pk_field_name=pk_field_name, skip_if_none=allow_create
            )
        ...
        if not instance and not allow_create:
            ...
            if return_data_after_done:
                ...
                return_data = [] if many else {}
            self.log_action(
                action_type="update", request=request, many=many, serializer=serializer
            )
...
class BulkDestroyModelMixin(DestroyModelMixin, UserEditViewLogMixin):
    def destroy(
        ...
        *args,
        ...
        pk_field_name="id",
        pk_skip_list=None,
        status_ok=RestStatus.HTTP_204_NO_CONTENT,
        status_err=RestStatus.HTTP_400_BAD_REQUEST,
        err_msg="not specify any ID, no object is deleted",
        **kwargs
    ):
        extra_log_kwargs = {}
        if many:
            ...
            model_cls = (
                instance.model
            )  # get everything ready before they're deleted for logging
    ...
    def perform_destroy(self, instance, id_list):
        ...
        is_softdelete = isinstance(
            instance, (SoftDeleteObjectMixin, SoftDeleteQuerySet)
        )
