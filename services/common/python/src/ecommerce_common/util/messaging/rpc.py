...
from .constants import (
    MSG_PAYLOAD_DEFAULT_CONTENT_TYPE,
    AMQP_SSL_CONFIG_KEY,
    SERIALIZER_CONFIG_KEY,
    DEFAULT_SERIALIZER,
    AMQP_EXCHANGE_NAME_CONFIG_KEY,
    AMQP_EXCHANGE_TYPE_CONFIG_KEY,
    RPC_EXCHANGE_DEFAULT_NAME,
    RPC_EXCHANGE_DEFAULT_TYPE,
    RPC_ROUTE_KEY_PATTERN_SEND,
)
...
class ReplyListener:
    ...
    def __init__(
        ...
        log_args = [
            "type",
            "python.messaging.rpc.ReplyListener",
            "reply_queue",
            queue_name,
            "msg_ttl_secs",
            str(msg_ttl),
        ...
        self.queue = KombuQueue(
            ...
            queue_arguments={"x-message-ttl": msg_ttl},
            auto_delete=False,
        )
...
class RpcReplyEvent:
    ...
    valid_finish_status = [
        status_opt.FAIL_CONN,
        status_opt.FAIL_PUBLISH,
        status_opt.REMOTE_ERROR,
        status_opt.SUCCESS,
    ]
    ...
    def refresh(self, retry=False, num_of_msgs_fetch=None, timeout=0.5):
        ...
        if _timeout is False and self.finished is False:
            err = self._listener.refresh_reply_events(
                num_of_msgs_fetch=num_of_msgs_fetch, timeout=timeout
            )
            _timeout = self.timeout
...
class RPCproxy:
    ...
    def __init__(
        ...
        **options
        ...
        self._rpc_reply_listener = ReplyListener(
            broker_url=_default_msg_broker_url,
...
class MethodProxy:
    ...
    def __init__(
        ...
        **options
        ...
        self._publisher = self.publisher_cls(
            amqp_uri=broker_url, serializer=serializer, ssl=self.ssl, **options
        )
    ...
    def _call(self, custom_payld: Optional[Any], *args, **kwargs):
        ...
        try:
            ...
            with get_connection(
                amqp_uri=self._broker_url,
                ssl=self.ssl,
                transport_options=extra_transport_opts,
            ) as conn:
