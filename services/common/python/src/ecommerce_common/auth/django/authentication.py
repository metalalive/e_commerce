    ...
    def authenticate(self, request):
        ...
        if encoded_rfr_tok:  # TODO, move keystore to global shared context
            _keystore = create_keystore_helper(
                cfg=django_settings.AUTH_KEYSTORE, import_fn=import_module_string
            )
            try:
                rfr_jwt = JWT()
                result = rfr_jwt.verify(
                    unverified=encoded_rfr_tok, audience=None, keystore=_keystore
                )
            except (
                DecodeError,
                ExpiredSignatureError,
...
class AccessJWTauthentication(DjangoGetProfileMixin):
    ...
    * user -- The user to which the token belongs
    ...
    def authenticate(self, request):
        ...
        audience = filter(
            lambda kv: kv[1] == host_name, cors_conf.ALLOWED_ORIGIN.items()
        )
    ...
    def authenticate_credentials(self, encoded_acs_tok, audience):
        ...
        try:
            acs_tok = JWT()
            result = acs_tok.verify(
                unverified=encoded_acs_tok, audience=audience, keystore=_keystore
            )
        except (
            DecodeError,
            ExpiredSignatureError,
...
class RemoteAccessJWTauthentication(AccessJWTauthentication):
    def authenticate_credentials(self, encoded_acs_tok, audience):
        account, payld_verified = super().authenticate_credentials(
            encoded_acs_tok=encoded_acs_tok, audience=audience
        )
