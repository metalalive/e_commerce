...
def monkeypatch_sqlcompiler():
    ...
    def patched_get_default_columns(
        self, start_alias=None, opts=None, from_parent=None
    ):
        result = old_get_default_columns(
            self, start_alias=start_alias, opts=opts, from_parent=from_parent
        )
    ...
    def patched_find_ordering_name(
...
def monkeypatch_sqlquery_build_filter():
        ...
        if isinstance(col, Iterable) and not isinstance(col, str):
            ...
            if isinstance(value, (list, tuple)) and "in" in lookups:
                ...
                for val_item in value:
                    nested_where_clause = self.where_class(
                        connector=OR, negated=current_negated
                    )
                    self._build_filter_single_composite_pkey(
                        value=val_item,
                        col=col,
                        parent_related_fields=parent_related_fields,
                        clause_out=nested_where_clause,
                        valid_fd_names=valid_fd_names,
                    )
            ...
            clause.add(condition_, AND)
            require_outer = self._build_filter_isnull_outer(
    ...
    def _build_filter_single_composite_pkey(
        self, value: dict, col, parent_related_fields, clause_out, valid_fd_names
    ):
        ...
        for nested_arg, nested_value in value.items():
            ...
            full_path_field.pop()
            lookup, fd_name, is_valid_name = self._build_filter_validate_fd_name(
                nested_arg, valid_fd_names
            )
            if not is_valid_name:
                raise FieldError(
                    "Multi-column Key Field got invalid column names : {}".format(
                        nested_arg
                    )
                )
    ...
    def _build_filter_isnull_outer(
        self, condition_, current_negated, clause, targets, join_list, join_info, alias
    ):
        lookup_type = condition_.lookup_name
        require_outer = (
            lookup_type == "isnull" and condition_.rhs is True and not current_negated
        )
        if (
            current_negated
            and (lookup_type != "isnull" or condition_.rhs is False)
            and condition_.rhs is not None
        ):
            require_outer = True
    ...
    def _build_filter_validate_fd_name(self, nested_arg, valid_field_names):
        ...
        if (
            parts
        ):  # `parts` includes optional list of related fields to final non-relation field
        ...
        try:
            ...
            for name in field_names:
                ...
                for target in targets:
                    ...
                    if isinstance(
                        col, (list, tuple)
                    ):  # implicitly means a multi-column key
                        cols.extend(col)
                    else:
                        cols.append(col)
            if cols:
...
def monkeypatch_deferred_attr():
    ...
    def patched_get_attr(self, instance, cls=None):
        ...
        if self.field.get_internal_type() == "CompoundPrimaryKeyField":
            return _get_composite_pk_val(
                model_instance=instance, field_instance=self.field
            )
...
def monkeypatch_django_model_ops():
    ...
    def patched_save_base(self, *args, **kwargs):
        ...
        if pk_field.get_internal_type() == "CompoundPrimaryKeyField":
            self.pk = _get_composite_pk_val(
                model_instance=self, field_instance=pk_field
            )
        return result
    ...
    def _update_composite_pk_after_bulk_ops(objs, only_for_nonpk):
        ...
        if pk_field.get_internal_type() == "CompoundPrimaryKeyField":
            ...
            for model_obj in filtered_objs:
                ...
                model_obj.pk = _get_composite_pk_val(
                    model_instance=model_obj, field_instance=field_obj
                )
...
def monkeypatch_queryset_lookup_values():
            ...
            for rel_fd in related_field_chain:
                ...
                if isinstance(fd_descriptor, (ReverseManyToOneDescriptor,)):
                    ...
                    if rel_fd == related_field_chain[-1]:
                        ...
                        is_compo_pk = (
                            pk_field.get_internal_type() == "CompoundPrimaryKeyField"
                        )
                        if is_compo_pk:
                            compo_fnames = [
                                f.attname for f in pk_field._composite_fields
                            ]
                            compo_fnames = list(
                                map(_extract_compo_pk_cols_fn, compo_fnames)
                            )
    ...
    def patched__values_fn(self, *fields, **expressions):
        ...
        if is_compo_pk:
            if (
                not fields
            ):  # note : don't use set, use tuple to remain the order of fields
            ...
            replaced = [
                compo_fnames if fname in (pk_field.attname, "pk") else fname
...
class CompoundPrimaryKeyField(models.Field):
    ...
    def __init__(self, inc_fields, **kwargs):
        ...
        self.db_column = (
            self._composite_columns()
        )  # TODO, seperate db columns from field name
    ...
    def get_col(self, alias, output_field=None):
        ...
        if alias != self.model._meta.db_table or output_field != self:
            gen_col_helper = partial(
                _create_col, alias=alias, output_field=output_field
            )
    ...
    def cached_col(self):
        gen_col_helper = partial(
            _create_col, alias=self.model._meta.db_table, output_field=None
        )
        return list(map(gen_col_helper, self._composite_fields))
