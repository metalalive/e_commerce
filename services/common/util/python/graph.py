...
def depth_first_search(graph: dict, node, visited: dict, parent: int, is_directed, options):
...
def is_graph_cyclic(graph: dict, is_directed=False):
    ...
    while True:
        ...
        depth_first_search(graph=graph, node=node_id, visited=visited, parent=-1,
                is_directed=False, options=subgraph)
    ...
    if result is True and is_directed is True:
        ...
        result = path_exists(graph=graph, node_src=node_start,
                node_dst=node_start, is_directed=True)
        if result is False:
            loop_start_node = None
...
def path_exists(graph: dict, node_src, node_dst, is_directed:bool = True):
    assert node_src is not None and node_dst is not None, \
            'both of node_src and node_dst have to be non-null value'
    visited  = {k: False for k in graph.keys()}
    subgraph = {"has_cycle": False, "node_visited":[]}
    node_id = node_src
    depth_first_search(graph=graph, node=node_id, visited=visited, parent=-1,
            is_directed=is_directed, options=subgraph)
