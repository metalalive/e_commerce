...
from common.util.python import util_os
...
class AbstractStorage:
    ...
    def _save_sanity_check(self, path, content):
        ...
        content_attr_check = (content is not None) and hasattr(content, "read") \
                and hasattr(content, "seek") and hasattr(content, "close") \
        ...
        if hasattr(content, "closed"):
            assert not content.closed, "the input content should be a opened file-like object"
...
def _fs_storage_save_common(func):
    ...
    is_async_fn = inspect.isasyncgenfunction(func) # don't use asyncio.iscoroutinefunction(func) 

    @functools.wraps(func) # TODO, figure out the way to refactor this function
    async def inner(self, path, content, chunk_sz = 1024 * 4, non_file_types=None, alt_path_autogen=True):
        ...
        while True:
            ...
            try:
                ...
                if getattr(content, 'name', None) and not isinstance(content, tuple(all_non_file_types)):
                    ...
                    try:
                        ...
                        if is_async_fn: # reset read pointer
                            ...
                            next(_generator)
                        while True:
                            if is_async_fn: # read chunk of data, or raise StopIteration
    ...
    def sync_wrapper_inner(self, path, content, chunk_sz = 1024 * 4, alt_path_autogen=True, non_file_types=None):
        coroutine = inner(self=self, path=path, content=content, chunk_sz=chunk_sz,
                alt_path_autogen=alt_path_autogen,  non_file_types=non_file_types)
...
class FileSystemStorage(AbstractStorage):
    OS_OPEN_FLAGS = builtin_os.O_WRONLY | builtin_os.O_CREAT | builtin_os.O_EXCL \
    ...
    def __init__(self, location, directory_permissions_mode=None):
        ...
        assert builtin_os.path.exists(location), "Base location of the file-system storage does NOT exist"
        acl_required = builtin_os.R_OK | builtin_os.W_OK | builtin_os.X_OK | builtin_os.F_OK
        assert builtin_os.access(location, acl_required), "Current application does NOT have \
