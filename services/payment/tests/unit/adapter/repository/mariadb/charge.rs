...
use chrono::{Duration, Local, SubsecRound, DateTime, FixedOffset};
...
fn ut_verify_fetched_order(
    ...
    actual.lines.into_iter().map(|line| {
        ...
        let actual_val = (
            ...
            paid_total.total, paid_total.qty],
            reserved_until
        );
...
async fn create_order_replica_ok() {
    let mock_order_toplvl_data = (
        123, "9d73ba76d5", 0, Local::now().fixed_offset()
    );
    let mock_olines_data = 
        vec![
            (
                2603,
                ProductType::Item,
    ...
    assert!(result.is_ok());
    let result = repo.get_unpaid_olines(
        mock_order_toplvl_data.0,
        mock_order_toplvl_data.1,
    ).await;
...
async fn read_unpaid_orderline_ok() {
    ...
    // repository
    let mock_order_toplvl_data = (
        124, "c071ce550de1", 2, Local::now().fixed_offset()
    );
    let mock_olines_data = 
        vec![
            (
    ...
    if let Ok(Some(v)) = result {
        let mock_olines_data = mock_olines_data.into_iter().filter(
            |c| c.3[2] > c.3[4]
