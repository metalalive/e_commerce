...
use crate::adapter::processor::{AppProcessorContext, AppProcessorError};
...
pub struct AppSharedState {
    ...
    _processors: Arc<AppProcessorContext>,
...
impl AppSharedState {
    pub fn new(cfg: AppConfig) -> Result<Self, ShrStateInitError> {
        ...
        let _processors = {
            let proc = AppProcessorContext::new(logctx.clone())?;
            Arc::new(proc)
        };
        Ok(Self {
            _config: Arc::new(cfg),
    ...
    pub fn processor_context(&self) -> Arc<AppProcessorContext> {
