...
from common.auth.keystore import JWKSFilePersistHandler
from tests.python.common import capture_error
...
def _setup_keyfile(filepath):
    ...
    if not builtin_os.path.exists(filepath):
        ...
        with open(filepath, 'wb') as f:
            empty_json_obj = '%s%s' % (JWKSFilePersistHandler.JSONFILE_START_LINE, \
...
class FilePersistHandlerTestCase(unittest.TestCase):
    _init_kwargs = {
        ...
        'name':'test_secret_storage', 'expired_after_days': 11,
        'max_expired_after_days': 90, 'flush_threshold':4, 'auto_flush':False,
    }
    ...
    def tearDown(self):
        _teardown_keyfile( filepath=self._init_kwargs['filepath'], del_dir=self._dir_tear_down,
    ...
    def test_save_key_items(self):
        ...
        keydata = {
            "crypto-key-id-%s" % idx: {
                'exp': (today + timedelta(days=random.randrange(2,max_expired_after_days))).isoformat(),
                'alg': 'ALGORITHM_OPTION_%s' % random.randrange(0x1000, 0xffff),
                'kty': 'CRYPTO_KEY_TYPE_%s' % random.randrange(0x1000, 0xffff) ,
                'use': 'Signature',
            } for idx in range(4)
        }
        self.assertEqual( persist_handler.expired_after_days.days , self._init_kwargs['expired_after_days'] )
    ...
    def test_set_invalid_items(self):
        ...
        err = capture_error(testcase=self, err_cls=ValueError, exe_fn=exe_fn)
        expect_err_msg = 'user-specified expiry date %s exceeds maximum allowed value' % exceed_expiry_date.isoformat()
    ...
    def test_evict_expired_keys(self):
        ...
        keydata = dict(filter(lambda kv: date.fromisoformat(kv[1]['exp']) >= date_limit, keydata.items()))
        self.assertEqual(len(keydata.keys()), len(persist_handler))
        actual_data_gen = persist_handler.items(present_fields=['exp','kty','use','alg'])
