...
class TestCreateTag:
    ...
    async def collect_node_id(
        cls, client: ITestClient, usr_id: int, name: str, parent_id: Optional[int]
    ) -> Optional[int]:
    ...
    async def test_one_ok(self, mock_client):
        ...
        respbody = await create_one_tag(mock_client, mock_usr_id, reqbody, 201)
    ...
    async def test_multi_nodes_ok(self, mock_client):
        ...
        root_id = await cls.collect_node_id(
            mock_client, mock_usr_id, name="home building tool", parent_id=None
        )
        data = ["saw", "hammer"]
        layer1_ids = [
        ...
        rootnode_id = await cls.collect_node_id(
            mock_client, mock_usr_id, "electronic project kit", None
        )
...
class TestUpdateTag:
    async def update_one(
        client: ITestClient, usr_id: int, node_id: int, body: TagUpdateReqDto
    ) -> Dict:
        headers: Dict[str, str] = {}
        add_auth_header(client, headers, usr_id, ["change_producttag"])
    ...
    async def test_same_tree(self, mock_client):
        ...
        layer2_1_ids = [  # noqa: F841
            await fn_add_tag(mock_client, mock_usr_id, nm, layer1_ids[1]) for nm in data
        ]
        cls = type(self)
        reqbody = TagUpdateReqDto(name="toilet paper", parent=root_id)
        respbody = await cls.update_one(
            mock_client, mock_usr_id, node_id=layer2_0_ids[0], body=reqbody
        )
    ...
    async def test_move_to_new_tree(self, mock_client):
        ...
        respbody = await cls.update_one(
            mock_client, mock_usr_id, node_id=layer1_ids[0], body=reqbody
        )
...
class TestDeleteTag:
    ...
    async def test_ok(self, mock_client):
        ...
        rootnode_id = await TestCreateTag.collect_node_id(
            mock_client, mock_usr_id, name="household II", parent_id=None
        )
        data = ["bucket", "shovel"]
        ...
        cls = type(self)
        await cls.delete_one(mock_client, mock_usr_id, rootnode_id, 204)
