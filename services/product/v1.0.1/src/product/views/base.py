...
from ecommerce_common.views.filters import (
    ClosureTableFilter,
    AggregateFieldOrderingFilter,
)
...
from ..permissions import (
    TagsPermissions,
    AttributeTypePermissions,
    SaleableItemPermissions,
    SaleablePackagePermissions,
)
...
class TagSearchFilter(SearchFilter):
    def filter_queryset(self, request, queryset, view):
        ...
        if (
            origin_qs is not queryset
        ):  # which implicitly means new queryset contains search result.
            matched_ids = queryset.values_list("pk", flat=True)
            ascs_ids = origin_qs.get_ascs_descs_id(
                IDs=matched_ids,
                fetch_asc=True,
                fetch_desc=False,
                depth=-1,
...
class TagView(AuthCommonAPIView):
    ...
    def get_IDs(
        ...
        pk_field_name="pk",
        delimiter=",",
        pk_src=LimitQuerySetMixin.REQ_SRC_QUERY_PARAMS,
        pk_skip_list=None,
    ):
        IDs = super().get_IDs(
            pk_param_name=pk_param_name,
            pk_field_name=pk_field_name,
            delimiter=delimiter,
            pk_src=pk_src,
            pk_skip_list=pk_skip_list,
        )
        if self.request.method == "GET" and (self._fetch_desc or self._fetch_asc):
            depth = self.request.query_params.get("depth", "")
            IDs = self.queryset.get_ascs_descs_id(
                IDs=IDs,
                fetch_asc=self._fetch_asc,
                fetch_desc=self._fetch_desc,
                depth=depth,
            )
        # pdb.set_trace()
        return IDs
...
class AttributeTypeView(AuthCommonAPIView, RecoveryModelMixin):
    ...
    def patch(self, request, *args, **kwargs):
        kwargs["resource_content_type"] = ContentType.objects.get(
            app_label="product", model=self.serializer_class.Meta.model.__name__
        )
        kwargs["return_data_after_done"] = True
        return self.recovery(
            request=request, *args, profile_id=request.user.profile, **kwargs
        )


class TaggedSaleableView(AuthCommonAPIReadView):
    filter_backends = [
        BaseIngredientSearchFilter,
        OrderingFilter,
    ]
...
class SaleableItemView(SaleableBaseView):
    ...
    permission_classes = AuthCommonAPIView.permission_classes.copy() + [
        SaleableItemPermissions
    ]
    queryset = serializer_class.Meta.model.objects.all()
    ordering_fields = SaleableBaseView.ordering_fields.copy() + [
        "ingredients_applied__ingredient__category"
    ]
    search_fields = SaleableBaseView.search_fields.copy() + [
        "ingredients_applied__ingredient__name",
        "pkgs_applied__package__name",
    ]
...
class SaleablePackageView(SaleableBaseView):
    ...
    queryset = serializer_class.Meta.model.objects.all()
    search_fields = SaleableBaseView.search_fields.copy() + [
        "saleitems_applied__sale_item__name"
    ]
