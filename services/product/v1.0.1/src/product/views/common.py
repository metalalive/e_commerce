...
from ecommerce_common.views.filters import AdvancedSearchFilter
...
class BaseIngredientSearchFilter(AdvancedSearchFilter):
    def _create_leaf_node(self, _operator, operands, metadata=None):
        ...
        if operands and operands[0] in self._map_attr_label.keys():
            ...
            filtered_dtype = tuple(
                filter(lambda x: x[0] == metadata["dtype"], self._valid_attr_dtypes)
            )
            assert len(filtered_dtype) == 1, err_msg
            full_related_name = [
                filtered_dtype[0][1],
                self._map_attr_label[operands[0]],
            ]
            operands[0] = LOOKUP_SEP.join(full_related_name)
        return super()._create_leaf_node(
            _operator=_operator, operands=operands, metadata=metadata
        )

    def filter_queryset(self, request, queryset, view):
        frontend_attr_type_label = LOOKUP_SEP.join(
            [attribute_field_label["_list"], attribute_field_label["type"]]
        )
        frontend_attr_val_label = LOOKUP_SEP.join(
            [attribute_field_label["_list"], attribute_field_label["value"]]
        )
        self._map_attr_label = {
            frontend_attr_type_label: "attr_type",
            frontend_attr_val_label: "value",
        }
        self._valid_attr_dtypes = tuple(
            map(
                lambda x: getattr(_ProductAttrValueDataType, x)[0],
                _ProductAttrValueDataType.names,
            )
        )
