...
class AugmentIngredientRefMixin(AugmentEditFieldsMixin):
    _field_name_map = {
        "ingredient": "_ingredient_instance",
    }
...
class AttrValueListSerializer(AugmentIngredientRefMixin, BulkUpdateListSerializer):
    ...
    def _get_valid_types(self, src):
        ...
        def fn_gather_type_id(d):
            return d.get(_label["type"])
        ...
        try:
            ...
            def fn_find_rootcause(d):
                nonlocal ve
                return value_error_msg_pattern % (id_name, d.get("type")) == ve.args[0]

            rootcause = tuple(filter(fn_find_rootcause, src))
            if any(rootcause):
                err_detail = {
                    "data": rootcause,
                    "code": "query_invalid_data_type",
                    "model": model_cls.__name__,
                    "field": id_name,
                }
                errmsg = {
                    _label["_list"]: [
                        json.dumps(err_detail),
                    ]
                }
    ...
    def extract(self, data, dst_field_name):
        ...
        valid_types = valid_types.filter(
            dtype=self.child.Meta.model.DATATYPE.value[0][0]
        )
    ...
    def validate(self, value, _logger=None, exception_cls=Exception):
        ...
        if id_required:
            err_msg_pattern = [
                '{"message":"duplicate ID found in ',
                self.child.Meta.model.DATATYPE.label.lower(),
                ' attribute","value":%s}',
            ]
            err_msg_pattern = "".join(err_msg_pattern)
            unique_id_checker = UniqueListItemsValidator(
                fields=["id"],
                error_cls=DRFRequestDataConflictError,
                err_msg_pattern=err_msg_pattern,
            )
...
class AbstractAttrValueSerializer(ExtendedModelSerializer, NestedFieldSetupMixin):
    ...
    def __init__(self, instance=None, data=DRFEmptyData, **kwargs):
        ...
        self.fields["extra_amount"].validators.append(
    ...
    def to_representation(self, instance):
        ...
        if not stored_attrtype:
            log_msg = [
                ...
                "msg",
                "product attribute type must not be empty in type-value pair",
                "id",
                out.get("id", None),
                "value",
                out.get("value", None),
    ...
    def extra_setup_before_validation(self, instance, data):
        self.fields["extra_amount"].required = (
            data.get("extra_amount", None) is not None
        )
    ...
    def run_validation(self, data=DRFEmptyData):
        ...
        try:
            validated_data = super().run_validation(
            ...
            ve.detail["_origin"] = {
                src_field_label["value"]: _value,
                src_field_label["type"]: _type,
            }
            raise
...
class BaseIngredientListSerializer(BulkUpdateListSerializer):
    def validate(self, value, _logger=None, exception_cls=Exception):
        ...
        if id_required:
            unique_id_checker = UniqueListItemsValidator(
                fields=["id"], error_cls=DRFRequestDataConflictError
            )
            unique_id_checker(value=value, caller=self)
        return value
...
class BaseIngredientSerializer(ExtendedModelSerializer, NestedFieldSetupMixin):
    ...
    def __init__(self, instance=None, data=DRFEmptyData, **kwargs):
        ...
        self.fields["attr_val_str"] = AttrValueStrSerializer(
            many=True, instance=instance, data=data
        )
        self.fields["attr_val_pos_int"] = AttrValuePosIntSerializer(
            many=True, instance=instance, data=data
        )
        self.fields["attr_val_int"] = AttrValueIntSerializer(
            many=True, instance=instance, data=data
        )
        self.fields["attr_val_float"] = AttrValueFloatSerializer(
            many=True, instance=instance, data=data
        )
        super().__init__(instance=instance, data=data, **kwargs)
    ...
    def to_representation(self, instance):
        ...
        if (not presentable_fields_name) or (
            src_field_label["_list"] in presentable_fields_name
        ):
            gather_all_attrs = []
    ...
    def extra_setup_before_validation(self, instance, data):
        for s_name in self.Meta.nested_fields:
            try:
                ...
                if (
                    rootcause["code"] == "query_invalid_data_type"
                    and rootcause["model"] == ProductAttributeType.__name__
                    and rootcause["field"] == "id"
                ):
            ...
            self._setup_subform_instance(
                name=s_name, instance=instance, data=data, pk_field_name="id"
            )
            self.fields[s_name].append_ingredient(
                name=s_name, instance=instance, data=data
            )
    ...
    def _raise_unknown_attr_type_error(self, unclassified_attributes):
        ...
        for idx in range(self._num_attr_vals):
            item = tuple(
                filter(lambda x: x.get("_seq_num", -1) == idx, unclassified_attributes)
            )
            detail = {}
            if any(item):
                err_detail = RestErrorDetail(
                    "unclassified attribute type `%s`" % (item[0]["type"]),
                    code="unclassified_attributes",
                )
    ...
    def _err_detail_invalid_attr_value(self, error, data):
        ...
        if any(err_fields):
            ...
            for idx in range(self._num_attr_vals):
                items = tuple(
                    filter(lambda x: x["origin"].get("_seq_num", -1) == idx, err_fields)
                )
    ...
    def update(self, instance, validated_data):
        ...
        validated_subform_data = {
            k: validated_data.pop(k, []) for k in self.Meta.nested_fields
        }
