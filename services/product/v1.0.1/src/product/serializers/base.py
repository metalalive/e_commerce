...
class TagSerializer(BaseClosureNodeMixin, ExtendedModelSerializer):
    ...
    def validate(self, value):
        return super().validate(
            value=value, exception_cls=DjangoValidationError, _logger=_logger
        )
...
class CommonSaleableMediaMetaField(ListField):
    def __init__(self, *args, model=None, ingredient_field_name=None, **kwargs):
        assert (
            model and ingredient_field_name
        ), "both of class variables `model` and `ingredient_field_name` must NOT be null"
        self.model = model
    ...
    def to_representation(self, instance):
        assert (
            instance.model is self.model
        ), "model mismatch, failed to serialize media set"
    ...
    def update(self, validated_data, ingredient):
        ...
        if any(validated_data):
            ...
            def _contain_filter_fn(resource_id):
                return resource_id not in resource_ids

            discarding.delete(hard=True)
            new_validated_data = tuple(filter(_contain_filter_fn, validated_data))
...
class AbstractSaleableCompositeListSerializer(
    AugmentEditFieldsMixin, BulkUpdateListSerializer
):
    _field_name_map = {}
    ...
    def _retrieve_ingredient_ids(self, data):
        assert (
            self.pk_field_name == self._assert_pk_field_name
        ), "Name of pk field should be %s , but git %s" % (
            self._assert_pk_field_name,
            self.pk_field_name,
        )
    ...
    def _update_data_map(self, data):
        ...
        return {
            ...
            for item in data
            if item[self.pk_field_name].pk in ingredient_ids
        }

    def _insert_data_map(self, data):
        ingredient_ids = self._retrieve_ingredient_ids(data)
        return [
            item for item in data if item[self.pk_field_name].pk not in ingredient_ids
        ]

    def update(self, current_ingredients_applied=None, **kwargs):
        self._current_ingredients_applied = current_ingredients_applied
        return super().update(
            instance=current_ingredients_applied,
            allow_insert=True,
            allow_delete=True,
            **kwargs
        )
...
class SaleItemIngredientsAppliedListSerializer(AbstractSaleableCompositeListSerializer):
    ...
    def update(self, validated_data, sale_item, **kwargs):
        ...
        return super().update(
            ...
            sale_item=sale_item,
            current_ingredients_applied=qset,
            **kwargs
        )
...
class AbstractSaleableCompositeSerializer(ExtendedModelSerializer):
    ...
    def run_validation(self, data=DRFEmptyData):
        validated_value = super().run_validation(
            data=data, set_null_if_obj_not_found=True
        )
...
class SaleItemIngredientsAppliedSerializer(AbstractSaleableCompositeSerializer):
    ...
    def __init__(self, *args, data=DRFEmptyData, **kwargs):
        ...
        self.fields["quantity"].validators.append(
            NumberBoundaryValidator(limit=0.0, larger_than=True, include=False)
        )
...
class AbstractSaleableSerializer(BaseIngredientSerializer):
    ...
    def __init__(self, instance=None, data=DRFEmptyData, usrprof_id=None, **kwargs):
        ...
        self.fields["tags"] = PrimaryKeyRelatedField(
            many=True, queryset=ProductTag.objects.all()
        )
    ...
    def create(self, validated_data):
        validated_data["usrprof"] = self.usrprof_id
        nested_validated_data = self.extract_nested_form(
...
class SaleableItemSerializer(AbstractSaleableSerializer):
    class Meta(AbstractSaleableSerializer.Meta):
        ...
        fields = AbstractSaleableSerializer.Meta.fields.copy() + [
            "unit",
        ]

    def __init__(self, instance=None, data=DRFEmptyData, **kwargs):
        self.fields["media_set"] = CommonSaleableMediaMetaField(
            child=CharField(max_length=42),
            model=ProductSaleableItemMedia,
            ingredient_field_name="sale_item",
        )
        self.fields["ingredients_applied"] = SaleItemIngredientsAppliedSerializer(
    ...
    def create(self, validated_data):
        nested_validated_data = self.extract_nested_form(
            formdata=validated_data, nested_fields=["ingredients_applied"]
        )
        instance = super().create(validated_data=validated_data)
        self.fields["ingredients_applied"].create(
            nested_validated_data["ingredients_applied"], sale_item=instance
        )
        return instance
    ...
    def update(self, instance, validated_data):
        ...
        instance = super().update(instance=instance, validated_data=validated_data)
        self.fields["ingredients_applied"].update(
            sale_item=instance,
            validated_data=nested_validated_data["ingredients_applied"],
        )
        return instance
...
class SaleablePackageSerializer(AbstractSaleableSerializer):
    ...
    def __init__(self, instance=None, data=DRFEmptyData, **kwargs):
        self.fields["media_set"] = CommonSaleableMediaMetaField(
            child=CharField(max_length=42),
            model=ProductSaleablePackageMedia,
            ingredient_field_name="sale_pkg",
        )
        self.fields["saleitems_applied"] = SalePkgItemsAppliedSerializer(
            many=True, instance=instance
        )
        super().__init__(instance=instance, data=data, **kwargs)
    ...
    def create(self, validated_data):
        nested_validated_data = self.extract_nested_form(
            formdata=validated_data, nested_fields=["saleitems_applied"]
        )
        instance = super().create(validated_data=validated_data)
        self.fields["saleitems_applied"].create(
            nested_validated_data["saleitems_applied"], package=instance
        )
        return instance
    ...
    def update(self, instance, validated_data):
        ...
        instance = super().update(instance=instance, validated_data=validated_data)
        self.fields["saleitems_applied"].update(
            package=instance, validated_data=nested_validated_data["saleitems_applied"]
        )
        return instance
