...
def diff_attrvals(testcase, expect_d, actual_d):
    for dtype_opt in _ProductAttrValueDataType:
        expect_attrs = filter(lambda d:d['type'].dtype == dtype_opt.value[0][0], expect_d)
...
def diff_created_ingredients(testcase, expect_data, actual_data, lower_elm_names, lower_elm_mgr_fields):
    ...
    for actual_d in actual_data:
        ...
        diff_composite(testcase, expect_d['nested'].get('composites',[]), actual_d,
                lower_elm_name, lower_elm_mgr_field)
        # -------------------------------
        #import pdb
        #pdb.set_trace()
...
class HttpRequestDataGenSaleablePackage(HttpRequestDataGen, AttributeDataGenMixin):
    ...
    def _refresh_attrtypes(self, num):
        return self._refresh_prerequisite_elements(num=num, model_cls=ProductAttributeType)

    def _refresh_saleitems(self, num):
        return self._refresh_prerequisite_elements(num=num, model_cls=ProductSaleableItem)
    ...
    def customize_req_data_item(self, item, **kwargs):
        applied_tags = listitem_rand_assigner(list_=self._stored_models['ProductTag'],
        ...
        applied_media = listitem_rand_assigner(list_=_fixtures['ProductSaleableItemMedia'],
        ...
        composite_gen = listitem_rand_assigner(list_=self._stored_models['ProductSaleableItem'],
                min_num_chosen=self.min_num_applied_saleitems)
        item['saleitems_applied'] = list(map(self._gen_saleitem_composite, composite_gen))
    ...
    def _gen_saleitem_composite(self, sale_item):
        ...
        return {'sale_item': _get_inst_attr(sale_item,'id'), 'unit': chosen_unit,
    ...
    def rand_gen_edit_data(self, editing_data):
        for item in editing_data:
            ...
            applied_tags = listitem_rand_assigner(list_=self._stored_models['ProductTag'], min_num_chosen=0)
            item['tags'] = list(map(lambda obj:obj.id , applied_tags))
            applied_media = listitem_rand_assigner(list_=_fixtures['ProductSaleableItemMedia'], min_num_chosen=1)
...
class SaleablePackageVerificationMixin(BaseVerificationMixin, AttributeAssertionMixin):
    ...
    def verify_data(self, actual_data, expect_data, usrprof_id, verify_id=False):
        _expect_data = {'ProductSaleablePackage':[] , 'ProductSaleableItem':[],}
        simple_field_names = ['visible','name','price','usrprof',]
        if verify_id is True:
            simple_field_names.append('id')
        for saleitem in self._stored_models['ProductSaleableItem']:
            simple_fields = {fname: getattr(saleitem,fname) for fname in simple_field_names}
            nested_fields = {'tags':[], 'attrvals': [], 'media': [], 'composites':[],}
        ...
        for pkg_data in expect_data:
            ...
            simple_fields = {fname: pkg_data[fname] for fname in simple_field_names if pkg_data.get(fname)}
            simple_fields['usrprof'] = usrprof_id
            tags = list(filter(lambda obj: obj.id in pkg_data.get('tags', []), self._stored_models['ProductTag']))
            media_meta = list(map(lambda v:{'media':v}, pkg_data.get('media_set',[])))
            attrvals = [ d.copy() for d in pkg_data.get('attributes',[]) ]
            for attrval in attrvals:
                if verify_id is False:
                    attrval.pop('id',None)
                attrtype_objs = tuple(filter(lambda obj:obj.id == attrval['type'], self._stored_models['ProductAttributeType']))
                attrval['type'] = attrtype_objs[0]
            composites = [d.copy() for d in pkg_data.get('saleitems_applied',[])]
            for composite in composites:
                saleitem_data = tuple(filter(lambda d:d['obj'].id == composite['sale_item'], _expect_data['ProductSaleableItem']))
                composite['sale_item'] = saleitem_data[0]
            nested_fields = {'tags':tags, 'media': media_meta, 'attrvals':attrvals, 'composites':composites,}
            item = {'simple':simple_fields, 'nested': nested_fields, 'obj':pkg_obj}
            _expect_data['ProductSaleablePackage'].append(item)
        diff_created_ingredients( testcase=self,
                expect_data=_expect_data['ProductSaleablePackage'],
                actual_data=actual_data,  lower_elm_names=['sale_item', None],
