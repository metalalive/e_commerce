...
class SaleablePkgCommonMixin(
    HttpRequestDataGenSaleablePackage, SaleablePackageVerificationMixin
):
    num_users = 1
    min_num_pkgs = 2
    ...
    def setUp(self):
        ...
        salepkgs_data_gen = listitem_rand_assigner(
            list_=_fixtures["ProductSaleablePackage"], min_num_chosen=self.min_num_pkgs
        )
        request_data = rand_gen_request_body(
            customize_item_fn=self.customize_req_data_item,
            data_gen=salepkgs_data_gen,
            template=http_request_body_template["ProductSaleablePackage"],
        )
...
class SaleablePkgCreationTestCase(SaleablePkgCommonMixin, TransactionTestCase):
    ...
    def test_saleitems_applied_validate_error(self):
        ...
        self.serializer_kwargs["data"] = list(
            filter(
                lambda d: any(d["saleitems_applied"]), self.serializer_kwargs["data"]
            )
        )
        for idx in range(len(self.serializer_kwargs["data"])):
            rand_chosen_idx_2 = random.randrange(
                0, len(self.serializer_kwargs["data"][idx]["saleitems_applied"])
            )

            def fn_choose_edit_item(x):
                nonlocal rand_chosen_idx_2, idx
                return x[idx]["saleitems_applied"][rand_chosen_idx_2]
...
class SaleablePkgUpdateTestCase(SaleablePkgBaseUpdateTestCase):
    ...
    def test_bulk_ok_some_items(self):
        ...
        self.verify_data(
            actual_data=edited_objs,
            expect_data=self.editing_data,
            usrprof_id=self.profile_ids[0],
            verify_id=True,
        )
        ...
        self.verify_data(
            actual_data=unaffected_objs,
            expect_data=sorted_unaffected_data,
            usrprof_id=self.profile_ids[0],
            verify_id=True,
        )
...
class SaleablePkgUpdateConflictTestCase(SaleablePkgBaseUpdateTestCase):
    ...
    def test_conflict_composite_id(self):
        ...
        _discarded_id = composite_data[1]["sale_item"]  # noqa: F841
        ...
        self.assertEqual(
            validated_data[1]["sale_item"].id, composite_data[1]["sale_item"]
        )
        for field_name in ["unit", "quantity"]:
            self.assertEqual(
                validated_data[0][field_name], composite_data[0][field_name]
            )
            self.assertEqual(
                validated_data[1][field_name], composite_data[1][field_name]
            )
        edited_objs = serializer.save()
        # the first composite instance should be lost
        _discarded_composite = composite_data[0]  # noqa: F841
...
class SaleablePkgRepresentationTestCase(SaleablePkgBaseUpdateTestCase):
    def test_represent_all(self):
        ...
        self.verify_data(
            actual_data=created_objs,
            expect_data=serializer_ro.data,
            usrprof_id=self.profile_ids[0],
            verify_id=True,
        )
    ...
    def test_represent_partial(self):
        ...
        serializer_ro = self.serializer_class(
            instance=created_objs, many=True, context=context
        )
        ...
        for actual_d in created_objs:
            ...
            diff_composite(
                testcase=self,
                expect_d=expect_d["saleitems_applied"],
                actual_d=actual_d,
