...
def _gen_common_nested_field_dataitem(actual_data):
    ...
    tags = list(tags_gen)
    bound_gen_attrval_dataitem = partial(
        _gen_attrval_dataitem, enable_extra_amount=True
    )
...
def _gen_pkg_expect_data(actual_data, num_salepkgs, num_saleitems, num_ingredients):
    ...
    saleitem_data_gen = listitem_rand_assigner(
        list_=_fixtures["ProductSaleableItem"],
        min_num_chosen=num_saleitems,
        max_num_chosen=(num_saleitems + 1),
    )
    for saleitem_data in saleitem_data_gen:
        ingre_composites = _gen_composite_data(
            out["ProductDevIngredient"], elm_field_name="ingredient"
        )
    ...
    pkg_data_gen = listitem_rand_assigner(
        list_=_fixtures["ProductSaleablePackage"],
        min_num_chosen=num_salepkgs,
        max_num_chosen=(num_salepkgs + 1),
    )
    for pkg_data in pkg_data_gen:
        pkg_composites = _gen_composite_data(
            out["ProductSaleableItem"], elm_field_name="sale_item"
        )
        nested_item = _gen_common_nested_field_dataitem(actual_data)
...
def _gen_saleitem_composite_obj(saleitem, nested_data):
    ...
    for data in nested_data.get("composites", []):
        init_kwargs = {
            "sale_item": saleitem,
            "ingredient": data["ingredient"]["obj"],
            "unit": data["unit"][0],
...
def _salepkg_instances_setup(num_salepkgs, num_saleitems, num_ingredients):
    actual_data = {
        ...
        "ProductSaleablePackage": None,
        "ProductSaleableItem": None,
        "ProductDevIngredient": None,
    }
    _init_tag_attrtype(stored_models=actual_data)
    expect_data = _gen_pkg_expect_data(
        actual_data,
        num_salepkgs=num_salepkgs,
        num_saleitems=num_saleitems,
        num_ingredients=num_ingredients,
    )

    def obj_attr_gen_fn(d):
        return d["simple"]

    _gen_actual_data(
        expect_data=expect_data,
        actual_data=actual_data,
        model_cls=ProductDevIngredient,
        obj_attr_gen_fn=obj_attr_gen_fn,
    )
    _gen_actual_data(
        expect_data=expect_data,
        actual_data=actual_data,
        model_cls=ProductSaleableItem,
        obj_attr_gen_fn=obj_attr_gen_fn,
        gen_mm_fn=_gen_saleitem_mediameta_obj,
        gen_compo_fn=_gen_saleitem_composite_obj,
    )
    _gen_actual_data(
        expect_data=expect_data,
        actual_data=actual_data,
        model_cls=ProductSaleablePackage,
        obj_attr_gen_fn=obj_attr_gen_fn,
        gen_mm_fn=_gen_salepkg_mediameta_obj,
        gen_compo_fn=_gen_salepkg_composite_obj,
    )
...
class SaleablePackageCreationTestCase(TransactionTestCase):
    ...
    def test_bulk_ok(self):
        ...
        num_saleitems = 6
        num_ingredients = 8
        expect_data, actual_data = _salepkg_instances_setup(
            num_salepkgs=num_salepkgs,
            num_ingredients=num_ingredients,
            num_saleitems=num_saleitems,
        )
        diff_created_ingredients(
            self,
            expect_data["ProductSaleablePackage"],
            actual_data["ProductSaleablePackage"],
...
class SaleablePackageDeletionTestCase(TransactionTestCase):
    ...
    def setUp(self):
        self._expect_data, self._actual_data = _salepkg_instances_setup(
            num_salepkgs=self.num_salepkgs,
            num_saleitems=self.num_saleitems,
            num_ingredients=self.num_ingredients,
        )
        num_delete = self.num_salepkgs >> 1
        self.expect_delete_pkgs = list(
            listitem_rand_assigner(
                list_=self._expect_data["ProductSaleablePackage"],
                min_num_chosen=num_delete,
                max_num_chosen=(num_delete + 1),
            )
        )
    ...
    def test_hard_delete(self):
        ProductSaleablePackage.objects.filter(id__in=self.expect_delete_ids).delete(
            hard=True
        )
        pkg_qset = ProductSaleablePackage.objects.filter(
            with_deleted=True, id__in=self.expect_delete_ids
        )
        compo_exists = ProductSaleablePackageComposite.objects.filter(
            with_deleted=True, package__id__in=self.expect_delete_ids
        ).exists()
        mm_exists = ProductSaleablePackageMedia.objects.filter(
            with_deleted=True, sale_pkg__id__in=self.expect_delete_ids
        ).exists()
        ...
        #    pass
        expect_remain_data = tuple(
            filter(
                lambda d: d["obj"].id not in self.expect_delete_ids,
                self._expect_data["ProductSaleablePackage"],
            )
        )
        expect_remain_objs = tuple(
            filter(
                lambda obj: obj.id not in self.expect_delete_ids,
                self._actual_data["ProductSaleablePackage"],
            )
        )
        diff_created_ingredients(
            self,
            expect_data=expect_remain_data,
            actual_data=expect_remain_objs,
            lower_elm_names=["sale_item", "ingredient", None],
            lower_elm_mgr_fields=["saleitems_applied", "ingredients_applied", None],
        )
    ...
    def test_soft_delete(self):
        profile_id = 123
        qset = ProductSaleablePackage.objects.filter(
            id__in=self.expect_delete_ids
        ).order_by("id")
        qset.delete(profile_id=profile_id)
        pkg_exists = ProductSaleablePackage.objects.filter(
            id__in=self.expect_delete_ids
        ).exists()
        compo_exists = ProductSaleablePackageComposite.objects.filter(
            package__id__in=self.expect_delete_ids
        ).exists()
        mm_exists = ProductSaleablePackageMedia.objects.filter(
            sale_pkg__id__in=self.expect_delete_ids
        ).exists()
        ...
        for dtype_opt in _ProductAttrValueDataType:
            ...
            attrval_exists = model_cls.objects.filter(
                ingredient_type=obj_ct, ingredient_id__in=self.expect_delete_ids
            ).exists()
        ...
        # -------------------------------------
        qset = (
            ProductSaleablePackage.objects.get_deleted_set()
            .filter(id__in=self.expect_delete_ids)
            .order_by("id")
        )
        qset.undelete(profile_id=profile_id)
        diff_created_ingredients(
            self,
            expect_data=self.expect_delete_pkgs,
            actual_data=qset,
            lower_elm_names=["sale_item", "ingredient", None],
            lower_elm_mgr_fields=["saleitems_applied", "ingredients_applied", None],
        )
...
class SaleableItemDeletionTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        expect_delete_items = list(
            listitem_rand_assigner(
                list_=self._expect_data["ProductSaleableItem"],
                min_num_chosen=num_delete,
                max_num_chosen=(num_delete + 1),
            )
        )
    ...
    def test_soft_delete(self):
        ...
        qset.delete(profile_id=profile_id)
        item_exists = ProductSaleableItem.objects.filter(
            id__in=self.expect_delete_ids
        ).exists()
        compo_exists = ProductSaleablePackageComposite.objects.filter(
            sale_item__id__in=self.expect_delete_ids
        ).exists()
        compo2_exists = ProductSaleableItemComposite.objects.filter(
            sale_item__id__in=self.expect_delete_ids
        ).exists()
        mm_exists = ProductSaleableItemMedia.objects.filter(
            sale_item__id__in=self.expect_delete_ids
        ).exists()
        ...
        for data in self.expect_delete_items:
            data["nested"]["tags"].clear()
        qset = (
            ProductSaleableItem.objects.get_deleted_set()
            .filter(id__in=self.expect_delete_ids)
            .order_by("id")
        )
        qset.undelete(profile_id=profile_id)
