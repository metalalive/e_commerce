...
class GetProductCase(TransactionTestCase):
    ...
    def setUp(self):
            ...
            "ProductSaleableItem": [
                {
                    ...
                    "price": random.randrange(1, 40),
                    "name": "".join(random.choices(string.ascii_letters, k=14)),
                    "usrprof": self.default_profile_id,
                }
                for _ in range(10)
            ],
        ...
        _common_instances_setup(
            out=self._primitives, data=self._data, models_info=models_info
        )
        random.shuffle(self._primitives["ProductSaleableItem"])
        random.shuffle(self._primitives["ProductSaleablePackage"])

    def tearDown(self):
        get_product.app.conf.task_always_eager = False

    def test_ok(self):
        chosen_items = self._primitives["ProductSaleableItem"][:3]
        chosen_pkgs = self._primitives["ProductSaleablePackage"][:3]
        ...
        input_kwargs = {
            ...
            "profile": self.default_profile_id,
        }
        eager_result = get_product.apply_async(kwargs=input_kwargs)
        self.assertEqual(eager_result.state, CeleryStates.SUCCESS)
        expect_val_items = list(
            map(lambda obj: {k: getattr(obj, k) for k in item_fields}, chosen_items)
        )
        expect_val_pkgs = list(
            map(lambda obj: {k: getattr(obj, k) for k in pkg_fields}, chosen_pkgs)
        )
    ...
    def test_different_users(self):
        ...
        item_ids = list(
            map(lambda obj: obj.id, self._primitives["ProductSaleableItem"])
        )
        pkg_ids = list(
            map(lambda obj: obj.id, self._primitives["ProductSaleablePackage"])
        )
        fields_present = ["id", "name"]
        input_kwargs = {
            "item_ids": item_ids,
