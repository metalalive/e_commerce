...
class SaleableItemBaseViewTestCase(
    TransactionTestCase, _MockTestClientInfoMixin, HttpRequestDataGenSaleableItem
):
...
class SaleableItemCreationTestCase(SaleableItemBaseViewTestCase):
    ...
    def test_bulk_ok_with_full_response(self):
        ...
        response = self._send_request_to_backend(
            path=self.path, body=self._request_data, access_token=access_token
        )
    ...
    def test_validation_error_unknown_references(self):
        _saleitem_related_instance_setup(
            self.stored_models, num_tags=0, num_ingredients=0
        )
        ...
        response = self._send_request_to_backend(
            path=self.path, body=request_data, access_token=access_token
        )
    ...
    def test_quota_exceed_limit(self):
            ...
            "quotas": [
                {
                    ...
                    "mat_code": _MatCodeOptions.MAX_NUM_SALE_ITEMS,
        ...
        request_data = self.refresh_req_data(
            fixture_source=model_fixtures["ProductSaleableItem"][0:3]
        )
        response = self._send_request_to_backend(
            path=self.path,
            body=request_data,
        ...
        request_data = self.refresh_req_data(
            fixture_source=model_fixtures["ProductSaleableItem"][3:7]
        )
        response = self._send_request_to_backend(
            path=self.path,
            body=request_data,
        ...
        request_data = self.refresh_req_data(
            fixture_source=model_fixtures["ProductSaleableItem"][3:6]
        )
        response = self._send_request_to_backend(
            path=self.path,
            body=request_data,
...
class SaleableItemUpdateTestCase(
    SaleableItemUpdateBaseTestCase, SaleableItemVerificationMixin
):
    def setUp(self):
        super().setUp()
        expect_usrprof = 71
    ...
    def _rand_gen_edit_data(self):
        ...
        created_ids_gen = listitem_rand_assigner(
            list_=self._created_items, max_num_chosen=(len(self._created_items) + 1)
        )
        edit_data = []
...
class SaleableItemDeletionTestCase(
    SaleableItemUpdateBaseTestCase,
    SaleableItemVerificationMixin,
    SoftDeleteCommonTestMixin,
):
    def setUp(self):
    ...
    def _verify_undeleted_items(self, undeleted_items):
        ...
        self.verify_objects(
            actual_instances=undeleted_objs,
    ...
    def test_undelete_by_time(self):
        ...
        for _ in range(3):
            self._softdelete_one_by_one(
                remain_items, deleted_items, delay_interval_sec=2
            )
    ...
    def test_undelete_specific_item(self):
        ...
        for _ in range(3):
            self._softdelete_one_by_one(
                remain_items, deleted_items, delay_interval_sec=0
            )
            undeleting_items_gen = listitem_rand_assigner(
                list_=deleted_items,
                min_num_chosen=len(deleted_items),
            ...
            self._verify_undeleted_items(affected_items)
            remain_items, deleted_items = (
                deleted_items,
                remain_items,
            )  # internally swap 2 lists
...
class SaleableItemQueryTestCase(
    SaleableItemUpdateBaseTestCase, SaleableItemVerificationMixin
):
    def setUp(self):
        super().setUp()
    ...
    def test_bulk_items_full_info(self):
        ...
        expect_items = self.serializer_class.Meta.model.objects.filter(
            id__in=created_ids
        )
        self.assertEqual(int(response.status_code), 200)
    ...
    def test_bulk_items_partial_info(self):
        ...
        for expect_item in expect_items:
            ...
            self._assert_simple_fields(
                ...
                exp_sale_item=expect_item,
                ac_sale_item=actual_item,
            )
            self._assert_mediaset_fields(
                exp_sale_item=actual_item, ac_sale_item=expect_item
            )
            self._assert_ingredients_applied_fields(
                exp_sale_item=actual_item, ac_sale_item=expect_item
            )
    ...
    def test_single_items_full_info(self):
        ...
        for created_item in self._created_items:
            ...
            response = self._send_request_to_backend(
                path=path_with_id, method="get", access_token=self._access_token
            )
            actual_item = response.json()
            expect_item = self.serializer_class.Meta.model.objects.get(
                id=created_item["id"]
            )
            self.assertEqual(int(response.status_code), 200)
            self.assertEqual(created_item["id"], actual_item["id"])
            self.verify_objects(
                actual_instances=[expect_item], expect_data=[actual_item]
            )


class SaleableItemSearchFilterTestCase(
    SaleableItemUpdateBaseTestCase, SaleableItemVerificationMixin
):
    ...
    def test_simple_search(self):
        ...
        for keyword in keywords:
            ...
            final_condition = (
                ...
                | Q(pkgs_applied__package__name__icontains=keyword)
                | Q(ingredients_applied__ingredient__name__icontains=keyword)
            )
            expect_items = (
                saleitem_qset.filter(final_condition).distinct().values("id", "name")
            )
            self.assertEqual(int(response.status_code), 200)
    ...
    def test_tags(self):
        ...
        qset = (
            self._model_cls.objects.filter(tags__isnull=False)
    ...
    def test_ingredients_applied(self):
                ...
                "operands": [
                    {
                        "operator": "==",
                        "operands": [
                            "ingredients_applied__ingredient",
                            value["ingredients_applied__ingredient_id"],
                        ],
                        "metadata": {},
                    },
                    {
        ...
        self.assertListEqual(
            sorted(actual_saleitems_found), sorted(expect_saleitems_found)
        )
    ...
    def test_complex_condition(self):
        ...
        filter_nested_fields = dict(
            map(
                lambda ag_fd: (LOOKUP_SEP.join([ag_fd, "gt"]), 0),
        ...
        for found_item in saleitems_found:
            ...
            ingredient_ids = tuple(
                map(lambda x: x["ingredient"], found_item["ingredients_applied"])
            )
