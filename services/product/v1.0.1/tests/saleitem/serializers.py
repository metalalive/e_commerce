...
from .common import (
    _fixtures as model_fixtures,
    listitem_rand_assigner,
    HttpRequestDataGenSaleableItem,
    _saleitem_related_instance_setup,
    SaleableItemVerificationMixin,
)


class SaleableItemCommonMixin(
    HttpRequestDataGenSaleableItem, SaleableItemVerificationMixin
):
    stored_models = {}
    num_users = 1
    ...
    def setUp(self):
        ...
        saleitems_data_gen = listitem_rand_assigner(
            list_=model_fixtures["ProductSaleableItem"]
        )
        self.request_data = rand_gen_request_body(
            customize_item_fn=self.customize_req_data_item,
            data_gen=saleitems_data_gen,
...
class SaleableItemCreationTestCase(SaleableItemCommonMixin, TransactionTestCase):
    ...
    def test_bulk_ok(self):
        ...
        self.verify_objects(
            actual_instances, expect_data, usrprof_id=serializer.child.usrprof_id
        )
    ...
    def test_fields_validate_error(self):
        invalid_cases = [
            ...
            (
                "price",
                -0.3,
                NumberBoundaryValidator._error_msg_pattern % (-0.3, 0.0, "gt"),
            ),
            (
                "price",
                -0.0,
                NumberBoundaryValidator._error_msg_pattern % (-0.0, 0.0, "gt"),
            ),
            (
                "price",
                0.0,
            ...
            (
                "media_set",
                ["x8Ej", "9u 2"],
                UnprintableCharValidator._error_msg_pattern % ("9u 2"),
            ),
            (
                "media_set",
                ["x8Ej", "9u 2", "8u%G", "d8'w"],
                UnprintableCharValidator._error_msg_pattern
                % ", ".join(["9u 2", "d8'w"]),
            ),
            (
                "media_set",
    ...
    def test_ingredients_applied_validate_error(self):
        ...
        self.serializer_kwargs["data"] = list(
            filter(lambda d: any(d["ingredients_applied"]), self.request_data)
        )
        for idx in range(len(self.serializer_kwargs["data"])):
            rand_chosen_idx_2 = random.randrange(
                0, len(self.serializer_kwargs["data"][idx]["ingredients_applied"])
            )

            def fn_choose_edit_item(x):
                nonlocal idx, rand_chosen_idx_2
                return x[idx]["ingredients_applied"][rand_chosen_idx_2]
    ...
    def test_skip_given_attribute_id(self):
        ...
        self.request_data = list(
            filter(lambda d: any(d["attributes"]), self.request_data)
        )
        serializer = SaleableItemSerializer(**self.serializer_kwargs)
        for invalid_case in invalid_cases:
            ...
            for dtype_opt in _ProductAttrValueDataType:
                ...
                if validated_attrs:
                    with self.assertRaises(KeyError):
                        validated_id = validated_attrs[0]["id"]  # noqa: F841
    ...
    def _loop_through_invalid_cases_common(
        self, fn_choose_edit_item, invalid_cases, **kwargs
    ):
        serializer = SaleableItemSerializer(**self.serializer_kwargs)
...
class SaleableItemUpdateTestCase(SaleableItemCommonMixin, TransactionTestCase):
    def saleitems_data_gen(self, saved_items, dataset):
        ...
        for sale_item in saved_items:
            new_item = {
                fname: dataset[random.randrange(0, dataset_limit)][fname]
    ...
    def _refresh_edit_data(self, num_edit_items: int):
        data_gen = self.saleitems_data_gen(
            saved_items=self._created_items[:num_edit_items],
    ...
    def _refresh_saved_instance(self, num_edit_items: int):
        saleitem_ids = tuple(
            map(lambda obj: obj.pk, self._created_items[:num_edit_items])
        )
    ...
    def test_bulk_ok_some_items(self):
        ...
        serializer, upadted_saleitems, _ = self._test_bulk_ok_certain_num_of_items(
            num_edit_items=num_edit_items
        )
        # double-check the instances which were not updated
        edited_ids = tuple(map(lambda x: x.pk, upadted_saleitems))
        unedited_objs = tuple(
            filter(lambda x: x.id not in edited_ids, self._created_items)
        )
        unedited_items = tuple(
            filter(lambda x: x["id"] not in edited_ids, self.request_data)
        )
        ...
        self.verify_objects(
            unedited_objs, unedited_items, usrprof_id=serializer.child.usrprof_id
        )
    ...
    def _test_bulk_ok_certain_num_of_items(self, num_edit_items):
        ...
        self.verify_objects(
            upadted_saleitems, expect_data, usrprof_id=serializer.child.usrprof_id
        )
    ...
    def test_edit_nested_field(self):
        ...
        unadded = list(
            filter(
                lambda obj: obj.pk not in edit_data["tags"],
                self.stored_models["ProductTag"],
            )
        )
        ...
        unadded = list(
            filter(
                lambda obj: obj.pk not in edit_data["ingredients_applied"],
                self.stored_models["ProductDevIngredient"],
            )
        )
        ...
        self.verify_objects(
            edited_objs, edit_data, usrprof_id=serializer.child.usrprof_id
        )
...
class SaleableItemRepresentationTestCase(SaleableItemCommonMixin, TransactionTestCase):
    ...
    def test_represent_all(self):
        ...
        self.verify_data(
            actual_data, expect_data, usrprof_id=self._serializer.child.usrprof_id
        )
        ...
        for idx in range(1, len(self.saved_saleitems)):
            ...
            self.verify_data(
                actual_data, expect_data, usrprof_id=serializer_ro.child.usrprof_id
            )
    ...
    def test_represent_partial_1(self):
        ...
        self._test_represent_partial(
            expect_fields=expect_fields, field_check_fn=field_check
        )
    ...
    def test_represent_partial_3(self):
        def item_check(value):
            ingredient_ids = list(
                map(lambda d: d["ingredient"], value["ingredients_applied"])
            )
            actual_cnt = ProductDevIngredient.objects.filter(
            ...
            expect_composite = sale_item.ingredients_applied.values(
        ...
        self._test_represent_partial(
            expect_fields=expect_fields, item_check_fn=item_check
        )
    ...
    def test_represent_partial_4(self):
        def item_check(value):
            ...
            for actual_attrval in value["attributes"]:
                ...
                chosen_dtype_opt = next(
                    filter(
                        lambda opt: opt.value[0][0] == chosen_dtype,
                        _ProductAttrValueDataType,
                    )
                )
                ...
                self.assertEqual(
                    expect_attrval.extra_amount,
                    actual_attrval.get("extra_amount", None),
                )
    ...
    def _test_represent_partial(
        self, expect_fields, field_check_fn=None, item_check_fn=None
    ):
        ...
        serializer_ro.context["request"].query_params["fields"] = ",".join(
            expect_fields
        )
        actual_data = serializer_ro.data
