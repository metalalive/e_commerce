...
class SaleableItemSimpleDeletionTestCase(TransactionTestCase):
    ...
    def test_hard_delete_single_item_ok(self):
        ...
        exist = ProductSaleableItem.objects.filter(
            with_deleted=True, pk=chosen_id
        ).exists()
    ...
    def test_soft_delete_single_item_ok(self):
        ...
        self.assertEqual(exist, True)
        exist = ProductSaleableItem.objects.filter(
            with_deleted=True, id=chosen_id
        ).exists()
        self.assertEqual(exist, True)
    ...
    def test_hard_delete_bulk_ok(self):
        total_num_stored, num_deleting, num_remained, chosen_ids = (
            self._choose_ids_to_delete()
        )
    ...
    def test_soft_delete_bulk_ok(self):
        ...
        cnt = ProductSaleableItem.objects.filter(
            with_deleted=True, id__in=chosen_ids
        ).count()
...
class SaleableItemCompositeCreationMixin:
    def setUp(self):
        ...
        models_info = [
            (ProductDevIngredient, len(_fixtures["ProductDevIngredient"])),
    ...
    def _bulk_create(self, num_compo):
        ...
        uom_gen = listitem_rand_assigner(
            list_=UnitOfMeasurement.choices,
            distinct=False,
            min_num_chosen=min_num_chosen,
            max_num_chosen=max_num_chosen,
        )
        saleitem_gen = listitem_rand_assigner(
            list_=self.instances["ProductSaleableItem"],
            distinct=False,
            min_num_chosen=min_num_chosen,
            max_num_chosen=max_num_chosen,
        )
        ingredient_gen = listitem_rand_assigner(
            list_=self.instances["ProductDevIngredient"],
            distinct=False,
            min_num_chosen=min_num_chosen,
            max_num_chosen=max_num_chosen,
        )
        ...
        while len(saleitems_composite) < num_compo:
            try:
                ...
                pass  # skip the errors whose root cuase is duplicate product ID
...
class SaleableItemCompositeCreationTestCase(
    SaleableItemCompositeCreationMixin, TransactionTestCase
):
    def test_null_test_obj_attrs(self):
        instance = ProductSaleableItemComposite(
    ...
    def test_create_one_ok(self):
        ...
        expected_compo_pk = {
            "ingredient": instance.ingredient.pk,
            "sale_item": instance.sale_item.pk,
        }
        ...
        cond = Q(
            pk__isnull={
                ...
                "ingredient__in": list(
                    range(instance.ingredient.pk - 5, instance.ingredient.pk + 5)
                ),
    ...
    def test_edit_one_ok(self):
        ...
        self.assertEqual(
            instance.ingredient.pk, self.instances["ProductDevIngredient"][-2].pk
        )
        ...
        instance.refresh_from_db()
        self.assertEqual(
            instance.sale_item.pk, self.instances["ProductSaleableItem"][-3].pk
        )
        num_stored = ProductSaleableItemComposite.objects.count()
        self.assertEqual(num_stored, 1)
    ...
    def test_bulk_edit_ok(self):
        ...
        for idx in range(num_compo):
            instance = ProductSaleableItemComposite(
                ...
                ingredient=self.instances["ProductDevIngredient"][
                    idx % num_ingredients
                ],
        ...
        for idx in range(1, num_compo):
            ...
            instance.sale_item = self.instances["ProductSaleableItem"][
                (idx + 2) % num_saleitems
            ]
            instance.ingredient = self.instances["ProductDevIngredient"][
                (idx + 3) % num_ingredients
            ]
            ...
            instance.quantity = random.randrange(
                instance.quantity, instance.quantity + 100
            )
        ...
        with self.assertRaises(IntegrityError):
            ...
            ProductSaleableItemComposite.objects.bulk_update(
                saleitems_composite[1:num_compo], fields=["sale_item", "quantity"]
            )
        ...
        with self.assertRaises(IntegrityError):
            ProductSaleableItemComposite.objects.bulk_update(
                saleitems_composite[1:num_compo], fields=["sale_item", "ingredient"]
            )
        ...
        # end of the list .
        ProductSaleableItemComposite.objects.bulk_update(
            saleitems_composite[1:num_compo], fields=["quantity", "sale_item"]
        )
        for idx in range(1, num_compo):
            instance = saleitems_composite[idx]
...
class SaleableItemCompositeQueryTestCase(
    SaleableItemCompositeCreationMixin, TransactionTestCase
):
    def setUp(self):
        super().setUp()
    ...
    def test_id_in_syntax(self):
        ...
        composite_gen = listitem_rand_assigner(
            list_=saleitems_composite,
            distinct=True,
            min_num_chosen=num_chosen_composites,
            max_num_chosen=(num_chosen_composites + 1),
        )
        ...
        actual = ProductSaleableItemComposite.objects.filter(cond)
        expect = list(
            filter(lambda obj: obj.quantity > avg_quantity, chosen_composites)
        )
        self._assert_query_dict_equal(actual=actual, expect=expect)
    ...
    def test_multi_columns_compare(self):
        ...
        avg_ingredient_id = reduce(lambda a, b: a + b, ingredient_ids) / len(
            ingredient_ids
        )
        ...
        chosen_unit = saleitems_composite[-1].unit
        cond = Q(
            pk={"sale_item__gt": avg_saleitem_id, "ingredient__lt": avg_ingredient_id}
        )
        cond = cond | Q(unit=chosen_unit)
        actual = ProductSaleableItemComposite.objects.filter(cond)

        def _filter_fn(obj):
            nonlocal chosen_unit, avg_saleitem_id, avg_ingredient_id
            return (obj.unit == chosen_unit) or (
                obj.sale_item_id > avg_saleitem_id
    ...
    def test_ingredients_applied_value(self):
        field_names = [
            "ingredients_applied",
            "ingredients_applied__unit",
            "ingredients_applied__quantity",
        ]
        ...
        actual_qset = (
            ProductSaleableItem.objects.annotate(
                num_ingre=Count("ingredients_applied__ingredient")
            )
        ...
        for value in actual_qset:
            compo_pk = {
                "ingredient": value["ingredients_applied__ingredient_id"],
                "sale_item": value["ingredients_applied__sale_item_id"],
            }
            saleitem = ProductSaleableItem.objects.get(
                id=value["ingredients_applied__sale_item_id"]
            )
            ...
            self.assertEqual(
                expect_composite.quantity, value["ingredients_applied__quantity"]
            )
    ...
    def test_ingredients_applied_valuelist(self):
        ...
        for value in actual_qset:
            compo_pk = {
                compo_fields[idx]: value[2 + idx] for idx in range(len(compo_fields))
            }
    ...
    def test_ingredients_applied_filter(self):
        base_qset = ProductSaleableItem.objects.annotate(
            num_ingre=Count("ingredients_applied__ingredient")
        ).filter(num_ingre__gt=0)
...
class SaleableItemCompositeDeletionTestCase(TransactionTestCase):
    ...
    def test_hard_delete_bulk_ok(self):
        ...
        cnt_before_delete = applied_ingredients.count()
        num_composite_half = cnt_before_delete >> 1
        ...
        composites_from_db = ProductSaleableItemComposite.objects.filter(
            pk__in=tuple(deleted_pks_2)
        )
        ...
        composite_field_names = [
            f.name for f in deleted_pks_1.model._meta.pk._composite_fields
        ]
        remain_pks = [
            {
                composite_field_names[jdx]: deleted_pks_1[idx][jdx]
                for jdx in range(len(composite_field_names))
            }
            for idx in range(cnt_after_delete)
        ]
        composites_from_db = ProductSaleableItemComposite.objects.filter(
            pk__in=remain_pks
        )
    ...
    def test_soft_delete_bulk_ok(self):
        ...
        empty_qset = saleitem.ingredients_applied.filter(
            with_deleted=False, id__in=discarded_pks
        )
        self.assertEqual(empty_qset.exists(), False)
        softdel_qset = saleitem.ingredients_applied.filter(
            with_deleted=True, id__in=discarded_pks
        )
        ...
        cnt_after_undelete = saleitem.ingredients_applied.all(
            with_deleted=False
        ).count()
        self.assertEqual(origin_num_compos, cnt_after_undelete)
        undel_qset = saleitem.ingredients_applied.filter(
            with_deleted=False, id__in=discarded_pks
        )
...
def _saleitem_attrvals_refresh_from_db(saleitems):
    fresh_attrvals = [
        getattr(saleitem, dtype_item[0][1]).all()
...
class SaleableItemAttributeCreationTestCase(TransactionTestCase):
    ...
    def _choose_attr_type(self, dtype: int):
        filtered = tuple(
            filter(lambda x: x.dtype == dtype, self.instances["ProductAttributeType"])
        )
        assert any(filtered), (
            "attribute type not found due to incorrect data type %s" % dtype
        )
        return filtered[0]
    ...
    def test_create_one_with_invalid_attr_value(self):
        ...
        for dtype_id, corresponding_test_value in data_types:
            ...
            _null_test_obj_attrs(
                testcase=self, instance=instance, field_names=field_names
            )
    ...
    def test_create_one_with_extra_charge(self):
        ...
        def assert_new_object(attrtype):
            ...
            attrval = _gen_ingredient_attrvals(
                attrtype, saleitem, idx, extra_charge=extra_charge
            )
    ...
    def test_create_bulk_ok(self):
        ...
        _attrval_objs = _ingredient_attrvals_common_setup(
            ingredients=saleitems, attrtypes_gen_fn=attrtypes_gen_fn
        )
        for saleitem in saleitems:
            for dtype_item in _ProductAttrValueDataType:
                ...
                expected_values = [
                    {
                        "value": x.value,
                        "type": x.attr_type.pk,
                        "saleitem": x.ingredient_id,
                    }
                    for x in expect_objs
                ]
                actual_values = [
                    {
                        "value": x.value,
...
def _saleitem_attrvals_extracharge_setup(attrvals, retrieve_id=False):
    attrvals_gen = listitem_rand_assigner(
        list_=attrvals, min_num_chosen=(len(attrvals) >> 1)
    )
    extra_charge_objs = [
        ProductAppliedAttributePrice(
...
class SaleableItemAttributeDeletionTestCase(TransactionTestCase):
    ...
    instances = {
        "ProductAttributeType": None,
        "ProductSaleableItem": None,
        "BaseProductAttributeValue": None,
        "ProductAppliedAttributePrice": None,
    }
    ...
    def setUp(self):
        ...
        self.instances["BaseProductAttributeValue"] = (
            _saleitem_attrvals_refresh_from_db(saleitems)
        )
        self.instances["ProductAppliedAttributePrice"] = (
            _saleitem_attrvals_extracharge_setup(
    ...
    def _retrieve_expect_deleted_data(self, qset):
        ...
        def serialize_fn(obj):
            qset = (
                type(obj).objects.filter(id=obj.id).values(*attrval_field_names_clone)
            )
            return qset.first()
    ...
    def _assert_before_delete(self):
        bound_dict_key_replace = partial(
            _dict_key_replace, to_="extra_charge", from_="_extra_charge__amount"
        )
        for saleitem in self.instances["ProductSaleableItem"]:
            for dtype_item in _ProductAttrValueDataType:
                ...
                actual_deleted_data = qset.values(
                    "_extra_charge__amount", *self.attrval_field_names
                )
                expect_deleted_data = list(
                    map(bound_dict_key_replace, expect_deleted_data)
                )
                actual_deleted_data = list(
                    map(bound_dict_key_replace, actual_deleted_data)
                )
    ...
    def test_soft_delete_bulk_ok(self):
        ...
        for (
            ...
            actual_deleted_data = qset3.values(
                "_extra_charge__amount", *self.attrval_field_names
            )
...
def _saleitem_composites_common_setup(saleitem, out: dict, ingredients):
    ...
    uom_gen = listitem_rand_assigner(
        list_=UnitOfMeasurement.choices,
        distinct=False,
        min_num_chosen=num_composites,
        max_num_chosen=(num_composites + 1),
    )
    ingredient_gen = listitem_rand_assigner(
        list_=ingredients, min_num_chosen=num_ingredients
    )
    gen_compo_fn = partial(
        _gen_saleitem_composite,
        uom_gen=uom_gen,
        saleitem=saleitem,
    ...
    stored_composites = ProductSaleableItemComposite.objects.filter(
        sale_item=saleitem.id
    ).values("id", "unit", "quantity")
    out[saleitem.pk] = list(stored_composites)


class SaleableItemAdvancedDeletionTestCase(
    TransactionTestCase, SoftDeleteCommonTestMixin
):
    ...
    def setUp(self):
        ...
        _ingredient_attrvals_common_setup(
            ingredients=self.instances["ProductSaleableItem"],
            attrtypes_gen_fn=attrtypes_gen_fn,
        )
        self.instances["BaseProductAttributeValue"] = (
            _saleitem_attrvals_refresh_from_db(
                saleitems=self.instances["ProductSaleableItem"]
            )
        )
        self.instances["ProductAppliedAttributePrice"] = (
        ...
        bound_composites_setup = partial(
            _saleitem_composites_common_setup,
        ...
        # tag
        self.instances["ProductTag"] = _modelobj_list_to_map(
            self.instances["ProductTag"]
        )
        ...
        for saleitem in self.instances["ProductSaleableItem"]:
            tags_gen = listitem_rand_assigner(
                list_=self.instances["ProductTag"].values()
            )
        ...
        # media link
        for saleitem in self.instances["ProductSaleableItem"]:
            _media_meta_gen = listitem_rand_assigner(
    ...
    def _check_remaining_saleitems_to_ingredients(self, saleitem_ids, is_deleted=False):
        chosen_composites = dict(
            filter(
                lambda kv: kv[0] in saleitem_ids,
        ...
        for ingredient in self.instances["ProductDevIngredient"]:
            init_qset = (
                ingredient.saleitems_applied.get_deleted_set()
                if is_deleted
                else ingredient.saleitems_applied.all()
            )
            qset = init_qset.filter(
                ingredient=ingredient.id, sale_item__in=saleitem_ids
            )
            actual = list(qset.values("sale_item", "unit", "quantity"))
            remain_composites = filter(
                lambda v: v["ingredient_id"] == ingredient.id, chosen_composites
            )
            expect = list(
                map(
                    lambda v: {
                        "sale_item": v["sale_item_id"],
                        "unit": v["unit"],
    ...
    def _assert_attr_fields_existence(self, saleitem_pk, _assert_fn):
        filtered_attrvals = filter(
            lambda attrval: attrval.ingredient_id == saleitem_pk,
            self.instances["BaseProductAttributeValue"],
        )

        def filter_extmnt_fn(extracharge, attrval):
            cond1 = (
                extracharge.attrval_type.pk
                == ContentType.objects.get_for_model(attrval).pk
            )
            cond2 = extracharge.attrval_id == attrval.pk
            return cond1 and cond2

        for attrval in filtered_attrvals:
            _assert_fn(attrval)
            bound_filter_extmnt_fn = partial(filter_extmnt_fn, attrval=attrval)
            filtered_extmnts = filter(
                bound_filter_extmnt_fn, self.instances["ProductAppliedAttributePrice"]
            )
    ...
    def _assert_composite_fields_existence(self, saleitem_pk, _assert_fn, with_deleted):
        ...
        composites_ids = list(
            map(
                lambda x: {
                    "ingredient": x["ingredient_id"],
                    "sale_item": x["sale_item_id"],
                },
                composites,
            )
        )
        qset = ProductSaleableItemComposite.objects.filter(
            with_deleted=with_deleted, id__in=composites_ids
        )
        _assert_fn(actual_objs=qset, expect_objs=composites)

    def _assert_media_link_fields_existence(
        self, saleitem_pk, _assert_fn, with_deleted
    ):
        media_meta = self.instances["ProductSaleableItemMedia"][saleitem_pk]
        media_meta_ids = list(map(lambda x: x["id"], media_meta))
        qset = ProductSaleableItemMedia.objects.filter(
            with_deleted=with_deleted, id__in=media_meta_ids
        )
        _assert_fn(actual_objs=qset, expect_objs=media_meta)

    def test_hard_delete_bulk(self):
        backup_saleitem_pks = list(
            map(lambda saleitem: saleitem.pk, self.instances["ProductSaleableItem"])
        )
        ...
        qset.delete(hard=True)
        qset = ProductSaleableItem.objects.filter(
            with_deleted=True, pk__in=backup_saleitem_pks
        )
        self.assertEqual(qset.count(), 1)
        ...
        for saleitem_pk in backup_saleitem_pks:
            ...
            self._assert_composite_fields_existence(
                saleitem_pk, _assert_compo_fn, with_deleted=True
            )
            self._assert_media_link_fields_existence(
                saleitem_pk, _assert_media_fn, with_deleted=True
            )
    ...
    def test_soft_delete_bulk(self):
        ...
        qset.delete(profile_id=profile_id)
        tuple(
            map(lambda obj: self.assertTrue(obj.is_deleted()), qset)
        )  # auto marked as deleted
        ...
        self._check_remaining_saleitems_to_ingredients(
            saleitem_ids=deleted_ids, is_deleted=True
        )
        ...
        for saleitem_pk in deleted_ids:
            ...
            self._assert_composite_fields_existence(
                saleitem_pk, _assert_compo_fn, with_deleted=False
            )
            self._assert_media_link_fields_existence(
                saleitem_pk, _assert_media_fn, with_deleted=False
            )
