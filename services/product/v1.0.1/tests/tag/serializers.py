...
from ecommerce_common.util import ExtendedDict, sort_nested_object
from tests.common import http_request_body_template
...
class TagCommonMixin(HttpRequestDataGenTag, TagVerificationMixin):
    ...
    def _init_new_trees(
        ...
        serializer = self.serializer_class(
            many=True, data=req_data, usrprof_id=self.usrprof_id
        )
...
class TagCreationTestCase(TransactionTestCase, TagCommonMixin):
    ...
    def test_append_new_trees_to_existing_nodes(self):
        ...
        for _ in range(num_rounds):
            ...
            obj_ids = obj_ids + tuple(
                map(lambda d: d["id"], existing_trees.entity_data)
            )
            entity_data, closure_data = self.load_closure_data(
                node_ids=obj_ids
            )  # serializer.data
    ...
    def test_loop_detection_rand_gen_trees(self):
        ...
        origin_new_parent = loop_data_end["data"]["new_parent"]  # noqa: F841
    ...
    def test_loop_detection_self_ref_node(self):
        ...
        with self.assertRaises(ValueError):
            try:
                serializer = self.serializer_class(  # noqa: F841
                    many=True, data=req_data, usrprof_id=self.usrprof_id
                )
...
class TagUpdateTestCase(TransactionTestCase, TagCommonMixin):
    ...
    def _update_and_validate_tree(self, moving_nodes):
        ...
        serializer = self.serializer_class(
            many=True, data=req_data, instance=tag_objs, usrprof_id=self.usrprof_id
        )
        serializer.is_valid(raise_exception=True)
        actual_instances = serializer.save()  # noqa: F841
    ...
    def test_subtrees_move_mix(self):
        self.assertGreaterEqual(
            self.num_trees, 3
        )  # this test case requires at least 3 existing trees
        moving_nodes = []
        ...
        while any(curr_node.children):
            sorted_children = sorted(
                curr_node.children, key=lambda node: node.num_nodes
            )
    ...
    def test_tree_chains(self):
        self.assertGreaterEqual(
            self.num_trees, 2
        )  # this test case requires at least 2 existing trees
        moving_nodes = []
        moving_node = None
        roots = self.existing_trees
        curr_node = roots[0]
    ...
    def test_loop_detection_across_2_trees(self):
        ...
        chosen_subtrees[0].parent, chosen_subtrees[1].parent = (
            chosen_subtrees[1].children[0],
            chosen_subtrees[0].children[0],
        )
        with self.assertRaises(DRFValidationError):
...
class TagRepresentationTestCase(TransactionTestCase, TagCommonMixin):
    ...
    def test_all_descs_of_roots(self):
        ...
        tag_objs = self.serializer_class.Meta.model.objects.filter(id__in=tag_ids)
        serializer = self.serializer_class(
            many=True, instance=tag_objs, usrprof_id=self.usrprof_id
        )
        ...
        for data in actual_data:
            ...
            self._assert_simple_fields(
                check_fields=["id", "name"], exp_sale_item=tree.value, ac_sale_item=data
            )
            ...
            expect_num_descs = nodes.closure_data.filter(
                ancestor=data["id"], depth__gt=0
            ).count()
    ...
    def _assert_adjacent_nodes(self, chosen_subtrees, mocked_request):
        ...
        tag_objs = self.serializer_class.Meta.model.objects.filter(
            id__in=tag_ids
        ).order_by("id")
        serializer = self.serializer_class(
            many=True,
    ...
    def test_adjacent_nodes_only(self):
        ...
        mocked_request.query_params.update(
            {"parent_only": "yes", "children_only": "yes"}
        )
        roots = self.existing_trees
        chosen_subtrees = []
        # --------------------------------
    ...
    def test_all_ascs_of_subtrees(self):
        ...
        for data in actual_data:
            ...
            expect_ancestor = list(
                map(lambda d: d.value["id"], expect_ancestors[tree.value["id"]])
            )
