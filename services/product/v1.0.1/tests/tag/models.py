...
class TagCreationTestCase(TransactionTestCase):
    ...
    def test_null_obj_fields(self):
        ...
        _null_test_obj_attrs(
            testcase=self, instance=self.instance, field_names=field_names
        )
        expect_node_id = 1
    ...
    def test_bulk_ok(self):
        ...
        closure_nodes = [
            ProductTagClosure(depth=0, ancestor=tag, descendant=tag) for tag in tags
        ]
        ...
        actual_closure_ascs = list(
            ProductTagClosure.objects.values_list("ancestor__id", flat=True)
        )
...
class TagDeletionTestCase(TransactionTestCase):
    ...
    def test_hard_delete_one(self):
        ...
        condition = Q(ancestor__id__in=expect_descs) | Q(
            descendant__id__in=expect_descs
        )
        ...
        self.assertFalse(qset_after_delete.exists())
        self.assertEqual(
            (num_all_nodes - num_del_nodes), ProductTagClosure.objects.count()
        )
    ...
    def test_hard_delete_bulk(self):
        ...
        condition = Q(ancestor__id__in=expect_del_descs) | Q(
            descendant__id__in=expect_del_descs
        )
...
class TagQueryTestCase(TransactionTestCase):
    ...
    def test_input_error(self):
        ...
        for invalid_depth in invalid_depth_list:
            actual_fetched_ids = init_qset.get_ascs_descs_id(
                IDs=["root"], fetch_asc=False, fetch_desc=True, depth=invalid_depth
            )
        ...
        for valid_depth in valid_depth_list:
            actual_fetched_ids = init_qset.get_ascs_descs_id(
                IDs=["root"], fetch_asc=False, fetch_desc=True, depth=valid_depth
            )
            self.assertSetEqual(set(expect_fetched_ids), set(actual_fetched_ids))
    ...
    def test_fetch_root_ascs(self):
        ...
        init_qset = ProductTag.objects.all()
        actual_fetched_ids = init_qset.get_ascs_descs_id(
            IDs=tag_ids, fetch_asc=True, fetch_desc=False, self_exclude=False
        )
        self.assertSetEqual(set(expect_fetched_ids), set(actual_fetched_ids))
        actual_fetched_ids = init_qset.get_ascs_descs_id(
            IDs=tag_ids,
        ...
        self.assertSetEqual(set(expect_fetched_ids), set(actual_fetched_ids))
        actual_fetched_ids = init_qset.get_ascs_descs_id(
            IDs=tag_ids, fetch_asc=True, fetch_desc=False, self_exclude=True
        )
        self.assertSetEqual(set(expect_fetched_ids), set(actual_fetched_ids))
    ...
    def test_fetch_root_descs(self):
        ...
        for idx in range(0, len(expect_fetched_id_map[tag_id]) + 1):
            ...
            self.assertSetEqual(
                set(expect_fetched_ids) - set([tag_id]), set(actual_fetched_ids)
            )
    ...
    def test_fetch_non_root_ascs(self):
        ...
        for idx in range(0, len(expect_fetched_ids)):
            ...
            self.assertSetEqual(
                set(expect_fetched_ids[: idx + 1]), set(actual_fetched_ids)
            )
