...
class TagBaseViewTestCase(
    TransactionTestCase,
    _MockTestClientInfoMixin,
    HttpRequestDataGenTag,
    TagVerificationMixin,
):
...
class TagCreationTestCase(TagBaseViewTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.num_trees = 2
        self._origin_trees, self._request_data = self.refresh_req_data(
            shuffle=True,
    ...
    def test_new_trees(self):
        ...
        created_tags = assert_view_bulk_create_with_response(
            testcase=self,
        ...
        saved_trees = TreeNodeMixin.gen_from_closure_data(
            entity_data=entity_data, closure_data=closure_data
        )
        matched, not_matched = TreeNodeMixin.compare_trees(
            trees_a=self._origin_trees,
            trees_b=saved_trees,
        ...
        tree, req_data = self.refresh_req_data(
            shuffle=True,
            num_trees=1,
            min_num_nodes=7,
            max_num_nodes=15,
        ...
        tree, req_data_2 = self.refresh_req_data(
            shuffle=True,
            num_trees=1,
            min_num_nodes=7,
            max_num_nodes=15,
        ...
        entity_data, closure_data = self.load_closure_data(node_ids=tag_ids)
        saved_trees_2 = TreeNodeMixin.gen_from_closure_data(
            entity_data=entity_data, closure_data=closure_data
        )
        matched, not_matched = TreeNodeMixin.compare_trees(
            trees_a=saved_trees,
            trees_b=saved_trees_2,
            value_compare_fn=self._value_compare_fn,
        )
        self.assertListEqual(not_matched, [])
...
class TagUpdateBaseTestCase(TagBaseViewTestCase):
    ...
    def setUp(self):
        super().setUp()
        path = "/tags"
        trees, request_data = self.refresh_req_data(
            shuffle=True,
            num_trees=self.num_trees,
            min_num_nodes=self.min_num_nodes,
            max_num_nodes=self.max_num_nodes,
            min_num_siblings=self.min_num_siblings,
            max_num_siblings=self.max_num_siblings,
        )
        permissions = ["view_producttag", "add_producttag"]
        access_tok_payld = {
            "id": 71,
        ...
        entity_data, closure_data = self.load_closure_data(node_ids=tag_ids)
        self._origin_trees = TreeNodeMixin.gen_from_closure_data(
            entity_data=entity_data, closure_data=closure_data
        )
...
class TagUpdateTestCase(TagUpdateBaseTestCase):
    ...
    def _update_and_validate(
        self, moving_nodes, expect_response_status, post_resp_fn=None
    ):
        req_data = list(
            map(
                lambda node: {
    ...
    def _post_update_response_ok(self, response):
        ...
        matched, not_matched = TreeNodeMixin.compare_trees(
            trees_a=saved_trees,
            trees_b=self._origin_trees,
            value_compare_fn=_value_compare_fn,
        )
        self.assertListEqual(not_matched, [])
        self.assertEqual(len(matched), len(saved_trees))
        ...
        for root in roots:
            ...
            while any(curr_node.children):
                sorted_children = sorted(
                    curr_node.children, key=lambda node: node.depth
                )
                curr_node = sorted_children[-1]
    ...
    def test_loop_detection(self):
        ...
        expect_loop_nodes = [
            ...
            chosen_subtrees[1],
            chosen_subtrees[0],
            chosen_subtrees[2].children[0],
            chosen_subtrees[1].children[0],
            chosen_subtrees[0].children[0],
        ]
        (
            chosen_subtrees[0].parent,
            chosen_subtrees[1].parent,
            chosen_subtrees[2].parent,
        ) = (
            chosen_subtrees[1].children[0],
            chosen_subtrees[2].children[0],
            chosen_subtrees[0].children[0],
        ...
        bound_cb = partial(
            self._post_update_response_loop_detected,
            expect_loop_nodes=expect_loop_nodes,
        )
        self._update_and_validate(
            moving_nodes, expect_response_status=400, post_resp_fn=bound_cb
        )
...
class TagDeletionTestCase(TagUpdateBaseTestCase):
    ...
    def test_descs_delete(self):
        ...
        expect_deleted_tag_ids = model_cls.objects.filter(
            ancestors__ancestor__in=chosen_tag_ids
        ...
        deleted_tags_exists = model_cls.objects.filter(
            id__in=expect_deleted_tag_ids
        ).exists()
...
class TagQueryTestCase(TagUpdateBaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self._access_tok_payld["roles"] = [
            {"app_code": app_code_product, "codename": "view_xxxx"}
        ]
        self._access_token = self.gen_access_token(
            profile=self._access_tok_payld, audience=["product"]
        )
    ...
    def test_load_specific_one(self):
        ...
        response = self._send_request_to_backend(
            path=path, method="get", access_token=self._access_token
        )
        self.assertEqual(int(response.status_code), 200)
...
class TaggedSaleableItemsQueryTestCase(TagUpdateBaseTestCase):
    ...
    def setUp(self):
        ...
        salable_items = list(
            map(
                lambda d: ProductSaleableItem(**d),
                _fixtures["ProductSaleableItem"][:num_tagged_saleable_items],
            )
        )
        salable_pkgs = list(
            map(
                lambda d: ProductSaleablePackage(**d),
                _fixtures["ProductSaleablePackage"][:num_tagged_saleable_items],
            )
        )
        ...
        for saleitem in salable_items:
            ...
            chosen_tags = self.serializer_class.Meta.model.objects.filter(
                id__in=chosen_tag_ids
            )
