...
# some of objects loaded here are re-imported to other test cases in other
# packages, they are unused at this module but used elsewhere, it is reasonable to
# skip lint check for such objects

from django.conf import settings as django_settings
from django.middleware.csrf import _get_new_csrf_token
from django.db.models.constants import LOOKUP_SEP
from django.db.utils import IntegrityError, DataError  # noqa: F401
...
from ecommerce_common.util.messaging.rpc import RpcReplyEvent  # noqa: F401
from ecommerce_common.tests.common import (
    rand_gen_request_body,  # noqa: F401
    listitem_rand_assigner,
    HttpRequestDataGen,  # noqa: F401
    KeystoreMixin,
)
...
_fixtures = {
    ...
    "ProductAttributeType": [
        {
            "id": 20,
            "name": "toppings category",
            "dtype": _ProductAttrValueDataType.STRING.value[0][0],
        },
        {
            "id": 21,
            "name": "color",
        ...
        {
            "id": 23,
            "name": "bread crust level",
            "dtype": _ProductAttrValueDataType.POSITIVE_INTEGER.value[0][0],
        },
        {
            "id": 24,
            "name": "cache size (KBytes)",
            "dtype": _ProductAttrValueDataType.POSITIVE_INTEGER.value[0][0],
        },
        {
            "id": 25,
            "name": "speed limit (km/s)",
            "dtype": _ProductAttrValueDataType.POSITIVE_INTEGER.value[0][0],
        },
        {
            "id": 26,
            "name": "min. working temperature (celsius)",
            "dtype": _ProductAttrValueDataType.INTEGER.value[0][0],
        },
        {
            "id": 27,
            "name": "min. dormant temperature (celsius)",
            "dtype": _ProductAttrValueDataType.INTEGER.value[0][0],
        },
        {
            "id": 28,
        ...
        },
        {
            "id": 30,
            "name": "Diameter (In.)",
            "dtype": _ProductAttrValueDataType.FLOAT.value[0][0],
        },
        {
            "id": 31,
            "name": "max resistence voltage",
            "dtype": _ProductAttrValueDataType.FLOAT.value[0][0],
        },
        {
            "id": 32,
            "name": "min distance between 2 metal wires (um)",
            "dtype": _ProductAttrValueDataType.FLOAT.value[0][0],
        },
    ],
    ...
    "ProductAttributeValueFloat": [
        random.randrange(-100, 100) * 0.31415926 for _ in range(25)
    ],
    ...
    "ProductDevIngredient": [
        ...
        {
            "id": 3,
            "category": ProductDevIngredientType.RAW_MATERIAL,
        ...
        },
        {
            "id": 6,
            "category": ProductDevIngredientType.RAW_MATERIAL,
            "name": "dry yeast powder",
        },
        {
            "id": 7,
            "category": ProductDevIngredientType.WORK_IN_PROGRESS,
            "name": "poolish",
        },
        {
            "id": 8,
        ...
        {
            "id": 18,
            "category": ProductDevIngredientType.EQUIPMENTS,
            "name": "Soldering kit",
        },
        {
            "id": 19,
    ...
    "ProductSaleableItem": [
        {
            "visible": True,
            "unit": random.choice(UnitOfMeasurement.choices)[0],
        ...
        },
        {
            "visible": True,
            "unit": random.choice(UnitOfMeasurement.choices)[0],
            "name": "Semi-prepared Beef Noodle Soup",
            "price": 6.49,
            "usrprof": 53,
        },
    ],
    "ProductSaleablePackage": [
        ...
        {
            "visible": False,
            "name": "Quick cook - beef noodle soup",
            "price": 6.6,
            "usrprof": 204,
        },
        {
            "visible": True,
            "name": "Quick cook - pizza set",
            "price": 6.6,
            "usrprof": 49,
        },
        {
            "visible": False,
            "name": "Trail runner gear set",
            "price": 899.63,
            "usrprof": 17,
        },
        {
            "visible": True,
            "name": "Machine Learning crash Full-course",
            "price": 841.9,
            "usrprof": 37,
        },
        {
            "visible": False,
            "name": "North Face camp gear set",
            "price": 12990.0,
            "usrprof": 701,
        },
    ],
...
def _load_init_params(init_params, model_cls):
    return model_cls(**init_params)


def _modelobj_list_to_map(list_):
    return {item.pk: item for item in list_}


def _dict_key_replace(obj, from_, to_):
    return {to_ if k == from_ else k: v for k, v in obj.items()}


def _dict_kv_pair_evict(obj, cond_fn):
    return dict(filter(cond_fn, obj.items()))
...
def _common_instances_setup(out: dict, models_info, data: dict = None):
    ...
    for model_cls, num_instance_required in models_info:
        ...
        params_gen = listitem_rand_assigner(
            list_=data[model_name],
...
def _null_test_obj_attrs(testcase, instance, field_names):
    for fname in field_names:
        ...
        with testcase.assertRaises(IntegrityError):
            instance.save(force_insert=True)
        setattr(instance, fname, old_value)


def _product_tag_closure_setup(tag_map, data):
    def _gen_closure_node(d):
        return ProductTagClosure(
            id=d["id"],
            depth=d["depth"],
...
def assert_view_permission_denied(
    testcase, request_body_data, path, permissions, http_method="post"
):
    # subcase 1, missing token authentication
    response = testcase._send_request_to_backend(
        path=path, method=http_method, body=request_body_data
    )
    ...
    response = testcase._send_request_to_backend(
        ...
        fn_mocked_get_jwks=fn_mocked_empty_jwks,
    )
    testcase.assertEqual(int(response.status_code), 401)
    ...
    access_token = testcase.gen_access_token(profile=profile, audience=["product"])
    response = testcase._send_request_to_backend(
        path=path, method=http_method, body=request_body_data, access_token=access_token
    )
    testcase.assertEqual(int(response.status_code), 403)


def assert_view_bulk_create_with_response(
    testcase,
    expect_shown_fields,
...
def assert_view_unclassified_attributes(
    testcase, path, body, permissions, method="post"
):
    profile = {
        "id": 321,
        "privilege_status": priv_status_staff,
    ...
    access_token = testcase.gen_access_token(profile=profile, audience=["product"])
    response = testcase._send_request_to_backend(
        path=path, body=body, method=method, access_token=access_token
    )
    ...
    for err_item in err_items:
        ...
        expect_err_msgs = map(
            lambda x: err_msg_pattern % x["type"], expect_item["attributes"]
        )
...
class SoftDeleteCommonTestMixin:
    def assert_softdelete_items_exist(
        self, testcase, deleted_ids, remain_ids, model_cls_path, id_label="id"
    ):
        model_cls = import_module_string(dotted_path=model_cls_path)
        changeset = model_cls.SOFTDELETE_CHANGESET_MODEL
        ct_model_cls = changeset.content_type.field.related_model.objects.get_for_model(
            model_cls
        )
        cset = changeset.objects.filter(
            content_type=ct_model_cls, object_id__in=deleted_ids
        )
        ...
        testcase.assertSetEqual(
            set(qset.values_list(id_label, flat=True)), set(remain_ids)
        )
        qset = model_cls.objects.get_deleted_set()
        testcase.assertSetEqual(
            set(qset.values_list(id_label, flat=True)), set(deleted_ids)
        )

    def _softdelete_by_half(
        self,
        remain_items,
        deleted_items,
        testcase,
        ...
        for chosen_items in chosen_items_set:
            ...
            response = testcase._send_request_to_backend(
                path=api_url, method="delete", body=body, access_token=access_token
            )
            ...
            deleted_ids = list(
                map(lambda c: _get_inst_attr(obj=c, attname="id"), deleted_items)
            )
            remain_ids = list(
                map(lambda c: _get_inst_attr(obj=c, attname="id"), remain_items)
            )
    ...
    def perform_undelete(
        self,
        testcase,
        path,
        access_token,
        body=None,
        expect_resp_status=200,
        expect_resp_msg="recovery done",
    ):
        # note that body has to be at least {} or [], must not be null
        # because the content-type is json
        body = body or {}
        response = testcase._send_request_to_backend(
            path=path, method="patch", body=body, access_token=access_token
        )
...
class AttributeDataGenMixin:
    ...
    def _gen_attr_val(self, attrtype, extra_amount_enabled):
        ...
        nested_item = {
            "id": None,
            "type": _get_inst_attr(attrtype, "id"),
            "value": None,
        }

        def _fn(option):
            return option.value[0][0] == _get_inst_attr(attrtype, "dtype")
    ...
    def gen_attr_vals(self, extra_amount_enabled, attr_type_src=None):
        if attr_type_src:
            num_attrvals = random.randrange(
                self.min_num_applied_attrs, len(attr_type_src)
            )
        else:
            attr_type_src = _fixtures["ProductAttributeType"]
            num_attrvals = random.randrange(
                self.min_num_applied_attrs, self.max_num_applied_attrs
            )
        attr_dtypes_gen = listitem_rand_assigner(
            list_=attr_type_src,
            min_num_chosen=num_attrvals,
            max_num_chosen=(num_attrvals + 1),
        )
        bound_gen_attr_val = partial(
            self._gen_attr_val, extra_amount_enabled=extra_amount_enabled
        )
...
class BaseVerificationMixin:
    ...
    def _assert_simple_fields(self, check_fields, exp_sale_item, ac_sale_item):
        ...
        bound_assert_fn = partial(
            assert_field_equal,
            testcase=self,
            expect_obj=exp_sale_item,
            actual_obj=ac_sale_item,
        )
        tuple(map(bound_assert_fn, check_fields))

    def _assert_serializer_validation_error(
        self, testcase, serializer, reset_validation_result=True
    ):
        error_details = None
        possible_exception_classes = (
    ...
    def _assert_single_invalid_case(
        self,
        testcase,
        field_name,
        invalid_value,
        ...
        origin_error_details = self._assert_serializer_validation_error(
            testcase=testcase, serializer=serializer
        )
...
class AttributeAssertionMixin:
    ...
    def _assert_attributes_data_change(
        self, data_before_validate, data_after_validate, skip_attr_val_id=False
    ):
        ...
        def _flatten_attr_vals(item):
            def evict_condition(kv):
                return kv[0] not in (
                    "ingredient_type",
                    "ingredient_id",
                )

            attrtype_key_replace_fn = partial(
                _dict_key_replace, to_="type", from_="attr_type"
            )
            ingredient_ctype_kv_evict_fn = partial(
                _dict_kv_pair_evict, cond_fn=evict_condition
            )
            flattened_attrvals = [
                attr_val
        ...
        attrs_before_validate = list(
            map(lambda x: x["attributes"], data_before_validate)
        )
        attrs_after_validate = list(map(_flatten_attr_vals, data_after_validate))
        if skip_attr_val_id:
            attrs_before_validate = list(
                map(_evict_attr_val_id_fn, attrs_before_validate)
            )
            attrs_after_validate = list(
                map(_evict_attr_val_id_fn, attrs_after_validate)
            )
    ...
    def _assert_product_attribute_fields(self, exp_sale_item, ac_sale_item):
        def cpompare_attribute_fields(kv):
            cond1 = kv[0] not in ("id", "ingredient_type", "ingredient_id")
            cond2 = kv[0] == "extra_amount" and kv[1] is None
            return cond1 and not cond2

        bound_dict_kv_pair_evict = partial(
            _dict_kv_pair_evict, cond_fn=cpompare_attribute_fields
        )
        bound_dict_key_replace = partial(
            _dict_key_replace, to_="extra_amount", from_="_extra_charge__amount"
        )
        ...
        for dtype_option in _ProductAttrValueDataType:
            ...
            expect_vals = sorted(
                expect_vals, key=lambda x: x["attr_type"]
            )  # noqa: E731
            actual_vals = sorted(
                actual_vals, key=lambda x: x["attr_type"]
            )  # noqa: E731
            expect_vals = json.dumps(expect_vals, sort_keys=True)
            actual_vals = json.dumps(actual_vals, sort_keys=True)
            self.assertEqual(expect_vals, actual_vals)

    def _test_unclassified_attribute_error(
        self, testcase, serializer, request_data, assert_single_invalid_case_fn
    ):
        invalid_cases = [
        ...
        def fn_choose_edit_item(x):
            return x[idx]["attributes"][jdx]
        ...
        for field_name, invalid_value, expect_err_msg in invalid_cases:
            for idx in range(len(request_data)):
                ...
                jdx = random.randrange(
                    0, len(serializer.initial_data[idx]["attributes"])
                )
    ...
    def _test_unclassified_attributes_error(
        self,
        serializer,
        request_data,
        testcase,
        assert_serializer_validation_error_fn,
        num_rounds=10,
        extra_invalid_cases=None,
        field_name="type",
    ...
    def _test_incorrect_attribute_value(
        self,
        serializer,
        request_data,
        testcase,
        ...
        _allowed_type_transitions = [
            (
                _ProductAttrValueDataType.FLOAT.value[0][0],
                _ProductAttrValueDataType.INTEGER.value[0][0],
            ),
            (
                _ProductAttrValueDataType.FLOAT.value[0][0],
                _ProductAttrValueDataType.POSITIVE_INTEGER.value[0][0],
            ),
            (
                _ProductAttrValueDataType.INTEGER[0][0],
                _ProductAttrValueDataType.POSITIVE_INTEGER.value[0][0],
            ),
            (
                _ProductAttrValueDataType.STRING.value[0][0],
                _ProductAttrValueDataType.INTEGER.value[0][0],
            ),
            (
                _ProductAttrValueDataType.STRING.value[0][0],
        ...
        for _ in range(num_rounds):
            ...
            while len(idx_to_attrs.keys()) < num_attr_fixture:
                ...
                if idx_to_attrs.get((idx, jdx)) is None:
                    attrtype_id = serializer.initial_data[idx]["attributes"][jdx][
                        "type"
                    ]
                    attrtype = filter(
                        lambda obj: obj.id == attrtype_id,
                        testcase.stored_models["ProductAttributeType"],
                    )
                    attrtype = tuple(attrtype)[0]
                    dtype_keys = list(_attr_fixture.keys())
                    dtype_keys.remove(
                        attrtype.dtype
                    )  # create invalid case by giving different data type of value
...
class _MockTestClientInfoMixin(_BaseMockTestClientInfoMixin, KeystoreMixin):
    ...
    _keystore_init_config = {
        ...
        "persist_secret_handler": django_settings.AUTH_KEYSTORE[
            "persist_secret_handler_test"
        ],
        "persist_pubkey_handler": django_settings.AUTH_KEYSTORE[
            "persist_pubkey_handler_test"
        ],
    }
    ...
    def _send_request_to_backend(
        ...
        method="post",
        body=None,
        expect_shown_fields=None,
        ids=None,
        extra_query_params=None,
        access_token="",
        extra_headers=None,
        fn_mocked_get_jwks=None,
    ):
        ...
        with patch("jwt.PyJWKClient.fetch_data", fn_mocked_get_jwks):
            response = super()._send_request_to_backend(
                path=path,
                method=method,
                body=body,
                ids=ids,
                ...
                headers=headers,
                enforce_csrf_checks=base_params["enforce_csrf_checks"],
