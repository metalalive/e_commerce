...
from tests.common import (
    ...
    assert_view_unclassified_attributes,
...
class DevIngredientBaseViewMixin(
    TransactionTestCase, _MockTestClientInfoMixin, HttpRequestDataGenDevIngredient
):
    permission_class = FabricationIngredientPermissions

    def refresh_req_data(self, num_create=None):
        return super().refresh_req_data(
            fixture_source=model_fixtures["ProductDevIngredient"],
...
class DevIngredientCreationTestCase(DevIngredientBaseViewMixin):
    ...
    def test_permission_denied(self):
        kwargs = {
            "testcase": self,
            "request_body_data": self._request_data,
            "path": self.path,
...
class DevIngredientUpdateBaseTestCase(
    DevIngredientBaseViewMixin, DevIngredientVerificationMixin
):
    path = "/ingredients"
    ...
    def setUp(self):
        ...
        self._created_items = self.serializer_class.Meta.model.objects.filter(
            id__in=created_ids
        )
...
class DevIngredientUpdateTestCase(DevIngredientUpdateBaseTestCase):
    ...
    def test_invalid_id(self):
        ...
        self.assertTrue(any(err_items))
        self.assertNotEqual(
            err_items[0].get(drf_default_settings["NON_FIELD_ERRORS_KEY"]), None
        )
        ...
        self.assertTrue(any(err_items))
        self.assertNotEqual(
            err_items.get(drf_default_settings["NON_FIELD_ERRORS_KEY"]), None
        )
    ...
    def _update_attr_val_id(self, src, dst):
        ...
        def gen_map_fn(data):
            return {
                d["id"]: {a.get("id", None): a for a in d["attributes"]} for d in data
            }
    ...
    def test_bulk_ok(self):
        ...
        unaffected_data = self._request_data[num_edit_data:]  # noqa: F841
    ...
    def test_conflict_attribute_id_different_dtypes(self):
        ...
        discarded_attr_type_obj = ProductAttributeType.objects.get(
            id=discarded_attr_type
        )
        # there must be at least one attribute with different data type from any other attributes
        chosen_attr = tuple(
            filter(
                lambda a: ProductAttributeType.objects.get(id=a["type"]).dtype
                != discarded_attr_type_obj.dtype,
                editing_attrs,
            )
        )
        ...
        edited_data = response.json()
        self.assertEqual(int(response.status_code), 200)
        actual_ingredient = self.serializer_class.Meta.model.objects.get(
            id=editing_data[0]["id"]
        )
        attrval_field_name = _ProductAttrValueDataType.related_field_map(
            discarded_attr_type_obj.dtype
        )
        attrval_manager = getattr(actual_ingredient, attrval_field_name)
        discarded_attrval_exists = attrval_manager.filter(
            id=discarded_attrval_id
        ).exists()
        ...
        expect_new_attrval_id = filter(
            lambda a: a["type"] == discarded_attr_type, edited_data[0]["attributes"]
        )
...
class DevIngredientDeletionTestCase(
    DevIngredientUpdateBaseTestCase, SoftDeleteCommonTestMixin
):
    ...
    def test_softdelete_ok(self):
        num_delete = 2
        deleted_ids = list(
            map(lambda x: {"id": x.id}, self._created_items[:num_delete])
        )
        response = self._send_request_to_backend(
            path=self.path,
            method="delete",
    ...
    def _verify_undeleted_items(self, undeleted_items):
        ...
        self.verify_objects(
            actual_instances=undeleted_objs, expect_data=undeleted_items
        )
    ...
    def test_undelete_by_time(self):
        ...
        while any(deleted_items):
            ...
            undeleted_ids = tuple(map(lambda x: x["id"], undeleted_items))
            moving_gen = tuple(
                filter(lambda obj: obj.id in undeleted_ids, deleted_items)
            )
...
class DevIngredientQueryTestCase(
    DevIngredientUpdateBaseTestCase, DevIngredientVerificationMixin
):
    def setUp(self):
        super().setUp()
    ...
    def test_bulk_items_partial_info(self):
        ...
        read_items_gen = listitem_rand_assigner(
            list_=self._created_items, max_num_chosen=(num_read + 1)
        )
        ...
        self.assertEqual(int(response.status_code), 200)
        self.verify_objects(
            actual_instances=expect_items,
            expect_data=actual_items,
            non_nested_fields=["id", "name"],
        )


class DevIngredientAdvancedSearchTestCase(
    DevIngredientUpdateBaseTestCase, DevIngredientVerificationMixin
):
    min_num_applied_attrs = 4
    ...
    def test_advanced_search_attr_str(self):
        ...
        qset = self._model_cls.objects.annotate(
            num_str_attr=Count("attr_val_str")
        ).filter(num_str_attr__gt=0)
    ...
    def test_advanced_search_attr_int(self):
        variation = 100
        qset = self._model_cls.objects.annotate(
            num_int_attr=Count("attr_val_int")
        ).filter(num_int_attr__gt=0)
        for expect_obj in qset:
            for attrval in expect_obj.attr_val_int.all():
                ...
                search_result = tuple(
                    filter(lambda x: x["id"] == expect_obj.id, actual_items)
                )
                self.assertEqual(len(search_result), 1)

    def test_advanced_search_attr_float(self):
        qset = self._model_cls.objects.annotate(
            num_float_attr=Count("attr_val_float")
        ).filter(num_float_attr__gt=0)
        data = qset.values(
            "id",
            "attr_val_float__attr_type__id",
            "attr_val_float__attr_type__dtype",
            "attr_val_float__value",
        )
                ...
                "operands": [
                    {
                        ...
                        "operands": [
                            "attributes__type",
                            d["attr_val_float__attr_type__id"],
                        ],
                        ...
                        "operands": [
                            {
                                ...
                                "operands": ["attributes__value", lower_bound],
                                "metadata": {
                                    "dtype": d["attr_val_float__attr_type__dtype"]
                                },
                            },
                            {
                                "operator": "<=",
    ...
    def test_advanced_nested_search_mix(self):
        ...
        attr_choices = {
            ingredient.id: {
                dtype_opt.value[0]: list(
                    getattr(ingredient, dtype_opt.value[0][1]).values(
                        "id", "attr_type", "value"
                    )
                )
        ...
        attr_choices = tuple(
            filter(lambda kv: len(kv[1].keys()) > 1, attr_choices.items())
        )
        ...
        items_found = self._test_advanced_search_common(adv_cond=adv_cond)
        actual_item = tuple(
            filter(lambda d: d["id"] == expect_ingredient_id, items_found)
        )
