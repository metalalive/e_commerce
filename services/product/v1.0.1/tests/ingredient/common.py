...
class HttpRequestDataGenDevIngredient(HttpRequestDataGen, AttributeDataGenMixin):
    ...
    def rand_gen_edit_data(self, editing_data):
        ...
        name_gen = listitem_rand_assigner(
            list_=new_name_options,
            distinct=False,
            min_num_chosen=num_edit_data,
            max_num_chosen=(num_edit_data + 1),
        )
        category_gen = listitem_rand_assigner(
            list_=ProductDevIngredientType.choices,
            distinct=False,
            min_num_chosen=num_edit_data,
            max_num_chosen=(num_edit_data + 1),
        )
        for edit_item in editing_data:
            ...
            edit_attr["value"] = (
                "new value is a text" if is_text else (old_attr_value * 2)
            )
            added_attrtype_ids = tuple(
                map(lambda x: x["type"], edit_item["attributes"])
            )
            unadded_attrtypes = tuple(
                filter(
                    lambda x: x["id"] not in added_attrtype_ids,
...
class DevIngredientVerificationMixin(BaseVerificationMixin, AttributeAssertionMixin):
    ...
    def verify_objects(
        self, actual_instances, expect_data, extra_check_fn=None, non_nested_fields=None
    ):
