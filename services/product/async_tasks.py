...
@celery_app.task(backend=CeleryRpcBackend(app=celery_app), queue='rpc_productmgt_get_product', \
         exchange=RPC_EXCHANGE_DEFAULT_NAME, routing_key='rpc.product.get_product')
@log_fn_wrapper(logger=_logger, loglevel=logging.WARNING, log_if_succeed=False)
def get_product(item_ids:List[int], pkg_ids:List[int], item_fields:List[str], pkg_fields:List[str], \
        profile:int ) -> dict:
    ...
    _map = {
        SaleableItemSerializer: {'ids':item_ids, 'fields':item_fields,  'output_key':'item'} ,
        SaleablePackageSerializer: {'ids':pkg_ids, 'fields':pkg_fields, 'output_key':'pkg'} ,
    }
    out = {}
    for serializer_cls, _info in _map.items():
        model_cls = serializer_cls.Meta.model
        qset = model_cls.objects.filter(id__in=_info['ids'], usrprof=profile, visible=True)
        class fake_request:
