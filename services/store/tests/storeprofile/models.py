...
class TestCreation:  # class name must start with TestXxxx
    def test_bulk_ok(self, session_for_test, store_data):
        ...
        chk_result1 = StoreProfile.quota_stats(
        ...
        chk_result2 = StoreProfile.quota_stats(
    ...
    def test_quota_statistics(self, session_for_test, store_data):
        ...
        chk_result = StoreProfile.quota_stats(
    ...
    def test_bulk_with_related_fields(
        self,
        session_for_test,
        store_data,
        email_data,
        ...
        query = session_for_test.query(StoreProfile).filter(StoreProfile.id.in_(ids))
        objs = query.all()
        chk_result = StoreEmail.quota_stats(
        ...
        chk_result = StorePhone.quota_stats(
            [], session=session_for_test, target_ids=ids
        )
        for store_id, info in chk_result.items():
...
class TestUpdate:
    def test_edit_contact(
        ...
        obj = query.first()
        stats_before_update = StorePhone.quota_stats(
            [], session=session_for_test, target_ids=[obj.id]
        )
        ...
        obj = query.first()
        stats_after_update = StorePhone.quota_stats(
            [], session=session_for_test, target_ids=[obj.id]
        )
        ...
        assert (
            1 + stats_before_update[obj.id]["num_existing_items"]
