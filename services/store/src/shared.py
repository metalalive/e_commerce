...
def _init_db_engine(conn_args:Optional[dict]=None):
    ...
    """
    kwargs = {
        'secrets_file_path':_settings.SECRETS_FILE_PATH, 'base_folder':_settings.SYS_BASE_PATH,
        'secret_map':(_settings.DB_USER_ALIAS, 'backend_apps.databases.%s' % _settings.DB_USER_ALIAS),
        'driver_label':_settings.DRIVER_LABEL, 'db_name':_settings.DB_NAME,
    }
    if conn_args:
...
async def app_shared_context_start(_app:FastAPI):
    data = {
        ...
        'product_app_rpc':RPCproxy(dst_app_name='product', src_app_name='store',
            srv_basepath = str(_settings.SYS_BASE_PATH)),
        'order_app_rpc':RPCproxy(dst_app_name='order', src_app_name='store',
            srv_basepath = str(_settings.SYS_BASE_PATH)),
        'auth_keystore': create_keystore_helper(cfg=_settings.KEYSTORE, import_fn=import_module_string),
