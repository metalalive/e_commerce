...
from ecommerce_common.migrations.alembic.config import downgrade_migration, auth_provider_upgrade, resource_app_upgrade
from settings import migration as mig_settings


def auth_migrate_forward(param):
    auth_curr_rev_id = '000002'
    auth_provider_upgrade(app_settings=mig_settings,
            init_round=True, next_rev_id=auth_curr_rev_id)

def store_migrate_forward(param):
    dependent_rev_id = None if param.prev_rev_id.lower() == 'init' \
            else [param.prev_rev_id]
    kwargs = {
        'app_settings':mig_settings, 'next_rev_id':param.new_rev_id,
        'new_label':param.new_label, 'dependent_rev_id':dependent_rev_id
    }
    resource_app_upgrade(**kwargs)

def migrate_backward(param):
    prev_rev_id = 'base' if param.prev_rev_id.lower() == 'init' \
            else param.prev_rev_id
...
if __name__ == "__main__":
    toplvl_parser = argparse.ArgumentParser(description="dev tools for store-front service maintenance")
    ...
    subparser.add_argument('--new-label', required=True, help='New label associated with the migration')
