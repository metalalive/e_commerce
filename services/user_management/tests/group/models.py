...
_data = {
    GenericUserGroupClosure: [
        ...
        #
        {
            "id": 1,
            "depth": 0,
            "ancestor": GenericUserGroup(id=3),
            "descendant": GenericUserGroup(id=3),
        },
        {
            "id": 2,
            "depth": 0,
            "ancestor": GenericUserGroup(id=4),
            "descendant": GenericUserGroup(id=4),
        },
        {
            "id": 3,
            "depth": 0,
            "ancestor": GenericUserGroup(id=5),
            "descendant": GenericUserGroup(id=5),
        },
        {
            "id": 4,
            "depth": 0,
            "ancestor": GenericUserGroup(id=6),
            "descendant": GenericUserGroup(id=6),
        },
        {
            "id": 5,
            "depth": 0,
            "ancestor": GenericUserGroup(id=7),
            "descendant": GenericUserGroup(id=7),
        },
        {
            "id": 6,
            "depth": 0,
            "ancestor": GenericUserGroup(id=8),
            "descendant": GenericUserGroup(id=8),
        },
        {
            "id": 7,
            "depth": 0,
            "ancestor": GenericUserGroup(id=9),
            "descendant": GenericUserGroup(id=9),
        },
        {
            "id": 8,
            "depth": 0,
            "ancestor": GenericUserGroup(id=10),
            "descendant": GenericUserGroup(id=10),
        },
        {
            "id": 9,
            "depth": 0,
            "ancestor": GenericUserGroup(id=11),
            "descendant": GenericUserGroup(id=11),
        },
        {
            "id": 10,
            "depth": 0,
            "ancestor": GenericUserGroup(id=12),
            "descendant": GenericUserGroup(id=12),
        },
        {
            "id": 11,
            "depth": 0,
            "ancestor": GenericUserGroup(id=13),
            "descendant": GenericUserGroup(id=13),
        },
        {
            "id": 12,
            "depth": 0,
            "ancestor": GenericUserGroup(id=14),
            "descendant": GenericUserGroup(id=14),
        },
        {
            "id": 13,
            "depth": 1,
...
class GroupDeletionTestCase(TransactionTestCase):
    def setUp(self):
        ...
        for quota_mat in objs[QuotaMaterial]:
            d = {
                "expiry": timezone.now(),
                "material": quota_mat,
                "maxnum": random.randrange(1, 25),
            }
            quota_rel = UserQuotaRelation(**d)
            objs[UserQuotaRelation].append(quota_rel)
        profile = GenericUserProfile.objects.create(
            id=4, first_name="Knowledge", last_name="Hoarder"
        )
    ...
    def _assert_hierarchy_change(self, nodes_before_delete, nodes_after_delete):
        ...
        for del_id in self._delete_grp_ids:
            keys = [
                (asc, desc)
                for asc in affected_path_map[del_id]["asc"]
                for desc in affected_path_map[del_id]["desc"]
            ]
    ...
    def test_bulk_hard_delete(self):
        ...
        remain_grps = filter(
            lambda obj: obj.pk not in self._delete_grp_ids, self._objs[GenericUserGroup]
        )
    ...
    def test_bulk_soft_delete(self):
        ...
        actual_softdelete_emails = list(
            self._chosen_grp.emails.get_deleted_set().values("id", "addr")
        )
        ...
        expect_softdelete_role_rels = set(
            map(lambda obj: obj.role.id, self._objs[GenericUserAppliedRole])
        )
        actual_softdelete_role_rels = set(
            self._chosen_grp.roles.get_deleted_set().values_list("role", flat=True)
        )
    ...
    def _bulk_undelete_evict_hierarchy(self, delete_grp_ids, delete_2nd_grp_id, expect_evict_ids):
        ...
        group_hierarchy = TreeNodeMixin.gen_from_closure_data(
            entity_data=grps_after_undelete, closure_data=grp_hier_after_undelete
        )
        grp_hier_after_undelete = list(grp_hier_after_undelete)
    ...
    def test_bulk_undelete_evict_hierarchy_1(self):
        self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(7, 5), delete_2nd_grp_id=4, expect_evict_ids=[7]
        )

    def test_bulk_undelete_evict_hierarchy_2(self):
        self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(4, 5), delete_2nd_grp_id=7, expect_evict_ids=[4]
        )

    def test_bulk_undelete_evict_hierarchy_3(self):
        self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(4, 5), delete_2nd_grp_id=11, expect_evict_ids=[4]
        )

    def test_bulk_undelete_evict_hierarchy_4(self):
        group_hierarchy = self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(11, 5), delete_2nd_grp_id=4, expect_evict_ids=[11]
        )
    ...
    def test_bulk_undelete_evict_hierarchy_5(self):
        group_hierarchy = self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(3, 6), delete_2nd_grp_id=7, expect_evict_ids=[3]
        )
    ...
    def test_bulk_undelete_evict_hierarchy_6(self):
        group_hierarchy = self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(7, 6), delete_2nd_grp_id=3, expect_evict_ids=[6, 7]
        )
    ...
    def test_bulk_undelete_evict_hierarchy_7(self):
        group_hierarchy = self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(8, 7, 6), delete_2nd_grp_id=3, expect_evict_ids=[6, 7, 8]
        )
    ...
    def test_bulk_undelete_evict_hierarchy_8(self):
        group_hierarchy = self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(11, 14), delete_2nd_grp_id=5, expect_evict_ids=[14]
        )
    ...
    def test_bulk_undelete_evict_hierarchy_9(self):
        group_hierarchy = self._bulk_undelete_evict_hierarchy(
            delete_grp_ids=(11, 4), delete_2nd_grp_id=13, expect_evict_ids=[4]
        )
