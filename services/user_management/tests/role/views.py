...
class RoleCreationTestCase(
    ...
    def test_input_error(self):
        ...
        body = [
            ...
            {"name": "security vend\x00r", "permissions": []},
        ]
        self.api_call_kwargs["body"] = body
        response = self._send_request_to_backend(**self.api_call_kwargs)
        self.assertEqual(int(response.status_code), 400)
        ...
        invalid_perm_id = "-123"
        body = [
            {"name": "Museu\x02m", "permissions": [invalid_perm_id]},
            {"name": "security vendor", "permissions": ["1o4"]},
        ]
        self.api_call_kwargs["body"] = body
        response = self._send_request_to_backend(**self.api_call_kwargs)
        self.assertEqual(int(response.status_code), 400)
    ...
    def test_bulk_ok(self):
        ...
        expect_result = body
        actual_result = list(
            map(lambda d: {key: d[key] for key in ("name", "permissions")}, result)
        )
...
class _RoleBaseUpdateTestCase(
    ...
    def _prepare_access_token(self, new_perms_info, app_labels=None):
        ...
        qset = Permission.objects.filter(
            content_type__app_label__in=app_labels, codename__in=new_perms_info
        )
        if qset.exists():
            self._roles[1].permissions.set(qset)
        acs_tok_resp = self._refresh_access_token(testcase=self, audience=["user_management"])
...
class RoleUpdateTestCase(_RoleBaseUpdateTestCase):
    ...
    def test_input_error(self):
        ...
        # subcase #2, lacking essential fields
        body = [
            {fd_name: getattr(role, fd_name) for fd_name in ("id", "name")}
            for role in self._roles[2:]
        ]
        body[0].pop("id", None)
        body[1]["id"] = None
...
class RoleQueryTestCase(_RoleBaseUpdateTestCase):
    ...
    def setUp(self):
        ...
        extra_role_data = [
            {
                "id": 10,
                "name": "unapplied role 001",
            },
            {
                "id": 11,
                "name": "unapplied role 002",
            },
            {
                "id": 12,
