...
class AccountActivationTestCase(BaseViewTestCase):
    ...
    def setUp(self):
        ...
        tuple(
            map(
                lambda profile: profile.groups.create(
                    group=group, approved_by=self._default_login_profile
                ),
                profiles,
            )
        )
        ...
        api_call_kwargs["headers"]["HTTP_AUTHORIZATION"] = " ".join(
            ["Bearer", default_user_access_token]
        )
    ...
    def test_invalid_input(self):
        ...
        body = [
            {},
            {"profile": None, "email": None},
            {"profile": -123, "email": -123},
    ...
    def test_reactivate(self):
        ...
        body = list(
            map(
                lambda profile: {"email": profile.emails.first().id},
                self._test_profiles[:-1],
            )
        )
...
class AccountDeactivationTestCase(BaseViewTestCase):
    ...
    def setUp(self):
        ...
        perm_objs = Permission.objects.filter(
            content_type__app_label="user_management", codename__in=perms_code_required
        )
        roles[1].permissions.set(perm_objs)
    ...
    def test_deactivate_ok(self):
        body = list(
            map(
                lambda profile: {"profile": profile.id, "remove_account": False},
                self._test_profiles,
            )
        )
...
class AuthEditAccountTestCase(BaseViewTestCase):
    def setUp(self):
        ...
        perm_objs = Permission.objects.filter(
            content_type__app_label="user_management",
            codename__in=["view_quotamaterial"],
        )
        roles[1].permissions.set(perm_objs)
        profile = GenericUserProfile.objects.create(**_fixtures[GenericUserProfile][0])
        self._setup_user_roles(
