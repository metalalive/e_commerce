...
from ecommerce_common.models.closure_table import (
    ClosureTableModelMixin,
...
class QuotaMaterial(models.Model):
    ...
    * This model is NOT for frontend client that attempts to modify the content of this model
    ...
    app_code = models.PositiveSmallIntegerField(
        null=False, blank=False, choices=AppCodeOptions.choices
    )
...
class PhoneNumber(AbstractUserRelation):
    ...
    ccode_validator = RegexValidator(
        regex=r"^\d{1,3}$",
        message="non-digit character detected, or length of digits doesn't meet requirement. It must contain only digit e.g. '91', '886' , from 1 digit up to 3 digits",
    )
    linenum_validator = RegexValidator(
        regex=r"^\+?1?\d{7,15}$",
...
class GenericUserGroup(GenericUserCommonFieldsMixin, MinimumInfoMixin):
    ...
    def filter_before_recover(self, records_in):
        return filter_closure_nodes_recovery(
...
class GenericUserProfile(GenericUserCommonFieldsMixin, MinimumInfoMixin):
    ...
    def undelete(self, *args, **kwargs):
        ...
        try:
            ...
            qset = self.SOFTDELETE_CHANGESET_MODEL.objects.filter(
                content_type=prof_cls_ct, object_id=self.pk
            )
            if not qset.exists():
                raise
    ...
    def estimate_inherit_quota(cls, groups):
        ...
        qset = (
            ...
            .values(quota_mat_field)
            .annotate(max_num_chosen=models.Max(quota_val_field))
            .order_by(quota_mat_field)
            .distinct()
            .filter(cond_after_groupby)
        )
    ...
    def inherit_roles(self):
        ...
        valid_role_rel_cond = (
            ...
            & models.Q(user_id__in=asc_ids)
            & role_rel_cls.expiry_condition()
        )
        role_ids = role_rel_cls.objects.filter(valid_role_rel_cond).values_list(
            "role__id", flat=True
        )
