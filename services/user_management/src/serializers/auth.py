    ...
    def __call__(self, value):
        ...
        if account.pk and account.username == value:
            ...
            try: # invoke existing validator at model level
                ...
                err_list = tuple(filter(lambda item: item.message.find("already exist") > 0, err_list))
                if any(err_list):
        ...
        if errmsg:
            log_msg = ['errmsg', errmsg, 'value', value, 'account_id', account.pk,
...
class  LoginAccountSerializer(Serializer):
    ...
    def __init__(self, data, account, rst_req, confirm_passwd=False, uname_required=False,
            old_uname_required=False,  old_passwd_required=False, passwd_required=False,
            mail_kwargs=None, **kwargs):
        ...
        if uname_required:
            ...
            self.fields['username'] = CharField(required=True, max_length=32, min_length=8, )
        ...
        if passwd_required:
            ...
            self.fields['password'] = CharField(required=True, max_length=128, min_length=12,)
        ...
        if old_uname_required:
            old_uname_validator = StringEqualValidator(limit_value=account.username, message="incorrect old username")
        ...
        if old_passwd_required:
            old_passwd_validator = OldPasswdValidator(limit_value=account.password, message="incorrect old password")
